sub EVENT_SAY {
 if($text =~ /Hail, a Tunarian scout/i) {
	plugin::Whisper("What a [sad] day no?");
 }
 if($text =~ /sad/i) {
	plugin::Whisper("Why is it sad? Mother Tunare has blessed nature with many beautiful things, and Innoruuk takes it upon himself to desecrate such beauty with the creation of [abominations] to the very senses. Creatures created, mind you, in his very image. These foul beings slither about and do his bidding, destroying and enslaving, all the while mocking the beauty of creation. It makes me ill to think upon it.");
 }
 if($text =~ /abominations/i) {
	plugin::Whisper("I have heard them referred to as Luggalds. They reside upon the blasted island of Broken Skull Rock. I was just now watching the seas to see perhaps if there was any word from our last scout we sent investigating. Unfortunately it has been many weeks and I fear that something has happened to him. Such is too often the fate of truly brave scouts, to disappear and simply never discover their fate. [Fate and mortality] are not why I am here this day, however.' The elf shifts his weight a bit and turns back to the sea, seeming to forget you are there.");
 }
 if($text =~ /fate and mortality/i) {
	plugin::Whisper("Fate, mortality, idle concepts that do not highlight my purpose. I am here for honor, and for Mother Tunare. Should our scout not turn up in a few days time I will need to go in his place and leave the coastline rather unprotected. Scouts are so few in numbers these days, there are too many who would overlook such a post in lieu of grandeur and riches. We can always use more good [help].");
 }
 if($text =~ /help./i) {
