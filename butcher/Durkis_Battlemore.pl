...
sub EVENT_SAY {
	if($text=~/Hail/i)   {
		plugin::Whisper("Hail, $name.");#Text made up
	}
}

sub EVENT_ITEM {
	if (plugin::check_handin(\%itemcount, 8910 => 1)) { #A Note to Battlemore
		plugin::Whisper("What have ye got there, lad? Ack. I thought I would never hear from that crazy old man again. He wants me to give you a pair o' me gauntlets. I suppose I kin do that. Here you are, $name . Wear them in good health.");
