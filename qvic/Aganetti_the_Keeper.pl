sub EVENT_SAY {
  if ($text=~/hail/i) {
    ...
    if(!$entity_list->GetMobByNpcTypeID(295140) && !$entity_list->GetMobByNpcTypeID(295149)) {
      plugin::Whisper("Very well! If you will not heed my warning, then you have chosen to suffer the consequences. I grant you entrance to Inktu'ta.");
      ...
      plugin::Whisper("The power of the Muramites is blocking the entrance to the temple.");
    }
  }
}