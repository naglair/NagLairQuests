	...
	if ($raid) {
		...
		for ($count = 0; $count < $raid->RaidCount(); $count++) {
			...
			if ($rpc->GetZoneID() == $zoneid) {
				$rpc->SetGlobal("Severilous",1,5,"F");
				$rpc->SendMarqueeMessage(15, 510, 1, 1, 3000, "Your raid received credit for killing Severilous.");
			}
		}
	}
	else {
		$client->SetGlobal("Severilous",1,5,"F");
		$client->SendMarqueeMessage(15, 510, 1, 1, 3000, "Your group received credit for killing Severilous.");
	}
}
