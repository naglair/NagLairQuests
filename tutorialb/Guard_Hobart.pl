...
sub EVENT_SAY {
   if($text=~/Hail/i) {
      ...
      if (quest::istaskactivityactive(30,1)) {
         ...
         plugin::Whisper("Excellent work, my friend. Take this potion brewed from Queen Gloomfang's chitin. May it make you strong enough to avenge the many slaves the kobolds have fed her.");
         ...
         plugin::Whisper("Ha Ha!  When I first laid eyes on you, I thought a strong breeze would knock you over.  Now, look at you!  The hero of the Gloomingdeep slave revolt!");
         if ($class eq 'Warrior' || $class eq 'Cleric' || $class eq 'Bard' || $class eq 'Shadowknight' || $class eq 'Paladin') {
