...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::Whisper("Did you know that when you die your soul can be recalled to your last bind spot? Well it can and that is exactly my purpose on this world. I can [bind] your soul so that after death you will be brought to life again in this spot.");
  }
  if ($text=~/bind/i) {
    quest::selfcast(35);
  }
  if ($text=~/siblings/i) {
    plugin::Whisper("Slavemaster Ruga has the rest of the dragon eggs on his person. He's in the Gloomingdeep jail trying to prevent the rest of the slaves from escaping. Be careful with Ruga though. That kobold's never without his bodyguards nearby!");
