...
sub EVENT_SAY {
  if (quest::istaskactivityactive(17,3)) {
    plugin::Whisper("Well done. The revolt's food supply is safe for the time being. Report back to the guards when you're rested.");
  }
  else {
    ...
    if ($text=~/hail/i) {
      ...
      plugin::Whisper("I'm glad you managed to escape the slave warrens in one piece. There are many [others] who were not as lucky. Here, take this burlap armor. It's not much, but it should keep you safe");
    ...
    if ($text=~/others/i) {
      ...
      plugin::Whisper("These few here. They were injured when the revolt broke out. I'm trying to help, but we have a short supply of [bandages], and more wounded keep coming. I fear many will not survive the hour.");
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 54234 => 1) || plugin::check_handin(\%itemcount, 54234 => 3)) { #Gloomingdeep Silk x 1 or 3
    plugin::Whisper("You have proven most resourceful $name.");
