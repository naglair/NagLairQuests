...
sub EVENT_SAY {
   if($text=~/Hail/i) {
      plugin::Whisper("My mate, Rahtiz, and I will hold this position.  We need you to explore the tunnels beyond.");
...
sub EVENT_ITEM {
   if (quest::istaskactivityactive(29,1)) {
      if (plugin::check_handin(\%itemcount, 54604 => 1)) {
         #Gloomingdeep Violet
         plugin::Whisper("Well done, my friend. You have probably saved many lives.");
