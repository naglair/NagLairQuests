...
if ($text=~/here to help/i) {
  ...
  if ($text=~/ready to go/i) {
    plugin::Whisper("Right, then. We'll be making our way to their secondary camp, which we believe follows the first into battle and may be the back up lines for the first camp. Make sure you have everyone with you, we'll be making our way to the camp in just a minute.");
  quest::start(1);
}
if ($text=~/ready to begin/i) {
   if($x == -1234) {
   plugin::Whisper("To arms, now! Rush into the heart of the beast and emerge victorious, as we have discussed! Charge!");
