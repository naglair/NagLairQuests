...
sub EVENT_SAY {
   if($text=~/Hail/i) {
      plugin::Whisper("Greetings. I would love to chat with you, but I just realized something.. I do not waste time with whelps.");
   }
   if($text=~/lens/i){
      plugin::Whisper("So you seek the Spare Lens. Yes. I have it. There are only two ways you can obtain it, [fight] the great Zenita or [play a game of chance].");
   }
   if($text=~/play a game of chance/i){
      plugin::Whisper("Great. It is rather simple. I have five cards and only one is King Naythox. Find it. In order to get one card all you need to do is buy me a bottle of Innoruuks Kiss of Death from the barkeep in Chops N Hops. One bottle for one card. Return the King Naythox card to me and you shall get the Spare Lens.");
   }
   if($test=~/fight/i){
      plugin::Whisper("Darn!! I was hoping not to hear that word, fight. Oh well.");
      quest::attack($name);
   }
}

sub EVENT_DEATH_COMPLETE {
   plugin::Whisper("You have no idea what a big mistake you made. Either my fellow rogues shall find you or the Freeport Mi.. Unnnghh.");
}
...
sub EVENT_ITEM {
   ...
   if(plugin::check_handin(\%itemcount,13121=>1)) {
      plugin::Whisper("Let see what card you pulled.");
      quest::summonitem(quest::ChooseRandom(22293,22294,22295,22296,22297,22298,22299));
   }
   elsif(plugin::check_handin(\%itemcount,22298>=1)) {
      plugin::Whisper("Why I will be.. You got it!! I thought I took it out of the deck. Very well. You win the Spare Lens fair and square. Here you are. Now get out of my sight.");
...
#she says she has 5 cards, but there are 7 cards in the database that are consecutive.. I assumed they are all hers
      ...
      plugin::Whisper("Bad luck must be one of your strong suits. You should have been a begger because you sure aren't a very good $class. You lose!");
   }
      ...
      plugin::Whisper("I see you have drawn the card that best represnts a $race such as yourself. You lose!");
   }
