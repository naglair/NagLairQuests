...
sub EVENT_SAY {
  if($class eq "Warrior") {
    if($text=~/Hail/i)   {
      ...
      plugin::Whisper("I said, can I help you or do you need help from [Wenden]?");
    }
    if($text=~/Wenden/i) {
    ...
    if($text=~/favor/i) {
      plugin::Whisper("An associate of mine is wandering somewhere in the Feerrott. His name is Oknoggin Stonesmacker. If you will Deliver this note to him, I would appreciate it.");
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 20674 => 1)) { # Tiny Lute
    plugin::Whisper("Thank you for delivering the note to my Associate. Take this book - It will tell you A little of the history of the old scabbard. If you would like to own the scabbard, seek out my younger brother Tenal and give him the book.");
    quest::ding();
    quest::exp(1000);
  ...
  if (plugin::check_handin(\%itemcount, 20684 => 1, 20679 => 1)) { # Tenal's Note to Kargek & Spiroc Wingblade
    plugin::Whisper("What is this? A note from my brother? Thank you for the gift as well.");
