...
sub EVENT_SAY {
  if(($ulevel >= 15) && (!defined($qglobals{Wayfarer}))) {
    if($text=~/hail/i) {
      plugin::Whisper("Come to find out about the Wayfarers Brotherhood, hm? I think I saw you around here long ago. Because you are familiar to me, I will trust you with some [information].");
    }
    if($text=~/information/i) {
      plugin::Whisper("The Wayfarers Brotherhood is pretty particular about who they do business with. You will need to prove yourself to them. You can start gaining their gratitude by helping them. The Wayfarers Brotherhood will ask you to answer some questions when you first meet with them. They tend to call all of their assignments 'adventures.' You'd do well to keep that in mind as they'll be more likely to give you some tasks. There is other information that makes them easier to [deal with] too.");
    }
    if($text=~/deal with/i) {
