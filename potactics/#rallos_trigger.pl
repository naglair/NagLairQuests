...
sub EVENT_SIGNAL {
        ...
        if ($signal == 214056) {        # Decorin Berik dead
                ...
                if ($Grunhork == 1) {   # Spawn Vallon and Tallon
                    ...
                    quest::settimer("VTZek", 2720); # 45 min to kill the Zek brothers
                    ...
                    quest::settimer("Decorin", 600); # 10min to kill other
                }
         }
        ...
        if ($signal == 214057) {        # Decorin Grunhork dead
                ...
                if ($Berik == 1) {   # Spawn Vallon and Tallon
                    quest::stoptimer("Decorin");
                    quest::settimer("VTZek", 2700); # 45 min to kill the Zek brothers
                    quest::spawn2(214111,188,0,996,580,133.1,192.5); #Vallon_Zek_
                    quest::spawn2(214108,187,0,996,-559,133.1,192.5); #Tallon_Zek
		...
		if ($signal == 214111) {	# Event Vallon Dead (#Vallon_Zek_)
			$event_vallon = 1;
			if ($event_tallon == 1) {	# check if Event Tallon is dead
				#Yep both dead, procedd to next phase
		...
		if ($signal == 214052) {	# Mini Rallos done
			...
			quest::spawn2(214113,0,0,550,24,-294.9,64.2); #Spawn Full Ralloz, he will depop himself in 2 hours
			
		}
		
		if ($signal == 214113) {	#RZtW dead, depop
			quest::updatespawntimer(214123,7200000);
...
sub EVENT_TIMER {
	...
	if ($timer eq "VTZek") {	# times up, despawn Zeks
		quest::stoptimer("VTZek");
        quest::signal(214111, 0);
        quest::signal(214108, 0);
		$event_vallon = 0;
		$event_tallon = 0;
	}
