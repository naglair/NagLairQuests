...
sub EVENT_SAY {
  if($text=~/Hail/i){
    plugin::Whisper("I do not have time to speak with thee. I have a problem on my hands. Feel free to speak with any of my trainers.");
  }
  if($text=~/problem/i){
    plugin::Whisper("My problems are of no concern of yours. but if you must know it deals with Nolusia's brother. I can tell you no more. Leave me be.");
  }
}

sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 18729 => 1)){
    plugin::Whisper("Welcome to the Craft Keepers! You have much to learn. and I'm sure you are anxious to get started. Here's your training robe. Go see Nolusia. she'll give you your first lesson.");
    quest::ding();
    quest::exp(100);
