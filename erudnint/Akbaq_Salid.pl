...
if ($text=~/hail/i) { 
       plugin::Whisper("Greetings, $name. I have recently returned to Erudin since hearing word of the titan spirits that inhabit the Stonebrunt Mountains. Should you be heading in that direction there are some [samples] that I am interested in acquiring."); 
    } 
    if ($text=~/samples/i) { 
      plugin::Whisper("The titan spirits of the Stonebrunt Mountains appear in several various animal forms seemingly with different areas of influence in the spirit world. I am interested in conducting some experiments on the hides of the giant leopard titan and the giant sabretooth titan. Once I have acquired those hides I will seek to conduct experiments on the [others]."); 
    } 
    if ($text=~/others/i) { 
      plugin::Whisper("Once I have finished my studies of the feline titans pelts I am interested in experimenting with the hides of the strongest of the stonebrunt titans. the gigantic gorilla and the gargantuan panda. The [ancient snake] that inhabits those mountains is another matter that you may be interested in as well."); 
    } 
    if ($text=~/ancient snake/i) { 
      plugin::Whisper("I believe I have uncovered a process with which to harness the mystical properties of the scales of the snake titan so that they can be fashioned into a powerful robe. If you bring me the ancient snake skin and some platinum thread I am sure there will be enough material to craft multiple robes and I shall reward you with one for your assistance."); 
    } 
  } 
  else { 
...
sub EVENT_ITEM { 
  if ($faction <= 5) { # Require indifferent or greater faction 
    if (plugin::check_handin(\%itemcount, 6960 => 1, 6946 => 1)) { # astral cloak of the titans 
      ...
      plugin::Whisper("These are the pelts of the most powerful of the known Titan spirits. You have done well, $name, and you will be rewarded for your services."); 
    } 
    elsif (plugin::check_handin(\%itemcount, 6958 => 1, 6964 => 1)) { # astral leggings of the titans 
      quest::summonitem(2575); 
      plugin::Whisper("Excellent! These pelts are in fine condition. I was 
...
in this realm.");      
      ...
      plugin::Whisper("Ah! I have been anxiously awaiting the oppurtunity to craft some powerful robes from these mystical scales."); 
    } 
    else { 
      plugin::return_items(\%itemcount); 
      plugin::Whisper("These are not the samples I need.");
      return 1;
    } 
  } 
  else { 
    plugin::Whisper("I do not know you well enough to entrust you with such an item, yet."); # Text here is made up 
    return 1;
  }    
