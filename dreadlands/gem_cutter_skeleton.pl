...
sub EVENT_SAY {
  ...
  if($text=~/green gem/i){
    plugin::Whisper("The greenest of gems I seek! Jade too brittle, emerald too hard, but [dufrenite]! Ahh, now there is a gem amongst gems.");
  }
  if($text=~/dufrenite/i){
    plugin::Whisper("Yes, dufrenite! Find me a dufrenite to replace the one you made me break. Now go!");
  }
}
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount,10073=>1)) {
    ...
    plugin::Whisper("Yes! Yes! This is the greenest of gems!");
    quest::summonitem("12946");
    plugin::Whisper("Worth more to you crushed, that is! Find a spectral pestle, and crush it to dust!");
    quest::exp("3000");
    quest::settimer("cutting",4);
  }
  else {
    plugin::return_items(\%itemcount);
    plugin::Whisper("Ha! Worthless to me, this is! I return it!");
  }
}
...
sub EVENT_TIMER {
  if ($timer eq "distracted") {
    quest::doanim(20);
    plugin::Whisper("Blast it! Distracted me you have! Now I need a new [green gem]!");
