...
sub EVENT_SAY {
  if($text=~/Hail/i){ 
    plugin::Whisper("Hello, me friend! I'm the resident healer o' Halas. Please inform me when ye've a need fer me talents to [bind wounds], [cure disease] or [cure poison]. Might I add, if ye're a young shaman o' Halas, ye can also [assist in gathering fungus]."); 
  }
  if($text=~/cure disease/i){
    plugin::Whisper("Two small quantities o' wooly fungus are needed before we can cure yer malady.");
  }
  if($text=~/bind wounds/i){ # This part has no reported outcome anywhere probably broken on live 
    plugin::Whisper("Before I bind yer wounds, ye must pay tribute to the Tribunal in the amount of ten gold coins.");
  }
  if($text=~/cure poison/i){
    plugin::Whisper("I'll be needing mammoth steaks to feed the unfortunate.  Then we'll drain the poison from yer veins.");
  }
  if($text=~/assist in gathering fungus/i){
    plugin::Whisper("As the wooly spiderlings get ready to molt, they'll carry wooly fungus. Oftentimes, one can find wooly fungus growing on their bellies. I use this in me healing practices. I'll reward ye if ye can fill this jar full o' the valuable fungus. Don't forget to combine them before ye return it to me. And have yerself some rations handy, or ye may find yerself snacking on this tasteless food source.");
...
sub EVENT_ITEM { 
  if (plugin::check_handin(\%itemcount, 13966 => 1)) { # Jar of Fungus turn in for random spell 
    ...
    plugin::Whisper("Aye! Ye've filled the jar. I'll see to it that Holana locks this away. Tis difficult to obtain and we can only spare the talents of our young shamans. Allow me to give ye a reward. Thank ye kindly fer yer service."); 
    ...
    quest::selfcast(203); 
    plugin::Whisper("Aye! I will draw the poison from your veins!");      
  } 
  elsif (plugin::check_handin(\%itemcount, 13967 => 2)) { # disease cure for wooly fungus totally bogus text 
    quest::selfcast(213); 
    plugin::Whisper("Ah! Now I can cure yer malady."); 
  } 
  else { 
