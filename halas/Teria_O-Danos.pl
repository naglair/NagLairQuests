sub EVENT_SAY {
  if($text=~/hail/i) {
    plugin::Whisper("Good day to ye! Ye look a wee bit hungrry. I sell many fine foods. Please have a look. Might I suggest the [lion roast]? It is a wee bit expensive, but well worth it.");
  }
  if($text=~/lion roast/i) {
    plugin::Whisper("The lion roast is quite good. Tis me specialty. Ah marinate the lion meat in mammoth fat fer two days before roasting it. Lion meat isn't common around here. I'll purchase what I can from strangers but I rely mostly on shipments from the Plains o' Karana. I'm presently searching fer someone to [pick up me lion meat shipment].");
  }
  if($text=~/lion meat shipment/i) {
    plugin::Whisper("It'd be grand o' ye to pick up me lion meat shipment. Ye must travel to the Plains o' Karana. Search fer a Northman fishing village. It should be Clan McMannus. Speak with the chieftain, Einhorst. Tell him ye're there to [pick up the lion meat shipment for Halas]. When ye return, I'll pay ye at least three gold. I look forward to yer return.");
  }
  if($text=~/lion delight/i) {
    plugin::Whisper("Oh, yes. A wonderful tasting dish. Unfortunately, I need more lion meat to make that dish. Might I instead suggest the [lion roast]?"); #Text made up
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 13961 => 1)) { #Lion Meat Shipment
    plugin::Whisper("Ye've returned!! How wonderful! The people o' Halas thank ye! It isn't often we get to indulge ourselves in the delicacies o' warmer climates. Here ye go, me friend. Ye've completed the delivery in good time. I hope ye deliver more often. Here, try some of me new creation.. [Lion Delight].");
    ...
    plugin::Whisper("I have no use for that $name. You may take it back.");
