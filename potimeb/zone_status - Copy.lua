...
function event_spawn(e)
	...
	if (qglobals[instance_id.."_potimeb_phase_bit"] == nil) then
		eq.set_global(instance_id.."_potimeb_phase_bit","0",7,"D3");
	end
	...
	eq.zone_emote(15,"Current phase: "..current_phase..".");
end
...
function event_signal(e)
	...
	elseif (e.signal == 2 and current_phase == "Phase1") then
		...
		if (event_counter == 5) then
			...
			eq.set_global(instance_id.."_potimeb_status","Phase2",7,"D3");
			current_phase = "Phase2";
			-- unlock all the phase 1 doors.
			UnlockPhaseOneDoors(entity_list);
	...
	elseif (e.signal == 5) then
		event_counter = event_counter + 1;
		eq.zone_emote(15,"Phase4 death count now "..event_counter.." of 4.");
		if (event_counter == 4) then
			-- update the qglobal in the zone gets reset.
			eq.set_global(instance_id.."_potimeb_status","Phase5",7,"D3");
			eq.set_global(instance_id.."_potimeb_phase_bit","0",7,"D3");
			current_phase = "Phase5";
	...
	elseif (e.signal == 6) then
		event_counter = event_counter + 1;
		eq.zone_emote(15,"Phase5 death count now "..event_counter.." of 4.");
		if (event_counter == 4) then
			-- disable the spawn condition
			eq.spawn_condition("potimeb",instance_id,1,0);
			-- update the qglobal in the zone gets reset.
			eq.set_global(instance_id.."_potimeb_status","Phase6",7,"D7");
			current_phase = "Phase6";
			...
			event_counter = 0;
			-- spawn Quarm
			eq.spawn2(223201,1,0,-401,-1106,32.5,92.8125);
			-- spawn #A_Servitor_of_Peace
			eq.spawn2(223101,0,0,244,-1106,-1.125,97.03125);
			-- spawn untargetable Zebuxoruk's Cage
			eq.spawn2(223228,0,0,-579,-1119,60.625,0);
	...
	-- signal 7 comes from Quarm, after hes dead
	elseif (e.signal == 7) then
		-- update the player lockout
		UpdateLockoutGlobal("potimeb_lockout","QuarmDead","H48");
		--eq.set_global(instance_id.."_potimeb_status","Phase6",7,"H47");
	-- signal 8 comes from Druzzil_Ro
	elseif (e.signal == 8) then
		eq.zone_emote(15,"THE ZONE IS CLOSING IN  2 MINUTES!!!!");
		eq.zone_emote(15,"THE ZONE IS CLOSING IN  2 MINUTES!!!!");
		eq.zone_emote(15,"THE ZONE IS CLOSING IN  2 MINUTES!!!!");
		eq.set_timer(instance_id .. "we_all_done", 120000);
	end
end
...
function ControlPhaseTwo()
	if (current_phase == "Phase2") then
		event_counter = event_counter + 1;
		eq.zone_emote(15,"Phase2 death count now "..event_counter.." of 45.");
	...
	elseif (current_phase == "Phase2.5") then
		event_counter = event_counter + 1;
		eq.zone_emote(15,"Phase2.5 death count now "..event_counter.." of 50.");
		if (event_counter == 50) then
			...
			eq.set_global(instance_id.."_potimeb_status","Phase3",7,"D7");
...
function ControlPhaseThree()
	...
	elseif (current_phase == "Phase3.1") then
		event_counter = event_counter + 1;
		eq.zone_emote(15,"Phase3.1 death count now "..event_counter.." of 10.");
	...
	elseif (current_phase == "Phase3.2") then
		event_counter = event_counter + 1;
		eq.zone_emote(15,"Phase3.2 death count now "..event_counter.." of 10.");
		if (event_counter == 10) then
			event_counter = 0;
	...
	elseif (current_phase == "Phase3.3") then
		eq.zone_emote(15,"Phase3.3 death count now "..event_counter.." of 10.");
		event_counter = event_counter + 1;
		if (event_counter == 10) then
			event_counter = 0;
	...
	elseif (current_phase == "Phase3.4") then
		event_counter = event_counter + 1;
		eq.zone_emote(15,"Phase3.4 death count now "..event_counter.." of 10.");
		if (event_counter == 10) then
			event_counter = 0;
	...
	elseif (current_phase == "Phase3.5") then
		event_counter = event_counter + 1;
		eq.zone_emote(15,"Phase3.5 death count now "..event_counter.." of 10.");
		if (event_counter == 10) then
			event_counter = 0;
	...
	elseif (current_phase == "Phase3.6") then
		event_counter = event_counter + 1;
		eq.zone_emote(15,"Phase3.6 death count now "..event_counter.." of 10.");
		if (event_counter == 10) then
			event_counter = 0;
	...
	elseif (current_phase == "Phase3.7") then
		event_counter = event_counter + 1;
		eq.zone_emote(15,"Phase3.7 death count now "..event_counter.." of 10.");
		if (event_counter == 10) then
			event_counter = 0;
	...
	elseif (current_phase == "Phase3.8") then
		event_counter = event_counter + 1;
		eq.zone_emote(15,"Phase3.8 death count now "..event_counter.." of 10.");
		if (event_counter == 10) then
			event_counter = 0;
	...
	elseif (current_phase == "Phase3.9") then
		event_counter = event_counter + 1;
		eq.zone_emote(15,"Phase3.9 death count now "..event_counter.." of 2.");
		if (event_counter == 2) then
			...
			-- update the status global
			eq.set_global(instance_id.."_potimeb_status","Phase4",7,"D7");
			-- add 4 hours (14400 seconds) to the fail timer
			--UpdateFailTimer(11700,14400);
			-- update the lockout for all players to 5 days 12 hours.
			--UpdateLockoutGlobal("potimeb_lockout","Phase3","H48");
			-- spawn phase 4
			SpawnPhaseFour();
		end
	end
...
function SpawnPhaseFour()
	eq.zone_emote(15," Spawning Phase 4!!");
	-- update qglobals
	qglobals = eq.get_qglobals();
	local phase_bit = tonumber(qglobals[instance_id.."_potimeb_phase_bit"]);
	if (phase_bit == 15) then
		eq.zone_emote(15,"AHHH NOOOOO zone Crash! Moving to Phase5!");
		-- this a rare crash handling instance. extremely hard to make happen
		eq.set_global(instance_id.."_potimeb_status","Phase5",7,"D7");
		eq.set_global(instance_id.."_potimeb_phase_bit","0",7,"D7");
		eq.repop_zone();
	else
		if (bit.band(phase_bit, 1) == 0) then
			eq.spawn2(223075,0,0,-310,307,365,95); -- Terris Thule
		else
			eq.zone_emote(15," Phase 4 Terris Thule killed previously, not spawning");
			event_counter = event_counter + 1;
		end
		if (bit.band(phase_bit, 2) == 0) then
			eq.spawn2(223076,0,0,-320,-316,358,32.5); -- Saryrn
		else
			eq.zone_emote(15," Phase 4 Saryrn killed previously, not spawning");
			event_counter = event_counter + 1;
		end
		if (bit.band(phase_bit, 4) == 0) then
			eq.spawn2(223077,0,0,405,-84,358,192); -- Tallon Zek
		else
			eq.zone_emote(15," Phase 4 Tallon Zek killed previously, not spawning");
			event_counter = event_counter + 1;
		end
		if (bit.band(phase_bit, 8) == 0) then
			eq.spawn2(223078,0,0,405,75,358,192); -- Vallon Zek
		else
			eq.zone_emote(15," Phase 4 Vallon Zek killed previously, not spawning");
			event_counter = event_counter + 1;
		end
	end
end
...
function SpawnPhaseFive()
	-- update qglobals
	eq.zone_emote(15,"Spawning Phase 5!");
	qglobals = eq.get_qglobals();
	local phase_bit = tonumber(qglobals[instance_id.."_potimeb_phase_bit"]);
	if (phase_bit == 15) then
		-- this a rare crash handling instance. extremely hard to make happen
		eq.zone_emote(15,"AHHH NOOOOO zone Crash! Moving to Phase6!");
		eq.set_global(instance_id.."_potimeb_status","Phase6",7,"D7");
		eq.set_global(instance_id.."_potimeb_phase_bit","0",7,"D7");
		eq.repop_zone();
	else
...
--]]
	...
	eq.set_timer("fail_timer",(current_remainder + seconds_to_add) * 2000); -- * 1000 to convert to milliseconds
	eq.zone_emote(14,"fail_timer set to " .. (current_remainder + seconds_to_add * 2) .. " seconds from now. This will update at the next phase.");
end

function event_timer(e)
	if (e.timer == "fail_timer") then
		eq.zone_emote(14,"TIME EXPIRED, YOU HAVE FAILED!");
		eq.zone_emote(14,"TIME EXPIRED, YOU HAVE FAILED!");
		eq.zone_emote(14,"TIME EXPIRED, YOU HAVE FAILED!");
		-- change the qglobal so zone status will not reset things if the zone reboots.
		eq.set_global(instance_id.."_potimeb_status","Failed",7,"H24");
	...
	elseif (e.timer == instance_id .. "_potimeb_p5army") then
		...
		if ( armiesdead == 1 ) then
			local phase_bit = tonumber(qglobals[instance_id.."_potimeb_phase_bit"]);
			eq.set_global(instance_id.."_potimeb_phase_bit",tostring(bit.bor(phase_bit,16)),7,"D7");
			eq.stop_timer(instance_id .. "_potimeb_p5army");
		end
	elseif (e.timer == instance_id .. "we_all_done") then
		eq.zone_emote(15,"CLOSING ZONE!!!!");
		local entity_list = eq.get_entity_list();
		...
		eq.depop_zone(false);
		eq.stop_timer(instance_id .. "we_all_done");
		-- destroy the instance. this will ensure everyone is removed from the instance
		eq.destroy_instance(instance_id);
	end
end
