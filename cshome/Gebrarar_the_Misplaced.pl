...
my $now = quest::saylink("now");
	...
	if($text=~/hail/i)
	{
		plugin::Whisper("Hello $uname. I can upgrade your [$charm] for you if you'd like. ");
	}


	if($text=~/charm/i)
	{
		$client->Message($whisper_color, "You can give me your charm and I'll upgrade it. It will require 50 unspent AA points plus your old charm. It Looks like you have $AAPTS AA points right now. If you want I can upgrade your charm  [$now]?");
	}
	...
	if($text=~/now/i)
	{
		...
		foreach $id (sort keys %defiant_turnins)
		{
			...
			$client->Message($whisper_color, "$item_link + 50 AA Points for $reward_link")
		}
	}
}
	...
	foreach $item_id (sort keys %defiant_turnins)
	{
		...
		if (plugin::check_handin(\%itemcount, $item_id => 1))
		{
			...
			quest::emote(" channels arcane energy into the charm to create a more powerful charm.");
			  $client->SetAAPoints($client->GetAAPoints() - 50);
