...
sub EVENT_HP {
if ($npc && $entity_list) {
my $tophatetarget = $mob->GetHateTop();
my $randomhate = $mob->GetHateRandom();


	if ($hpevent <= 75 && $hpevent >= 74) { 
	     quest::shout("$tophatetarget You are boring me!");
		 quest::setnexthpevent(50);
	...
	elsif ($hpevent <= 25 && $hpevent >= 24) { #second aggro wipe
			 quest::shout("$tophate! You are boring me!");
		   ...
		   if ($check_adds) {
			  ...
			  if ($check == 1) {
				  quest::shout("Foolish Mortals!  I am Too powerful for you!  Grant me your Lifeforce my minions!");
...
quest::shout("No?  How is this Possible?");
quest::depopall(999260);
}
...
if (defined $qglobals{$globalname}) {
	if ($qglobals{$globalname} == 1) {
		if ($qglobals{$globalname2} < 4) {
			my $counter = $qglobals{$globalname2};
			$counter++;
			quest::setglobal("$globalname2",$counter,5,'F');
			quest::emote("You Feel Your gaze pulled to the Lich's Phylactery.  This is what you are after.");
			}
		}
	}
}	
