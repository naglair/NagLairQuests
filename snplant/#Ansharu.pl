...
if (($text=~/hail/i) && ($event == 1))  {
   plugin::Whisper("Don't bother me until your work is done!");
   }
...
foreach $player (@player_list) {
 ...
 if (($text=~/hail/i) && ($event == 0)) {
    plugin::Whisper("Find the aged stonemites.  They are causing great trouble, come back and talk to me once you have solved the problem.");
