...
sub EVENT_ITEM {
	if (plugin::check_handin(\%itemcount, 132547 => 1, 132543 => 1, 132542 => 1)) { ## Head of VS, Surgical Scalpel and Jar
		if ($qglobals{"levelquest"} == 5) {
			...
			quest::say ("Oh thank you for the equipment, I'll know right what to do with it.  This head though, must be ".
						"properly disposed of.  I'll take care of that for you!");
		}
	}
		...
		if ($qglobals{"levelquest"} == 4) {
			...
			quest::emote ("gasps.");
			quest::say ("I'll dispose of that head for you, else I get in trouble for malpractice.");
		}
	}
