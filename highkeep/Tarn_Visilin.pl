...
sub EVENT_SAY {
  if($text=~/Hail/i){
    plugin::Whisper("Hello. I am Tarn Visilin.");
  }
}

sub EVENT_ITEM {
  if(($gold >= 50) && plugin::check_handin(\%itemcount, 10086 => 1)) { #Gold x 50, Glamour Stone
    plugin::Whisper("Here is your stone, fully enchanted."); #Made this text up
    quest::summonitem(10087); #Glowing Glamour Stone
  }
  elsif(plugin::check_handin(\%itemcount, 10019 => 1)) { #Bloodstone
    plugin::Whisper("I see that Sultin has sent you to me.  Very well, here are your gloves."); # Made this text up
    quest::summonitem(2355);  # Gleaming Gloves
  }
  elsif(plugin::check_handin(\%itemcount, 10790 => 1)) { #Coin of Tash
    plugin::Whisper("The Coin of Tash. Very good. I have laid a basic enhancement on it - to get it further enhanced you must take the coin to Mizn N'Mar in the Library of Neriak.");
