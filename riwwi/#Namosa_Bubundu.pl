  ...
  if($text=~/hail/i) {
    if ($faction <= 3) { # Require Kindly and greater faction
      if (defined $qglobals{bic} && $qglobals{bic} >= 1) {
        ...
        plugin::Whisper("This gazebo was once a place of contemplation, meditation, and reflection, but now it is a place of torture. These slavers shackle us here within feet of the pool of water and refuse us food or drink for days at a time, accompanied by horrible beatings. And now, I have had enough. I am through with this world and can only hope for peace in my passing. Before I pass on I wonder if you could do a [favor] for me?");
      }
    }
  }
  if($text=~/favor/i) {
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 52178 =>1 )) {
    quest::emote("gasps");
    plugin::Whisper("What?! Oh no, this isn't good at all. She isn't supposed to die. I am! Quick, go and see Councilman Tentric in Qinimi and let him know what's happening. He has always helped our family when we were in need. If you bring this token he will know I sent you!");
