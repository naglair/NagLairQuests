...
sub EVENT_SIGNAL {
    if($signal == 1) { 
  quest::emote("sneers");
  plugin::Whisper("Then let's have a demonstration, shall we? How to be slaughtered by a rabid animal in front of a captivated audience.");
  quest::spawn2(282100,0,0,37,540,3,189);  #Gibbering Hynid
    ...
    if($signal == 3) {
       quest::emote("gloats");
       plugin::Whisper("Your little stand is at an end, $name. Prepare to be slaughtered. I will enjoy this thoroughly.");
    ...
    if($signal == 7) {
    quest::emote("laughs");
    plugin::Whisper("Here come the Kyv. It's over now $name");
    ...
    if($signal == 8) {
    quest::emote("roars");
    plugin::Whisper("'GAH! Why won't you die?! $name, your time has come to an end. Send in the SLAYER!");
