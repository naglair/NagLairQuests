...
sub EVENT_SAY {
  ...
  if (defined $qglobals{bic_riw} && $qglobals{bic_riw} == 8 && !defined $qglobals{riwwiarena}) {
     plugin::Whisper("It's you! The champion everyone has spoken of! Have you come to defy the Muramites?");
     quest::setglobal("riwwiarena",1,5,"F");
    }
     }
  if ($text=~/hail/i) {
   if (defined $qglobals{riwwiarena} && $qglobals{riwwiarena} == 1) {
       plugin::Whisper("You are very strong. If you wish to challenge the Muramites, hand me the item Turlini gave you and I will gain their attention again.");
     }
     }
  if ($text=~/defy/i) {
   if (defined $qglobals{riwwiarena} && $qglobals{riwwiarena} >= 1) {
...
sub EVENT_SIGNAL {
  if($signal == 1) { 
    plugin::Whisper("Stand fast and prepare for a fight! They're sending a bloodthirsty beast into the arena to kill you as we speak. If you defeat it, place the head in this sack as proof and return it to Turlini. He is the leader of the resistance and may be able to assist you for helping us.");
