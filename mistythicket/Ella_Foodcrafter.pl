...
sub EVENT_WAYPOINT_ARRIVE{
	if(($wp eq 13) || ($wp eq 19)) {
		...
		if($random == 1) {
			plugin::Whisper("Hope things are going well here.");
		}
		if($random == 2) {
			plugin::Whisper("Air's nice as always out here.");
		}
	}
}
