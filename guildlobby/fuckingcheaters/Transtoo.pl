...
if ($text =~/Hail/i)
{
plugin::Whisper ("Well hello there $name. Would you like to go to [$bazaar],/t elric [$qeynos], [$efp], [$wfp], [$halas], [$erudin],[$surefall], [$rivervale], [$akanon], [$felwithe],[$kaladim],[$kelethin],[$oggok], [$grobb], [$neriak], [$lavastorm], or [$nektulos]?  ");
}
if ($text =~/bazaar/i)
{
plugin::Whisper ("$name , stand close to me while I cast..");
quest::emote ("moves his hands as if chanting...");
quest::movepc(-71,-250,-33,0);
}
if ($text =~/kaladim/i)
{
plugin::Whisper ("$name , stand close to me while I cast..");
