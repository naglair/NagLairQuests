...
my $dreadlands = quest::saylink ("The Dreadlands", 1);
 ...
 if ($text =~/hail/i && $status < 50)
 {
plugin::Whisper ("What is this? I cant tell you my [$secrets]");
 } 
 
if($text=~/hail/i && $status > 90)
        {
               $client->Message(17,"You must be a GM. Would you like to hear more about the new content sabrina is working on? [$yes]? Or do you want updates on the [$progress] she has made?");
        }
