...
if ($text =~/Hail/i)
 {
plugin::Whisper ("Good day to you, $name. Welcome to The Lair! You are about to embark upon a unique [$adventure] unlike any other. Expansions here are unlocked by Epic quests involving all of the bosses of your current expansion! We are in beta-testing, so as a reward for testing, there is a Server Charm free on the newbie vendor! Would you like to know about [$grouping], [$skills], [$spells], [$travel] or [$server_vision]? "); 
...
if ($text =~/adventure/i)
 {
plugin::Whisper ("On this sever, you unlock each expansion on a Character level basis. So the quest to unlock Kunark requires you to have the quest items from Vox, Nagafen, Phinny, Cazic and Innothule. Once you have unlocked Kunark, you will have to do the quest on each character / alt who wants the flag, Each expansion is unlocked by each person for himself.");
 }
if ($text =~/grouping/i)
 {
