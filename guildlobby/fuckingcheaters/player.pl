...
sub EVENT_CLICKDOOR {
  if($doorid == 2 || $doorid == 4) {
    if (!defined $qglobals{donator}) {
	$client->Message(315,"Donator Door");
	quest::setglobal("donator", 0, 5, "F");
		}
		  if (defined $qglobals{donator}) {
				if ($qglobals{donator} == 0 && $status < 8) {
			$client->Message(315,"I'm sorry, you aren't a donator. You may not pass through this door.");
			}
			if ($qglobals{donator} == 1) {
			$client->Message(315,"Welcome, Donator, $name!");
			quest::movepc(345, -1.2, -126, 2.2);
			}
			if ($status >= 8) {
			$client->Message(315,"Welcome, Donator, $name!");
			quest::movepc(345, -1.2, -126, 2.2);
	  ...
	  if($doorid == 3 || $doorid == 258 && $qglobals{flagging} >=4) {
	  $client->Message(315,"Welcome to the planes.");
	  quest::movepc(202, -285, -148, -159);
	  }
	  	  if($doorid == 3 || $doorid == 258 && $qglobals{flagging} <4) {
	  $client->Message(315,"You are not flagged for Poknowledge yet - speak to the keepers.");
	  }
