...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    ...
    if ($event == 1) { #Event ready, not started
      plugin::Whisper("Have you come to hear my riddle? Tell me when you are [ready]. When you know the answer give me the correct item.");
    }
    if ($event == 2) { #Event in progress
      plugin::Whisper("I am waiting for the answer...");
    }
    if (($event == 3) || ($qglobals{tipt_progress} == 3)) { #Event success
      plugin::Whisper("The door is open, hurry through!");
  ...
  if (($text=~/ready/i) && ($event == 1)) {
    ...
    if ($question == 1) {
      plugin::Whisper("Who makes has no need of it, who buys has no use of it, who uses it can neither see nor feel it.");
    }
    if ($question == 2) {
      plugin::Whisper("What can be held in the left hand, but not in the right hand.");
    }
    if ($question == 3) {
      plugin::Whisper("I drive men mad for love of me, Easily beaten, Never free.");
    }
    if ($question == 4) {
      plugin::Whisper("I'm always hungry, I must always be fed, The finger I lick will soon turn red.");
    }
    if ($question == 5) {
      plugin::Whisper("My life is measured in hours. I serve by being devoured. Thin, I am quick, Fat I am slow. Wind is my foe.");
    }
    if ($question == 6) {
      plugin::Whisper("My mouth bears whiskers but no teeth. I have scales but I do not weigh. You can bait me with delicious food but please, no drink today.");
    }
    if ($question == 7) {
      plugin::Whisper("Lovely and round, I shine with pale light, grown in darkness, a lady's delight.");
    }
    if ($question == 8) {
      plugin::Whisper("I'm offered to the loved, and also to the dead. I come in many varied hues, most notably red. My pricks are known to pierce the skin, often resulting in wounds that bled.");
    }
    if ($question == 9) {
      plugin::Whisper("When I am filled I can point the way, When I am empty Nothing moves me, I have two skins One without and one within.");
    }
    if ($question == 10) {
      plugin::Whisper("I help to mature your spirits. When moistened I fulfill my purpose. Should I dry out, my task will fail and my quarry may be worthless.");
    }
    if ($question == 11) {
      plugin::Whisper("I build up castles. I tear down mountains. I make some men blind, I help others to see. What am I?");
    }
    if ($question == 12) {
      plugin::Whisper("When young, I am sweet in the sun. When middle-aged, I make you happy. When old, I am valued more than ever.");
    }
    if ($question == 13) {
      plugin::Whisper("I scribble forms of the finest letter, and repel elements of the harshest weather. I am an arrow-aimer and a dust-breaker.");
    }
    if ($question == 14) {
      plugin::Whisper("Searing 'cross the pitch-black skies, I scream in celebration, yet moments later, my outburst through, I am naught but imagination.");
    }
    if ($question == 15) {
      plugin::Whisper("It floats on water, light as can be. A thousand men can't lift it free.");
    }
  }
}
...
sub EVENT_ITEM {
  if (($question == 1) && plugin::check_handin(\%itemcount,  54068 => 1)) { #Coffin
    ...
    plugin::Whisper("That is incorrect.");
  ...
  if ($event == 3) {
    plugin::Whisper("Well done! You may pass!");
    quest::forcedooropen(4);
