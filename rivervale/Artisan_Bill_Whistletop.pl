...
sub EVENT_SAY {
	if($race eq "Halfling")
	{
		...
		if($text=~/beholder's maze/i)
		{
			...
			{
				plugin::Whisper("I'm sorry, I don't think you are ready for that task yet. Please come back once you've gotten a bit more experience.");
			} 
		} # if($text=~/beholder's maze/i)		
		if($text=~/the hole/i)
		{
	...
	{
		plugin::Whisper("Hello $name!  Another fine day here in Rivervale!");
	}
}
...
sub EVENT_ITEM {
	if($race eq "Halfling")
	{
		...
		{ 
			plugin::Whisper("Oh, hey there, $name. I'm still working on my history book. In fact, I just wrote down the part	where we first met. Here let me read it for you!");
			quest::emote("clears his throat.");
			plugin::Whisper("Ahem, so I was writing my history book when Dryssa comes up to me and says 'Hail.' And so I reply...");
			...
			plugin::Whisper("Oh, yeah, the armor pattern book. Sorry, I forgot about that. Here you go!");
