sub EVENT_SAY {
  if($text=~/hail/i) {
    plugin::Whisper("Greetings and salutations, $name! My name is Reebo Leafsway, loyal Druid of [Karana]. I am in charge of helping young druids who wish to get started down the [trail to Karana's wisdom]. I also do my share of tending the [crops].");
  }
  if($text=~/shakey/i) {
    plugin::Whisper("Shakey is our protector and friend. He is just getting old. Losing his [stuffing]. His [head] is getting very cracked and dry as well.");
  }
  if($text=~/stuffing/i) {
    plugin::Whisper("The hay he requires is difficult to obtain. It must have been harvested from the Plains of Karana, then cursed by a high priest of the Faceless, [Cazic-Thule]. Finally, you must cast the hay and a flask of blessed Oil of Life into an armorer's forge. Only then will the hay be ready. We Stormreapers will reward you greatly if you could accomplish this task. Remember, you must give the scarecrow stuffing to Shakey only after it has been properly prepared.");
  }
  if($text=~/who is karana/i){
    plugin::Whisper("Karana is known as the Rainkeeper. It is through His will that our [crops] and our children grow big and healthy. He watches over us and protects us. calling down the fury of a tempest on those who wish harm upon His followers.");
  }
  if($text=~/crops/i) {
    plugin::Whisper("The crops we grow here are mostly carrots, lettuce and squash. We also are the only place on all of Norrath where the soil can support the mystical Jumjum Stalk.");
  }
  if($text=~/head/i) {
    plugin::Whisper("The head of a scarecrow is difficult to replace. We do not know what is needed and all of our attempts have failed. Perhaps you could [research] this for us and bring back a new head for our old friend Shakey?");
  }
  if($text=~/research/i) {
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 13971 => 1)) {
    ...
    plugin::Whisper("Oh good! I see you have taugh that nasty Nillipuss a thing or two! Good. But it seems to me that he has stolen more jumjum than this. Perhaps he needs another lesson?");
    plugin::return_items(\%itemcount);
  }
  elsif(plugin::check_handin(\%itemcount, 13974 => 4)) {
    plugin::Whisper("Excellent!!  You must have taught ol' Nillipuss a great deal!  But he never seems to learn..  Oh well.  The Stormreapers and all of Rivervale owe you a debt of gratitude.  Please accept this token of our appreciation.");
...
sub EVENT_SIGNAL {
 if($signal == 0) {
  plugin::Whisper("Old [Shakey] hasn't been feeling like himself lately, I'm afraid.");
 }
}
