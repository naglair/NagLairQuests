  ...
  if($ulevel > 54) { #Must be level 55 or higher
    if ($cash >= 500000) { #Money for class emblem
      ...
      if ($class eq "Warrior") {
    	...
    	plugin::Whisper("What ar ye?");
        quest::givecash($copper,$silver,$gold,$platinum); #Return money
        return 1;
      }
      plugin::Whisper("Wonderful! This coin will go towards me fines with the Myrist library. They charge quite a bit fer overdue volumes! 'ere be yer emblem an' a kit in which ye may craft planar armor. The kit only 'as enough magical energy t'craft one piece before the energies expire, be sure ye understand. May the armor ye make with it provide ye with much protection.");
    }
    else {
      plugin::Whisper("Tis not enuff!");
      if ($cash > 0) {
        quest::givecash($copper,$silver,$gold,$platinum); #Return money
      }
