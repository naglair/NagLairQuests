...
sub EVENT_SAY
{
		...
		if ((defined $qglobals{pop_pot_saryrn_final} && $qglobals{pop_pot_saryrn_final} == 1)) {
			if ((!defined $qglobals{pop_pot_tt_hedge_bypass}) || ((defined $qglobals{pop_pot_tt_hedge_bypass}) && ($qglobals{pop_pot_tt_hedge_bypass} == 1))) {
				if (!defined $qglobals{pop_ponb_terris} || !defined $qglobals{pop_ponb_poxbourne} || !defined $qglobals{pop_pon_hedge_jezith} || !defined $qglobals{pop_pon_construct}) {
					plugin::Whisper("Excellent work $name, I hear great things of you from Odaen. I have more work for you if you wish to help. Do you [believe] that we can help another suffering soul?");
				}
			}
			
			if (defined $qglobals{pop_pot_tt_hedge_bypass} && $qglobals{pop_pot_tt_hedge_bypass} == 2) {
				plugin::Whisper("$name we have done it! I heard all about your adventures into the Plane of Nightmare from Kelletia. Do you believe it? I always believed that you believed that we could do it and look we have done it! Our work here is done friend.");
			...
			if (defined $qglobals{pop_pot_tt_hedge_bypass} && $qglobals{pop_pot_tt_hedge_bypass} == 3) {
				plugin::Whisper("Excellent work $name, but there is no time to waste. [Mylik] needs our help.");
			}
		}
		else {
			plugin::Whisper("Greetings fellow adventurer, I can see that you're interested in curing the [illness] that seeps through Norrath.");
		}
	}
	...
	if ($text=~/I believe/i) {#Despite the previous text in brackets, Gordish-Dev, Eq Designer says "The trigger for that should be "I believe". "
		plugin::Whisper("We will see how much you believe in my powers and your abilities. This man Thelin over here is lost in a world of Nightmares. Thelin was once a great man who opened a portal to the Plane of Nightmare is now a weak man trapped inside his own [nightmares].");
	}
	
	if ($text=~/nightmares/i) {
		plugin::Whisper("Thelin had used all of his energy to open the portal to the Plane of Nightmare. He went home that day and fell asleep clutching a [jeweled dagger] that was once given to him. While sleeping, he had this terrible nightmare and he started to toss and turn from the vision that was sent to him. He tossed and turned so much that he landed on the dagger and critically injured himself. The elders found him in time and were able to save him but he has slipped into a coma and is in an endless world of nightmares.");
	}
	
	if ($text=~/jeweled dagger/i) {
		plugin::Whisper("Ah yes the jeweled dagger. It was precious to him and he always kept it by his side. I believe that retrieving the dagger is the key to saving him. I have a scout inside the Plane of Nightmare trying to recover it for me. Terris Thule has broken it into many pieces and scattered it inside a maze so Thelin couldn't use it to escape his nightmares. Our scout has been gone for awhile now and I'm getting worried. Can you [find her] and assist her in bringing the jeweled dagger back to me?");
	}
	
	if ($text=~/find her/i) {
		plugin::Whisper("Excellent her name is Kelletia. She is one of our best scouts.");
	}
	
	if ($text=~/illness/i) {
		plugin::Whisper("$name, look at these people. Their minds are being tormented, they are diseased and their souls are decaying more and more each day. You only see four in front of you but tomorrow it might be eight and the day after it could be hundreds. Who knows, it might even be [you] on your sick bed.");
	}
	
	if ($text=~/me/i) {
		plugin::Whisper("Yes $name even you. You have to believe that I can help these people. You need to believe that I can help you. You need to believe that you can help me. I believe you can help me. Will you [help me] save these people?");
	}
	
	if ($text=~/help you/i) {
		plugin::Whisper("Then let's get started! This man Tylis suffers from mental torment. Look at him ______ he is in pain. His illness is no ordinary illness. Someone is controlling his mind and we need to stop the source ofthis torment. You can find the one who controls his mind inside the Plane of Torment. We want to learn more about how his brain works. Destroy the source and bring me back his brain so we can set those under his spell free.");
	}
	
	if ($text=~/mylik/i) {
		plugin::Whisper("Excellent! Let me tell you about Mylik. Much like Thelin, Mylik was a strong brave man too. Mylik stood in these lands and focused his energy and mana and opened the portal to the Plane of Disease. Bertoxxulous set a curse on those who dared open a portal to his plane and he defiled Mylik's soul with a potent toxin that has left him lying there slowly decaying away. Do you [understand] the amount of death that Bertoxxulous can spread over Norrath?");
	}
	
	if ($text=~/understand/i) {
		plugin::Whisper("Ok then pay attention $name. I have already sent in an old friend of mine to handle what needs to be done inside the Plane of Disease. What he doesn't know is that Bertoxxulous is aware that he has breached the portal and is planning on sending four guardians to eliminate him. Enter the Crypt of Decay and destroy the guardians. Bring me back proof of their death and I'll give you further instructions. Hurry $name, this task is vital to saving Milyk.");
	}
}
...
sub EVENT_ITEM
{
	...
	if(plugin::check_handin(\%itemcount, 51614 => 1)) {#Brains of the Tormentor
		...
		if ((defined $qglobals{pop_pot_saryrn_final} && $qglobals{pop_pot_saryrn_final} == 1)) {
			plugin::Whisper("A fine step towards curing this illness, $name.  I thank you.");#Text made up.
	...
	#Grummus bypass, need to have Saryn, Keeper of Sorrows, TT and Hedge done.
		...
		if (((defined $qglobals{pop_pot_saryrn_final} && $qglobals{pop_pot_saryrn_final} == 1)) &&
		   ...
		   ((defined $qglobals{pop_pon_construct} && $qglobals{pop_pon_construct} == 1))) {
				plugin::Whisper("Go forth to the Crypt of Decay, $name.");#Text made up.
				#Grummus flags
