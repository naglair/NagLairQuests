...
sub EVENT_SAY
{
	if(defined $qglobals{pop_poj_mavuin}) {
		if($text=~/Hail/i) {
			...
			plugin::Whisper("Very well. When you are ready, you may begin the trial of torture. Only when a wraith of agony dies will the prisoners feel any relief. Take care to find and kill it quickly, lest their torment overcome them. We shall judge the mark of your success.");
		}

		elsif($text=~/begin the trial of torture/i) {
			if (!defined $torture) {
				plugin::Whisper("Then begin.");
				...
				if (($torture > 0) && ($torture < 6)) {
					...
					plugin::Whisper("I'm sorry, the Trial of Torture is currently unavilable to you.");
				}
			}
		}
