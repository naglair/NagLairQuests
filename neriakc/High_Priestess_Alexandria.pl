sub EVENT_SAY {
  ...
  if(($class eq "Cleric") && ($race eq "Dark Elf")) {
    ...
    plugin::Whisper("I'm sorry but I will only deal with Clerics that are willing to prove their loyalty to Innoruuk.");
  }
}
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 22617 => 1, 22618 => 1, 22619 => 1)) {
    ...
    plugin::Whisper("$name, I don't need this."); #text made up
