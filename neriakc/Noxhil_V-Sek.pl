...
sub EVENT_SAY { 
	if($text=~/Hail/i){
		plugin::Whisper("Greetings, $name!  We of the Dead are proud to have you among us.  Your lineage is well known.  Still, you must learn to claw your way to the upper echelon.  There are many [menial tasks] to be performed before you can truly be inducted as a member.");
	}
	if($text=~/menial tasks/i){
		plugin::Whisper("You cannot avoid the toil of peons. We have all spent our time in the lower ranks. There are duties such as [collecting beetle eyes].");
	}
	if($text=~/collecting beetle eyes/i){
		plugin::Whisper("Take this chest.  It has been fitted with a mold designed to hold ten beetle eyes.  We require them for further experiments.  Do not return until you fill the chest.");
		quest::summonitem(17930);#Empty Box
	}
	if($text=~/other components/i){
		plugin::Whisper("The other components that are required are two Bull Elephant tusks.  Mammoth tusks would do as well.");#text made up
	}
}
sub EVENT_ITEM {
	if(plugin::check_handin(\%itemcount, 13389 => 1)){#Box of Beetle Eyes
		plugin::Whisper("Fantastic work, my child! We shall store these for further experiments. Take this as extra payment for a fine job. You have done so well I believe you can assist in obtaining two [other components].");
		...
		plugin::Whisper("These will do.  Thank you, child.");#text made up
		quest::faction(322, 5); #The Dead
		quest::faction(268, 5); #Queen Cristanos Thex
		quest::faction(177,-10); #King Naythox Thex
		quest::faction(170,-10); #Keepers of Art
