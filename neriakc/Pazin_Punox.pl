...
sub EVENT_SAY { 
	if($text=~/Hail/i){
		plugin::Whisper("Welcome to the Hall of the Ebon Mask. You had best be a rogue or you have no business here. We have nothing to say to outsiders.");
	}
	if ($text=~/dark assassin/i) {
		plugin::Whisper("Aye we were visited by that dark assassin, he left some sketch in my safekeeping, I would gladly give you a copy if you would take care of a problem that has been vexing me.");
	}
	if ($text=~/vexing you/i) {
		plugin::Whisper("An old student of mine who goes by the name Lon has disgraced this House; his betrayal to Innoruuk and Neriak is unforgivable.");
