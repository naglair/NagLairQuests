...
sub EVENT_SAY {
  ...
  if ($faction <= 2 && $makepoison >= 278) {
    if($text=~/Hail/i) {
      plugin::Whisper("Master Hanns is going to kill me! I let that dark assassin look at his prized poison collection and he lifted it all when I wasn't looking! He even snatched his Grandmaster's Vial! Master Hanns was so furious he grabbed one of his old blades from his collection and jabbed it at the wall so hard it shattered! I really need some help replacing all that was [ruined] or I have a feeling Master Hanns is going to send me on a suicide mission!");
    }
    if($text=~/ruined/i) {
      plugin::Whisper("Fantastic! I need to replace all those old poisons, the vial and that old scareling blade. I think the bottles had the following written on them Bane, Burn, Essence, Sloth, Rage, Swiftness, Disorientation, and Dimethyl. Please hurry!");
      quest::summonitem(17148);
    }
  }
  else {
    plugin::Whisper("Go away! We don't have time for the likes of you.");
  }
} 
...
sub EVENT_ITEM {
  ...
  if ($faction <= 2 && $makepoison >= 278) {
    if (plugin::check_handin(\%itemcount, 24072 => 1)) {
      ...
      plugin::Whisper("I do not see everything that I asked for...");
