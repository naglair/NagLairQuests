...
sub EVENT_SAY {
  if($text=~/hail/i) {
    plugin::Whisper("Greetings, $name. Welcome to the fishing village of Clan McMannus. We sell what we can here. Have a look around but, please do not buy too much. We must save most of our stock for delivery to our home.");
  }
  if($text=~/lion meat shipment/i) {
    plugin::Whisper("Oh, yes, the lion meat shipment. Aye, it's here somewhere. Just a moment. Ah! Here you go. Apologize for the delay in this.");
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 18831 => 1)) { #Barbarian Head
    plugin::Whisper("What? Ok well, hmm let me see...... Oh there it is. Here is the shipment as ordered!");
    quest::exp(1600);
    quest::ding();
