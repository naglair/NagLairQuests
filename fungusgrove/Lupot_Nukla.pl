...
if(plugin::check_hasitem($client, 57620) && defined $qglobals{shaman_epic} && $qglobals{shaman_epic} == 1){
 if($text=~/hail/i) {
    plugin::Whisper("Surprising. Didn't think you'd be up to the task and be able to muster the knowledge to figure it out. If you have both with you, hurry along. The spirit is in great need of the potion.");
  }
 }
}
