...
sub Event_Say {
	...
	plugin::Whisper ("All subvertents will be converted! Meldrath is your new lord and master! All who resist shall be punished!"); 
	}
if ($status >= 80 && $text=~/who the hell is meldrath/i) {
	plugin::Whisper ("Blasphemer! Meldrath is the king of all gnomes! The god of tinkering.... The guy I'm here representing? Wow, you guys really dont get out much do you? Surrender or die shorties!");
	}
if ($status >= 80 && $text=~/We shall never surrender/i) {
        plugin::Whisper ("That does it! Your in trouble now! Im telling Meldrath!"); 
...
if ($status >= 80 && $text=~/your forces are deafeated/i) {
	plugin::Whisper ("Oh really? Look out unto the battlefield!");
...
if ($status >= 80 && $text=~/you shall not break our defense/i) {
	plugin::Whisper ("Now you have done it! Surely the pinicale of tinkering robotics! An Austrian action filmstar shall make you grovel!");
