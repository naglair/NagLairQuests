...
sub EVENT_SAY {
  if($text=~/hail/i){
    plugin::Whisper("Hail and well met $name! I give the second half of the test of songs. If you are ready, choose between the tests of brass, wind, and harmony.");
  }
  if($text=~/brass/i){ #Bard Test of Brass
    plugin::Whisper("Then bring back to me an Efreeti War Horn, a Saffron Spiroc Feather, Adamintium Bands, and a Glowing Diamond. Then I will give you the horn of disaster.");
  }
  if($text=~/wind/i){ #Bard Test of Wind
    plugin::Whisper("Bring me an imp statuette, a dull stone, and an amulet of woven hair. Then I will give you the fae amulet.");
  }
  if($text=~/harmony/){ #Bard Test of Harmony
    plugin::Whisper("Working with the environment to enthrall those that listen to you is of the utmost importance to us. Go out and retrieve an efreeti war spear, some manna nectar, a nebulous emerald, and a nebulous diamond. Return these items to me and receive the harmonic spear as your reward.");
  }
}
