sub EVENT_SAY { 
  if($text=~/hail/i) {
    plugin::Whisper("Greetings, $name. Do you believe that you are a [great Wizard]?");
  }
  if($text=~/great wizard/i) {
    plugin::Whisper("I will not take your word for it! You must prove your greatness to my apprentices. Do you wish to be tested by Neasin or by Abec?");
  }
