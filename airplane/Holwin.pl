sub EVENT_SAY {
     if($text=~/Hail/i){
       plugin::Whisper("Peace to you, $name.  I see that you have come far along the path of tranquility and enlightenment.  Do you wish to test yourself further, and perhaps complete the path you started on so long ago?");
       }
      if(($text=~/tranquility and enlightenment/i) && ($class eq "Monk")){
        plugin::Whisper("Choose you path. Wu or Ton Po");
        }
