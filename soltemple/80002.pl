...
sub EVENT_SAY {
  ...
  if ($text=~/plasmatic priest/i) {
    plugin::Whisper("You speak of things that should not be spoken! It would be in your best interest to leave this temple at once and return to lands less consumed by flame!");
  }
}
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 28051 => 1)) { #Coral Statue of Tarew
    ...
    plugin::Whisper("The Triumvirate thinks that destroying my spirit of flame will stop me?!! I will kill you all now, the goblins will continue to war, and when my spirit is restored I will ensure that Ixiblat Fer consumes all of Norrath in his flames!!");
