...
sub EVENT_SAY {
  ...
  if ($text=~/scale/i){
    plugin::Whisper("Tektite and peridot shall get you what you desire.");
  }
  if ($text=~/xyfyl/i){
    plugin::Whisper("So you can pronounce my name backwards..  Is that your lot in life?  Pronouncing names backwards?  Be gone with your silliness!  We need no court fool.");
  }
}
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 10028 => 1, 12831 => 1)){
    ...
    plugin::Whisper("This is of no use to me.");
