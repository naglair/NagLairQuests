...
sub EVENT_SAY {
  if($text=~/hail/i) {
    if(defined($qglobals{NecroPre}) && ($qglobals{NecroPre} == 1)) {
      plugin::Whisper("So you have come to stop me?");
      quest::emote("throws a magnificent, yet evil looking staff into the water.");
      plugin::Whisper("The precious staff you seek is now lost in the lake for all eternity!");
      quest::attack($name);
    }
    else {
      plugin::Whisper("I am busy."); #Real text still needed
    }
  }
}
