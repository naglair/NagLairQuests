...
sub EVENT_SAY {
  ...
  if($text=~/additional information/i) {
    plugin::Whisper("There is a magic in Kuua that is blessed with the power of Discord itself. We have found we can use that magic to help those who help us. If you return three [discordant crystal shards] and a [magic parchment], we will imbue it with our power so you may travel there alone. Obviously, you must be a master of teleportation yourself -- a wizard or druid, is what I mean.");
  }
  if($text=~/shards/i) {
    plugin::Whisper("The crystal shards are very unique and bear a magic that brims with the power of Discord. We found we can use them to create spells on a specific type of discordant parchment that allows wizards and druids to pass through to the other realm.");
  }
  if($text=~/magic parchment/i) {
    plugin::Whisper("We are not certain what the parchments were meant for, but they burn hot and cold and are imbued with a magic that protects it from destroying itself. We need one of these parchments to make a travel spell to Kuua in the Realm of Discord.");
  }
  if($text=~/tome/i) {
    plugin::Whisper("The Tome of Order and Discord was penned by the seventh member of the Tribunal and has become the key to a life of Discord, in spite of the author's pitiful warnings. Do you not have one, child of Order? Would you [like to read] it?");
  }
  if($text=~/read/i) {
    plugin::Whisper("Very well. Here you go. Simply return it to me to be released from the chains of Order.");
    quest::summonitem(18700); #Tome of Order and Discord
  }
  if($text=~/wish/i) {
    plugin::Whisper("Prepare yourself to cross into the depths of Discord!");
    #quest::setglobal("OOW_PoD_Origin", $zoneid, 5, "F");
    #quest::movepc(302, -1485.52, -1263.29, 222.0);
quest::shout2("$name was trying to zone into Omen of War zone. For Shame!");
  }
}

sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 18700 => 1)) { #Tome of Order and Discord
    plugin::Whisper("I see you wish to join us in Discord! Welcome! By turning your back on the protection of Order you are now open to many more opportunities for glory and power. Remember that you can now be harmed by those who have also heard the call of Discord.");
    quest::pvp("on");
    quest::ding();
  }
    ...
    plugin::Whisper("You will now be able to teleport into the realm of discord, true power is at your fingertips!");
    if($class eq "Wizard") {
