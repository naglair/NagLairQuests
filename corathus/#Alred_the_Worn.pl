...
sub EVENT_SAY
{
	if($client->GetLevel() >= 55)
	{
		if($text=~ /Hail/i)
		{
			if(quest::istaskactive(145))
			{
				...
				{
					plugin::Whisper("You've briefly granted us some rest from the onslaught of the minotaurs, for that I thank you.");
			...
			else
			{
				...
				{
					plugin::Whisper("My weary bones ache, we need a [reprieve] from these assaults.");
				}
			}
		}
		elsif($text=~ /reprieve/i)
		{
			...
			{
				plugin::Whisper("If you wish to earn my thanks then go out and [slay the minotaurs]. If you return to me alive I will reward you.");
			}
		}
	...
	else
	{
		if($text=~ /Hail/i)
		{
			plugin::Whisper("My weary bones ache, we need a reprieve from these assaults, but I am afraid you are far too inexperienced to help right now.");
		}
	}
