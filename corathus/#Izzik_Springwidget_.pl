sub EVENT_SAY
{
	if($text=~ /Hail/i)
	{
		plugin::Whisper("Hail, friend, Izzik's my name and [tinkering] is my game.");
	}
	elsif($text=~ /Tinkering/i)
	{
		plugin::Whisper("Oh yes I love to tinker, so did Wilfred but I'm afraid he's past that now. It's sort of [unfortunate]...");
	}
	elsif($text=~ /Unfortunate/i)
	{
		plugin::Whisper("Well you see I love to tinker like no one else around, but supplies around here are short. Most of the robots we just destroyed were not well maintained and salvaging them didn't get me much. There are some clockworks to the south that if you were to [assist] me in salvaging I would reward you for.");
	}
	elsif($text=~ /Assist/i)
	{
		my @task_array;
