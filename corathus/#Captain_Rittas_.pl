sub EVENT_SAY
{
	...
	if($boss_check)
	{
		...
		elsif($text=~ /Assignment/i)
		{
			...
			{
				plugin::Whisper("These assignments are all military in nature, please return to me if you succeed in finishing one.");
				quest::taskselector(@task_array);
			}
			else
			{
				plugin::Whisper("I have nothing to offer you currently.");
			}
		}
	}
}