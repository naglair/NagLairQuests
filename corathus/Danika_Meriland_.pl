sub EVENT_SAY
{
	if($text=~ /Hail/i)
	{
		plugin::Whisper("'ello I am Danika, I hail from the MIGHTY town of Rivervale! I am a fierce fighter by spirit but a [trader] by reputation, if there's anything you need just let me know.");
	}
	elsif($text=~ /Trader/i)
	{
		quest::emote("smiles.");
		plugin::Whisper("Yes! I trade a great many goods with all sorts of people, if you wish I'm always looking for more people to [trade with].");
	}
	elsif($text=~ /Trade With/i)
	{
		...
		{
			plugin::Whisper("I don't have any work for you right now, check back later!");
		}
	}
