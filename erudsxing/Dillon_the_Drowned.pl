...
sub EVENT_SIGNAL {
 if($signal == 0) {
  plugin::Whisper("Again? Oh I don't know Srafen, I don't want to [bore] our company, even if it is an illusion.");
 }
 if($signal == 1) {
  plugin::Whisper("Whatever.");
 }
 if($signal == 2) {
  plugin::Whisper("Hey wait! I'm coming with!");
  quest::start(58);
