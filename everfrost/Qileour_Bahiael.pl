...
sub EVENT_SAY {
  ...
  if ($text=~/hail/i && $client->KeyRingCheck(41000)) {
    plugin::Whisper("I am busy building a strategy to take on those oddities deep under the ice here. If you need something, speak up now. I have little time for pleasantries. However, if you can rally the call of your friends perhaps you can help us with a serious [problem].");
...
#        $client->Message(13, "You have recently completed this raid!");
    ...
    if ($NewInstance > 0) {
      plugin::Whisper("Keep an eye out for the lost members while you're there, and always be on your toes. Miragul's oddities are as strange as they are deadly. These caves are not called the Frozen Nightmare for nothing.");
