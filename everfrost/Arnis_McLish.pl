sub EVENT_SAY {
  if($text=~/Hail/i){
    plugin::Whisper("My word!! I cannot believe how cold it is out here. I must keep running around just to keep warm.");
  }
  if($text=~/where is megan/i){
plugin::Whisper("I heard she got lost on the plains. You should go ask her dog, Snowflake, where she is. I hear she is somewhere around the pass to Halas.");
  }
}
...
sub EVENT_ITEM {
  ...
  if ($itemcount{13243} == 1){
    plugin::Whisper("Mmmm.. Thank you stranger. I feel a lot warmer now. You should now go and find [Megan] O'Reilly.");
    quest::exp(125);
