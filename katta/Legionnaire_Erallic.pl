sub EVENT_SAY { 
if($text=~/Hail/i){
plugin::Whisper("Well met friend. I am sorry but I musn't converse while I am on duty."); }
}

sub EVENT_ITEM
{
  if (plugin::check_handin(\%itemcount, 3881 => 1)) {
    plugin::Whisper("gasps silently for air and the falls completely still! No one seems to have heard you.");
