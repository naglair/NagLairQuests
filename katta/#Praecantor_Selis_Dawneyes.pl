...
sub EVENT_SIGNAL {
	if($signal == 1){
		plugin::Whisper("So it seems you are aware of the outcome of his hidden experimentation. Care to enlighten us as to what that outcome was?");
		quest::signalwith(160458,3,5000);
	}
	if($signal == 2){
		plugin::Whisper("Ajeea! What's happened?! I sense there are dark powers at work here!");
