sub EVENT_SAY { 
	if($text=~/Hail/i){
		plugin::Whisper("Ho there.  Whatcha want?  Yer interruptin' Arboast's story- he was just in the middle of laying some truth on me."); 
	}
}

sub EVENT_SIGNAL {
	if($signal == 1) {
		plugin::Whisper("Yeah, you tell 'em Arbogast!  I bet this one doesn't even know what's going on right under his nose in this here city.  Like for example, you probably don't even know the truth about about his importantness Phenic Dionicas do you?");
	}
	if($signal == 2) {
		plugin::Whisper("Yeah, splash it right in his unliving face!");
	}
	if($signal == 3) {
		plugin::Whisper("Ooh! I know, I know!  Nathyn Illuminious!!!");
	...
	if($signal == 5) {
		plugin::Whisper("A bloody, boldfaced lie!");
