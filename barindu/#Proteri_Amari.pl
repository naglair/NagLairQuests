...
sub EVENT_SAY {
  ...
  if($text=~/talwin/i) {
    if ($faction <= 2) { # Require Warmly and greater faction
    if (defined $qglobals{bic_bar} && $qglobals{bic_bar} == 5) {
    plugin::Whisper("Yes ... Talwin. He and I were becoming fast friends until Ixvet took him. I am not sure what has happend to him, but if you would be [interested] in helping me I may be able to assist you in return.");
  }
  }
  }
  if($text=~/interested/i) {
    if ($faction <= 2) { # Require Warmly and greater faction
    if (defined $qglobals{bic_bar} && $qglobals{bic_bar} == 5) {
    plugin::Whisper("Good, good. As you may have noticed, the majority of these creatures are brutish types best suited for destruction. They are kept under control solely by strong [leadership].");
  }
  }
  }
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 64016 => 1)) { #Crude Stone Ring
    ...
    plugin::Whisper("I knew Kunimi would come through! Our day of escape is almost here. Take this to Abena Taifa. She is the servant who brings Ixvet his meals and is loyal to our cause.");
    quest::summonitem(64007); # Poisoned Fruit V2	
  }
  if (plugin::check_handin(\%itemcount, 64012 =>1 )) {
    plugin::Whisper("Vasha told me what you did for her and I extend you my thanks as well. The bracelet she gave you is special beyond its emotional value. I sensed some magic locked away within the stone ages ago.");
    quest::emote("holds the bracelet between his hands and whispers to the stone.");
    plugin::Whisper("It's all about knowing how to convince the stone that it wants to awaken. When he opens his hands, the bracelet seems to glow from within.");
    plugin::Whisper("There you are. Again, thank you for your aid and take care with that bracelet. When the stone is freshly awakened, it tends to be a bit fragile.");
