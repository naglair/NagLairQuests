  	...
  	if ($text=~/sewers/i) {
		if($group){
			if(defined $qglobals{sewers} && $qglobals{sewers} == 2) {			
		    		...
		    		plugin::Whisper("Utandi, the map maker, should be in the sewers. I must apologize for him in advance. He is wonderful with his maps, but very timid. Good luck and I hope that we can stand together, defiant against the invaders.");
		    		if($InInstancePlant == 0 && $InInstanceCrem == 0 && $InInstancePool == 0 && $InInstanceLair == 0){
				...
				plugin::Whisper("Go find Ansharu. He has been researching their behavior patterns, and will be able to tell you what needs to be done. I hope that you will be able to help us out in these dire times. Please head to the sewer plant and find the source of the stonemite infestation. If you do this for us, I will share with you how to pass through the mountains and up to the temples created by the trusik, our exiled kin.");
				if($InInstancePlant == 0 && $InInstanceCrem == 0 && $InInstancePool == 0 && $InInstanceLair == 0){
