sub EVENT_SAY {
   if($text=~/hail/i)   {
	plugin::Whisper("Greetings $name. Lately the creatures of Lake of Ill Omen have be causing problems for us. Are you [interested] in helping me reduce the threat from this creatures?");
   }
   if($text=~/interested/i)   {
	plugin::Whisper("Good. Bring me a Brittle Iksar Skull, a Sabertooth Cub Canine, and a Large Scorpion Pincer. And i shall reward you with a spell.");
   }
}
...
sub EVENT_ITEM {
  ...
  if (plugin::check_handin(\%itemcount, 12739 => 1, 12659 => 1, 12426 => 1)) {
    plugin::Whisper("Good Work $name. This will surely hold back those vile creatures for some time. Here is your spell.");
