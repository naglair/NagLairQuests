sub EVENT_SAY {   
  if($text=~/hail/i) {
    plugin::Whisper("Quite busy!! Quite busy!! Things must be done. [New components] to be collected!!");
  }
  if($text=~/hail/i && plugin::check_hasitem($client, 4246)) {
    plugin::Whisper("So you are expecting to earn your way to rank of revenant, eh? You shall when I have the base and stem of the candle your occultist skullcap.");
  }
  if($text=~/New components/i) {
    plugin::Whisper("Yes, yes!! I will need components from beyond the gates. I must
find an [apprentice of the third rank].");
  }
  if($text=~/apprentice of the third rank/i) {
    plugin::Whisper("If you truly be an apprentice of the third circle, then there is a Dark Binder skullcap to be earned. Take this sack and fill it with a creeper cabbage, a heartsting telson with venom, brutling choppers and a scalebone femur. When they are combined within the sack, you may return it to me with your third rank skullcap and and we shall bid farewell to the title, apprentice.");
    quest::summonitem(17024);
  }
  if($text=~/true mission/i) {
    plugin::Whisper("I have been waiting for a Nihilist to return. His name was Ryx and I fear his love of ale and the high seas has kept him from his mission. All I want you to do is find him. He should be disguised as a worker and he will give you a tome to bring to me. Return it with your Dark Binder Cap. I am sure that is simple enough for one as simple as you. Be sure to give him this.");
    quest::summonitem(12848);
  }
  # missing text
  if($text=~/Kor Sha Candlestick/i) {
    plugin::Whisper("I need the foot and stem of my candlestick. The Stem comes from
...
sub EVENT_ITEM { 
  if(plugin::check_handin(\%itemcount, 12420 => 1, 4262 => 1)) { #full component sack, apprentice skullcap - 3rd rank
    plugin::Whisper("Well done, here's your fourth rank skull cap. You can now start your [true mission].");
    quest::exp(200);
  ...
  if(plugin::check_handin(\%itemcount, 55978 => 1, 4263 => 1)) { #a journal and dark binder skullcap
    plugin::Whisper("Well done, here's your fifth rank skull cap. You can now track
...
takes your cap which disintegrates in his palm. He hands you another cap.");
    plugin::Whisper("Welcome, Revenant $name. You have done well. The Harbinger awaits you. He seeks a [new revenant].");
    quest::exp(200);
