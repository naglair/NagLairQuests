...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 12914 =>1 )) {
    plugin::Whisper("Very good. I cannot see any reason a warrior like you should waste their time on the patrols of a trooper. I shall recommend you for a promotion. I hope the War Baron agrees.");
    quest::ding();
    quest::exp(2000);
  ...
  if (plugin::check_handin(\%itemcount, 12408 =>4 )) { 
    plugin::Whisper("Bravo!! You have done well. Here is a small reward. The greater reward is service to the empire.");
    quest::ding();
    quest::exp(3000);
    quest::faction(193,1);
    quest::faction(30,1);
