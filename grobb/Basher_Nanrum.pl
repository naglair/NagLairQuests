...
sub EVENT_SAY {
  if($text=~/Hail/i){
    plugin::Whisper("Peh! What am you wanted?! I am Basher Nanrum. You? $name ? Heh, you look for works? Hmm, me tinks you too weakling for [job] me need done. Hmm.. You might do, mebbe.");
  }
  if($text=~/job/i){
    plugin::Whisper("Me in charge of making torches for basher patrols. But Nanrum is much too mighty for such stupid job and Nanrum get idea. Dem fire bugses in da desert - dem eyes glowed. And dem don't burneded like torches. If $name getted Nanrum three fire beetle eyes me would giveded $name a shiny thingie dat you wanteded. Go ahed, $name , an' get me da eyes.");
  }
}

sub EVENT_ITEM {
if($itemcount{10307} <= 2){
  plugin::Whisper("Well dat be some of da eyeballses I askeded for. But I you needs ta give me three for da shiny.");
  }
if($itemcount{10307} >= 3){
  plugin::Whisper("Heh heh. All da eyeballses! I didn't think ya could do it but ya did. Here is da shiny. If you gets more I always have more shinies.");
