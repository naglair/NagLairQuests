sub EVENT_SAY {
 if($text =~ /Hail/i) {
	plugin::Whisper("Wot you want, weakling? You tink you be strong [berserker]? You fight with da anger in yer heart? Me give you test. If you can do, mebbe me thinkz you be strong.");
 }
 if($text =~ /berserker./i) {
	plugin::Whisper("Dis test be to make [armor]. Grigdor be strong. Me pass test after three timez.");
 }
 if($text =~ /armor/i) {
	quest::emote("Grigdor let's out a gravelly laugh.");
     plugin::Whisper("So you tink you can pass my test? Ok. We will see. You take dis here kit. You put stuff in dere and combine it to help make Bloodwolf Armor. Grigdor can give test to make [boots], [leggings], [bracers], [gloves], a [coif], a [gorget], or a [tunic]. Only da strongest of da Trolls can make da tunic first. It be da hardest.");
     quest::summonitem(55375);
 }
 if($text =~ /boots/i) {
	plugin::Whisper("To make da Bloodwolf Boots, youz gonna need to get one low quality bear skin, one bone chips, and two small pieces of ore. Mebba youz can find animal partz out in da Nektulos Forest. For da ore, mebbe you can find somewhere here in da city. Once you have all of dese itemz, combine dem in your assembly kit and take the Bloodwolf Boots Material to a forge, with dis mold to make your new boots.");
