...
sub EVENT_SAY {
   if ($text=~/hail/i && defined($qglobals{MMCtalk}) && plugin::check_hasitem($client, 41000)) {
      ...
      if($mmc_wins >= 24 && $qglobals{MMCtalk} == 11) {
         ...
         plugin::Whisper("Ah, Vual has sent you to me. Well met! I've been sent here by the Wayfarers Brotherhood to walk with the creatures in Mistmoore's Catacombs. It's been a very interesting experience so far. I was able to figure out how some of the more powerful vampires behave, but it's been mighty tricky. The class known as the Sustainers certainly has some appalling dynamics friend! Come back soon. I will no doubt have more information for you after my next foray into the catacombs.");
         ...
         plugin::Whisper("It's good to know I'm not the only one investigating Mistmoore's Catacombs. I am impressed! The numbers of foul beasts in there are somewhat unter control. I discovered that the Sustainers occupations are, in general, to keep the catacombs maintained and protected. For some reason, there are disputes among them even though they perform different functions. I've got to get back in there to find out more about it.");
         $client->Message(15, "You have received an invaluable piece of information!");
         if ($mmc_wins >= 28) {
            $client->Message(13, "You have additional information to uncover! Try speaking to this adventurer again.");
         }
