...
sub EVENT_SAY {
  ...
  if($text=~/amstaf/i) {
    plugin::Whisper("I've known Amstaf since he was a child. We grew up in the same city together. Ever since I've 

known him there's always been something [unusual] about him.");
  }
  if($text=~/unusual/i) {
    plugin::Whisper("When we were growing up we used to venture outside the city gates. Some times we would 
...
being with very little effort. I [learned] many things growing up with Amstaf.");
  ...
  if($text=~/learn/i) {
    plugin::Whisper("Amstaf told me many things when we were younger. Once Amstaf told me that life isn't about how 
...
[seen] before.");
  ...
  if($text=~/see/i) {
    plugin::Whisper("I saw desperation. I spoke to many people who knew Amstaf trying to find out if anyone heard 
...
did tell me that there were others that may be able to help me. She told me that I should find [Ruathey].");
  ...
  if($text=~/ruathey/i) {
    plugin::Whisper("Ruathey is known for her ability to look into people's souls and find truth. Though I wasn't 
