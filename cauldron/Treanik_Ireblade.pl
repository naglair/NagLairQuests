...
sub EVENT_SAY
	...
	if($text=~/Hail/i && $class == 16 && $level >= 46)
		{
		plugin::Whisper("You berserker, but me have nothing for you!")
		}

	
	if($text=~/Hail/i && $class == 16 && $level < 46)
		{
		plugin::Whisper("Me has nothing to do with you youngling.")
		}

	
	if($text=~/Hail/i && $class != 16)
		{
		plugin::Whisper("You not berserker, so you go away. Me no talk to you!");
		}
	
	
	if($text=~/beast fights/i && $handin == 1)
		{
		plugin::Whisper("Da only way to learn is to fight. It called Praklion of the Cauldron or sometin. Me hear it live here very long time, but rarely come out. It feed at weird times. Course, dat when it most mad. McArik found it traveling through deep craggy lands. Dat is how he decided it was good beast to test his ability to beat a baddy dat had some [sense of strategy]. It even took him long time to defeat it.");
		}
	
	if($text=~/sense of strategy/i)
		{
		plugin::Whisper("It do have sense of stragegy. It a creature dat has got smartz wit da magic too. Da way it mixes its abilities is good stuffs. You gots to try to catch it when it eats. It usually comes out when it dark out. And when you kill it, can you get my axe back? Me lost it to dat beast. Plus me needs proof you killed it so me can give you sumtin to give to McArik.");
		$handin=undef;
		}
...
sub EVENT_ITEM
	...
	if(plugin::check_handin(\%itemcount, 60200 => 1))
		{
		plugin::Whisper("My axe! Me so happy! You are big force to be rekoned with, $name. Me wasn't sure... but now, look at you, like spiroc fluffing feathers. McArik be so pleased! Take dis amulet me made. I know not much, but me swore to McArik that anyone who passes dis test would get a trinket from me.");
