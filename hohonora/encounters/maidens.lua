...
function RoomOneTrashDeath(e)
	...
	if ( roomonetrash == 3 and roomonemaid < 3 ) then 
...
function RoomTwoTrashDeath(e)
	...
	if ( roomtwotrash == 3 and roomtwomaid < 3 ) then
...
function RoomThreeTrashDeath(e)
	...
	if ( roomthrtrash == 3 and roomthrmaid < 3 ) then
...
function BossDeath(e)
	boss = boss + 1;
	if ( boss == 3 ) then
		--eq.update_spawn_timer(44032,3000000); --Alekson Garn 3 days on win
		eq.spawn2(202368,0,0, e.self:GetX(), e.self:GetY(), e.self:GetZ(), e.self:GetHeading() ); -- Spawn planar project flag mob for 10 mins
		DespawnEventMobs();
	end
end

function FailEvent()
	--eq.update_spawn_timer(44032,3000000); --Alekson Garn 24 hours on fail
	eq.zone_emote(14, "All the maidens in a room are dead! YOU HAVE FAILED!");
	DespawnEventMobs();
end

function SpawnTimer(e)
	eq.set_timer('maidens', 7100000); -- 2 hour timer
end

function MaidensTimer(e)
	if ( e.timer == 'maidens' ) then 
		eq.zone_emote(14, "Maiden Event: Time is up!!!!");
		FailEvent();
	end
end
