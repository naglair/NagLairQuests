...
sub EVENT_SAY {
  if($text=~/hail/i) {
    plugin::Whisper("Hello there friend. My name is Gearo and as you can see I'm a Gnome, so what can I do for ya?");
  }
  if(($text=~/take the gems/i) && (defined($qglobals{DeliveredToGearo}) && $qglobals{DeliveredToGearo} == 1)) {
    plugin::Whisper("Good $name! Here they are, please take them back to that gargantuan barbarian and let him know I want a raise for all this hard work!");
