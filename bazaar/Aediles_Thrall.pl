...
sub EVENT_SAY {
   if(defined $qglobals{bot_spawn_limit} && (quest::spawnbotcount() > $qglobals{bot_spawn_limit})) {
      if(($ulevel >= $firstbotlevel) && ($qglobals{bot_spawn_limit} <= 0)) {
         ...
         if($text=~/Hail/i) {
            plugin::Whisper("Hey der youngster.  Lookin' fer a bit o' [help] with yer adventurin'?");
         }
         if($text=~/Help/i) {
            quest::emote("looks around the room and pauses a few seconds");
            plugin::Whisper("Well, I kin sell ye a [product] ye can't find no place else.");
         }
         if($text=~/Product/i) {
            quest::emote("looks around the room again");
            plugin::Whisper("Shhh, i'm involved in wut I like ta call A.I.D., I kin [AID] ye in yer adventurin'");
         }
         if($text=~/AID/i) {
            quest::emote("looks around the room again");
            plugin::Whisper("It be 'Acquired Individuals Delivery' and it means I kin supply ye with [Individuals].");
         }
         if($text=~/Individuals/i) {
            quest::emote("looks around the room again");
            plugin::Whisper("Shhh, not too loud... I kin deliver ye an Individual if yer [interested]?");
         }
         if($text=~/Interested/i) {
            plugin::Whisper("Let me tell ye, keepin' up a supply o' Individuals can be a costly endeavor and so's I kin feed 'em I need ye ta be collectin' up sum [supplies] fer me.");
         }
         if($text=~/Supplies/i) {
            plugin::Whisper("The Individual will cost ye $firstbotcost platinum pieces an' 4 o' dem Fish Rolls.");
         }
      }
      elsif(($ulevel >= $secondbotlevel) && ($qglobals{bot_spawn_limit} <= 1)) {
         if($text=~/Hail/i) {
            plugin::Whisper("Ahh, a returnin' customer. Are ye [interested] in another Individual?");
         }
         if($text=~/Interested/i) {
            quest::emote("looks around the room");
            plugin::Whisper("Shhh, I kin deliver ye another Individual for $secondbotcost platinum an' 4 o' dem Gnomish Spirits... heh, dey gets thirsty ya knows.");
         }
      }
         ...
         if($text=~/Interested/i) {
            quest::emote("looks around the room");
            plugin::Whisper("Shhh, I kin deliver ye another Individual for $thirdbotcost platinum an' 4 o' dem Small Bowls... heh, dey needs ta eat outta sumtin' ya knows.");
         }
      }
         ...
         if($text=~/Interested/i) {
            quest::emote("looks around the room");
            plugin::Whisper("Shhh, I kin deliver ye another Individual for $fourthbotcost platinum an' 4 o' dem Studded Tunics ta keeps da Individuals warm.");
         }
      }
         ...
         if($text=~/Interested/i) {
            quest::emote("looks around the room");
            plugin::Whisper("Shhh, I kin deliver ye another Individual for $fifthbotcost platinum an' 4 o' dem Banded Bracers ta keeps da Individuals protected.");
         }
      }
         ...
         if($text=~/Interested/i) {
            quest::emote("looks around the room");
            plugin::Whisper("Shhh, I kin deliver ye another Individual for $nextbotcost platinum an' 4 o' dem Small Nock Class 4 Ceramic Hooked Arrows for shootin' at dem runaway Individuals.");
         }
      }
         ...
         if($text=~/Interested/i) {
            quest::emote("looks around the room");
            plugin::Whisper("Shhh, I kin deliver ye another Individual for $nextbotcost platinum an' 4 o' dem Small Nock Class 4 Ceramic Hooked Arrows for shootin' at dem runaway Individuals.");
            plugin::Whisper("If ye dun have dem arrows, ye can just pay me $nextextracost platinum.");
         }
      }
      else {
         if($text=~/Hail/i) {
            plugin::Whisper("eh? Come back when yer a bit older. I kin help ye then.");
      ...
      if($text=~/Hail/i) {
         plugin::Whisper("eh? Mind yer own business, go away!");
         quest::settimer("face", 5);
...
sub EVENT_SIGNAL {
   if($signal == 1) {
      if((defined $qglobals{bot_spawn_limit}) && ($qglobals{bot_spawn_limit} > 0)) {
         plugin::Whisper("Hey! No talkin' to da merchandise!");
      }
   }
}
