...
sub EVENT_SAY {
  if ($faction < 5) { #amiable or better Keepers of the Art
    if ($text=~/hail/i) {
      plugin::Whisper("Ah. Hello there, adventurer. Come to search for ancient artifacts with Fizzlebik and myself?");
    }
    if ($text=~/ancient artifacts/i) {
      plugin::Whisper("Oh. All types of things. This area is great. So many artifacts and remains of things all over. It's starting to fall into the pattern of things.");
    }
    if ($text=~/ancient type of bowl/i) {
    ...
    if ($text=~/take the artifact/i) {
      plugin::Whisper("Hah! Okay, then. Here take this to Farios Elianos in Felwithe. He will give you the receipt.");
...
sub EVENT_ITEM {
  if ($faction < 5) { #amiable or better Keepers of the Art
    ...
    plugin::Whisper("I do not know you well enough to help you.");
