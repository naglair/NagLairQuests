...
sub EVENT_SAY {
  ...
  if ($text=~/receive the reward/i) {
    ...
    $check_named = $entity_list->GetMobByNpcTypeID(296072);
      if($check_named == 0) {
        plugin::Whisper("I'm afraid I have nothing else to offer. See to the sentinel if you have not already.");
