...
sub EVENT_SAY { 
	if($text=~/Hail/i){
		plugin::Whisper("Hello.  Let me know if I can get you a drink.");
	}
}
sub EVENT_ITEM {
	if(plugin::check_handin(\%itemcount, 18200 => 1)){#
		plugin::Whisper("Take this note and be gone with you.");
		...
		plugin::Whisper("Take this book and be gone with you.");
