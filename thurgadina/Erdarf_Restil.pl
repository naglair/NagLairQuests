...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::Whisper("Huh? Don't you know better than to interrupt a Coldain enjoying his ale? I may be a user of priestly magic but that doesnt mean I can't bash your head in.");
  }
  if ($text=~/priestly magic/i) {
    plugin::Whisper("Aye. An old one not much taken with idle chit chat.");
  }
  if ($text=~/old one/i) {
    plugin::Whisper("Very old, very wise, and on and on. You must have some point to bothering me. I'm not a merchant and I don't contract myself out for work. I've performed works for the Dain. There isn't much I haven't done..");
  }
