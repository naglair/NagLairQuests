...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 1416 => 1, 1426 => 1)) {
    plugin::Whisper("Why thank you, $name, I haven't tasted snow bunny stew this good in quite some time. Be a dear and take this back to Mordin for me.");
    quest::summonitem(1423);
    quest::exp(100000);
  }
  elsif (plugin::check_handin(\%itemcount, 1426 => 1)) {
    plugin::Whisper("Thanks for the Snow Bunny Stew, $name.");
    quest::exp(5000);
  }
  else {
