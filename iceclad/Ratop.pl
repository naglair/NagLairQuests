sub EVENT_SIGNAL {
  if ($signal == 1) {
    plugin::Whisper("Squawk! Pieces of eight! Pieces of eight!");
  ...
  if ($signal == 2) {
    plugin::Whisper("We be out. You'll be needin to make more. Arg!");
  }
    # shawl signal
  if ($signal == 101) {
    plugin::Whisper("Yar ye scruvy bearded dwarf, just over tha hill there.");
