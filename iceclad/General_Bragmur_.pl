...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 8897 => 1)) {
    # Expedition Orders
    plugin::Whisper("The final orders, follow me closely! I pray that the Kromrif have not learned of our plans to allign with the gnomes.");
    my $x = $npc->GetX();
    my $y = $npc->GetY();
    my $z = $npc->GetZ();
    my $h = $npc->GetHeading();
...
sub EVENT_SIGNAL {
   ...
   if ($signal == 10) {
      plugin::Whisper("I must go tell the Dain of these events immediately!");
      quest::signalwith(110069, 10, 1000);
      quest::settimer("bragmur4", 5);
   }
   elsif($signal == 2) {
      plugin::Whisper("Greetings Captain Nalot, I have come to you with a proposition to make.");
      quest::signalwith(110069, 3, 1000);
   }
   elsif($signal == 3) {
      plugin::Whisper("Well ya see sir, we have a surplus of Velium that we would not mind sharing with you for yer tinkerin' but would ask a favor in return for it.");
      quest::signalwith(110069, 4, 1000);
   }
   elsif($signal == 4) {
      plugin::Whisper("I represent the Dain in this matter, we would like for you to use yer Icebreaker there to head to Antonica and bring reinforcements for the war that is preparing to ensue. We are needin' all the help we can get against the Kromrif.");
...
sub EVENT_WAYPOINT_ARRIVE {
   ...
   # Hail gnome camp
      ...
      plugin::Whisper("Lo there gnomes! Tell me is your camp nearby?");
      # signal Ratop
