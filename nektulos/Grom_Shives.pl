	...
	if(quest::istaskactivityactive(216,10)){
		if($text=~/Hail/i) {
			if(defined $qglobals{halloween_race_victory} && !defined $qglobals{halloween_race_complete}) {
				...
				plugin::Whisper("Amazing! You rode swiftly and true... few could rival your speed! Take this cloak, and wear it to commemorate this day.");
				quest::updatetaskactivity(216,10);
			}
			elsif(defined $qglobals{"halloween_race_victory"} && defined $qglobals{halloween_race_complete}){
				plugin::Whisper("Amazing! You rode swiftly and true... few could rival your speed!");
				quest::updatetaskactivity(216,10);
			}
			else {
				plugin::Whisper("You managed to find your way through Nektulos to Neriak, but you did not as quickly as you could have... Let me know if you would like to [try] again.");
				quest::failtask(216);
			}
		}
		...
		if($text=~/Hail/i) {
      			plugin::Whisper("What are you waiting for? Get going!");
      		}
		if(!defined $qglobals{halloween_race_start}) {
			if($text=~/Ready/i) {
				quest::setglobal("halloween_race_start",1,1,"M4");
   				plugin::Whisper("Ride quickly and do not hesitate... Remember, you have only four minutes to reach the gates of Neriak.");
			}
		}
      	}
	elsif(!quest::istaskactive(216)){
		...
		if($text=~/Eyes/i) {
   			plugin::Whisper("Blinded, and in great pain, Ritrim mounted his steed to make his escape, the voice of his son guiding him as he charged through the forest of Nektulos weaving a [crooked path] in order to evade his pursuers, and return to Neriak.");
		}
		if($text=~/Crooked/i) {
