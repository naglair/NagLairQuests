...
sub defaultSay
{
  ...
  if (!$handled)
  {
    if ($mname=~/^Soulbinder\w/)
    {
      if($text=~/^hail/i)
      {
        plugin::Whisper("Greetings, ${name}. When a hero of our world is slain, their soul returns to the place it was last bound and the body is reincarnated. As a member of the Order of Eternity, it is my duty to [bind your soul] to this location if that is your wish.");
        quest::doanim(29);
    ...
    elsif ($mname=~/^Guard\w/)
    {
      ...
      {
        plugin::Whisper("Hail, $name! Pardon me. I am on duty, keeping $zoneln safe.");
    ...
    elsif (($mname=~/^Merchant\w/) || ($mname=~/^Innkeep/) || ($mname=~/^Barkeep/))
    {
      if($text=~/^Hail/i)
      {
        plugin::Whisper("Welcome, $name! Why don't you browse through my selection of fine goods and pick out some things you like?");
...
sub defaultDeath
{
  ...
  if (!$handled)
  {
    ...
    elsif ($mname =~ /^Guard/i)
    {
      ...
      plugin::Whisper( ($city eq 'Kelethin') ? 'Kelethin guard death!' :
