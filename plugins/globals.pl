...
{    
  ...
  if ($items > 0)
  {
    if (plugin::humanoid())
    {
      ...
      plugin::Whisper("I have no need for $itemtext1, $name. You can have $itemtext2 back.");
  ...
  if ($platinum || $gold || $silver || $copper)
  {
    if ($items == 0) # NOTE: If $items > 0, already giving back items with message, just tack coin onto it
    {
      if ((plugin::val('$item1') == 0) && (plugin::val('$item2') == 0) && (plugin::val('$item3') == 0) && (plugin::val('$item4') == 0))
      {
        ...
        if (plugin::humanoid())
        {
          plugin::Whisper("I appreciate the offer, but I can't take this money, $name.");
        }
        else
        {
      ...
      else
      {
        ...
        if (plugin::humanoid())
        {
          plugin::Whisper("Here is your change, $name.");
        }
      }
