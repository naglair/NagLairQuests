...
sub EVENT_SAY {
  if($text=~/hail/i) {
    plugin::Whisper("I had a wrench around here somewhere.")  
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 55557 => 1,55561 => 1)) { # Note to Gilipus and Worker's Badge - number 2
    plugin::Whisper("Yes, what is it? Just a moment, I cannot read without my glasses.");
    ...
    plugin::Whisper("Thank you. That is much better.' Gilipus begins to read the note. 'You need something to remove rust? Yes, I do believe I have just the thing for you. Just bring me some rubbing alcohol and I can make what you need. Now where are my tools?");
    quest::exp(80000);  
