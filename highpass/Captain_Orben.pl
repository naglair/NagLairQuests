...
sub EVENT_SAY {
  if($text=~/Hail/i){
    plugin::Whisper("Greetings, traveler! I am Captain Orben of the Highpass Guards. I keep watch over my men and the [volunteers] here at the West Gate. It's been a busy [job] here lately, with the [gnoll raids] and all.");
  }
  if($faction>=8||$faction<=5) {
    #[job]
    if($text=~/what job/i){
      plugin::Whisper("We're short-handed around here, as usual. Would you like to help us out with the [Volunteer Watch]?");
    }
    #[Volunteer Watch], [Volunteers]
    if($text=~/what volunteer/i){
      plugin::Whisper("The Volunteer Watch guards the entry gates of Highpass. Since the [gnoll raids] are becoming more and more frequent, it's a busy job. But it can pay well, depending on how many [gnolls] you slay.");
    }
    #[gnolls], [gnoll raids]
    if($text=~/what orc/i){
      plugin::Whisper("The Mucktail Gnolls of the Karanas have been trying to expand their territory. Small gnoll raiding parties are frequently rushing the West Gate. Without the [Volunteer Watch] helping us out, Highpass would probably be overrun by those vile beasts.");
    }
  } else {
    #INCORRECT TEXT
    plugin::Whisper("I will not deal with ilk such as you.");
  }
}
  ...
  if ($scalps>=1) {
    for ($i=0; $i<$scalps; $i++) {
      if ($faction>=8||$faction<=5) {
        ...
        plugin::Whisper("Great work! Maybe you can help us out again sometime?");
        ...
        plugin::Whisper("I will not aid beings such as you.");
      }
    }
  }
