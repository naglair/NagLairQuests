...
sub EVENT_SAY {
   if ($text=~/hail/i && defined($qglobals{RUJtalk}) && plugin::check_hasitem($client, 41000)) {
      ...
      if ($ruj_wins >= 62 && $qglobals{RUJtalk} == 30) {
         ...
         plugin::Whisper("Thankfully I just ate, fair $name! Not sure you know this yet, but even though the Rujarkian orcs would rather take than trade, one line of their breed, the Captive Coin, has managed to survive through treacherous, but careful, use of wealth. They build quiet trade alliances with outsiders, but still use brute strength and coercion more often than crafty wit to obtain their monies. Still, as far as orcs go, you understand, they are considered clever . . . and fearsome. Speaking of fearsome, I've got myself a raging appetite I must appease. I'll be ready to talk to you later.");
         $client->Message(15, "You have received an invaluable piece of information!");
