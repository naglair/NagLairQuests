...
sub EVENT_AGGRO {
  plugin::Whisper("Have at you intruder. This is the domain of the commander and only those strong enough to beat me shall pass.");
  quest::settimer(1,15);
}
...
sub EVENT_HP {
  ...
  if($hpevent == 31) {
    plugin::Whisper("I commend you on your tenacity, infidels. However I am through playing games. Witness the true fighting power of an Ixt Berserker.");
    quest::setnexthpevent(11);
    quest::modifynpcstat("min_hit",2500);  #increase min hit, no hard data on what should happen
  }
  if($hpevent == 11) {
    plugin::Whisper("You may yet have the strength to defeat me but I am not through with you yet.");
