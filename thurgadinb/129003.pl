...
#Handin for the 9th ring. Needs correct dialogue
  ...
  #Tormax's head
    ...
    plugin::Whisper("You have done what no Coldain could do, %t! This is indeed a glorious say in our people's history. In return for your invaluable service I present you with the Tri-plated Golden Hackle Hammer. Its magic is powerful and I am sure it will serve you well.");
...
#Dirk handin for the 10th ring
  ...
  # Runed Coldain Prayer Shawl (7th shawl)
    ...
    plugin::Whisper("Ah $name. I was hoping to see you. We require your skilled hand. The Armory is overtaxed preparing armor for the war we are preparing to wage on the Kromzek. We need you to help by creating some Field Plate for a mission that is near to execution. Go see Loremaster Solstrin in the Hall of Ancestors, give him these orders. He holds the lore recorded on how to make the field plate. Return to me when you have completed a Standard Issue Kit.");
...
# Coldain Standard Issue Kit
    ...
    plugin::Whisper("Excellent work $name. If I didn't know I would assume this was made by our most skilled artisans.  You must hurry, General Bragmur has formed camp in Iceclad. Take the kit to the General, he had to drudge forward without any armor. His [mission] must be a success if we hope to successfully defend Thurgadin against the Giants.");
	...
	if ($raid) {
		...
		for ($count = 0; $count < $raid->RaidCount(); $count++) {
			...
			if ($rpc->GetZoneID() == $zoneid) {
				$rpc->SetGlobal("Dain Frostreaver",1,5,"F");
				$rpc->SendMarqueeMessage(15, 510, 1, 1, 3000, "Your raid received credit for killing Dain Frostreaver.");
			}
		}
	}
	else {
		$client->SetGlobal("Dain Frostreaver",1,5,"F");
		$client->SendMarqueeMessage(15, 510, 1, 1, 3000, "Your group received credit for killing Dain Frostreaver.");
	}
}
