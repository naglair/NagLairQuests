...
sub EVENT_SAY {
  if ($faction == 1) { #requires ally Kromzek faction
    if ($text=~/hail/i) {
      plugin::Whisper("Greetings to you. I seek one that calls himself a druid. What do you call yourself, manling?");
    }
    ...
    if ($text=~/cap/i) {
      plugin::Whisper("I shall weave one of exceptional quality for you but you must gather the ingredients first. I require an ancient leather cap and three crushed onyx sapphires.");
    }
    if ($text=~/tunic/i) {
      plugin::Whisper("You shall be a force of nature with this tunic. Once I have gained an ancient leather tunic and three pieces of black marble the item is yours.");
    }
    if ($text=~/sleeves/i) {
      plugin::Whisper("As the bark protects the limbs of the tree, so shall these sleeves protect your arms. Bring me a pair of an ancient leather sleeves and three jaundice gems.");
    }
    if ($text=~/bracer/i) {
      plugin::Whisper("For a bracer I shall require an ancient leather bracelet, and three crushed opals. Do this and the reward shall be yours to keep.");
    }
    if ($text=~/gloves/i) {
      plugin::Whisper("The gloves shall help protect you from the elements and harm. In order for me to complete them I require a pair of an ancient leather gloves and three crushed lava rubys.");
    }
    if ($text=~/leggings/i) {
      plugin::Whisper("As the roots support the mighty oak, so shall these leggings support you. Furnish a pair of an ancient leather leggings as well as three chipped onyx sapphires and they are yours.");
    }
    if ($text=~/boots/i) {
      plugin::Whisper("Even the swiftest feet need protection so I shall provide you with these. They should help. Acquire a pair of ancient boots and three crushed flame emeralds.");
    }
  }
  else {
