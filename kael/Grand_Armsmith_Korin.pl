...
sub EVENT_SAY {
  if ($faction <= 3) {
    if ($text=~/hail/i) {
      plugin::Whisper("What do you want from me $race? I can only imagine that you are here to ask me to use my obviously superior smithing talents to create some [special helmets] to help protect your obviously frail body.");
    }      
    if ($text=~/special helmets/i) {
      plugin::Whisper("Just as I suspected $race. Using my superior skills I can customize a plate helmet to help better protect you. Give me your Malevolent Crown, Warlord's Crown, Shining Helm, Templar's Crown, Troubadour's Helm, Frostreaver's Velium Crown, Crown of the Kromzek Kings or a Cowl of Mortality and I shall make you a new helm that will better protect you.");
    }
  }
  else {
    plugin::Whisper("You must prove your dedication to Kael Drakkal and the Kromzek clan before I will speak to you $race now begone!");
  }
}
...
sub EVENT_ITEM {
  if ($faction <= 3) {
    if (plugin::check_handin(\%itemcount, 25349 => 1)) { #Malevolant Crown - Shadowknight
      plugin::Whisper("Here you are $race just as I promised. May it guard you well.");
      ...
      plugin::Whisper("These are not the item I asked for $race do not waste my time!");
      plugin::return_items(\%itemcount);
    }
  }   
  else {
    plugin::Whisper("I do not know you well enough to entrust you with such an item yet, now leave my sight $race.");
    plugin::return_items(\%itemcount);
  }
}
