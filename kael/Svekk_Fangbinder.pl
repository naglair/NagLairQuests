...
sub EVENT_SAY { 
	if((($text=~/hail/i) || ($text=~/supplies/i) || ($text=~/giants/i) || ($text=~/eastern wastes/i) || ($text=~/wakening land/i) || ($text=~/great divide/i) || ($text=~/klezendian/i)) && ($faction > 5)) {
		...
		if ($random == 0) {
			plugin::Whisper("I didn't know Slime could speak common.  Go back to the sewer before I lose my temper.");
		}
		if ($random == 1) {
			plugin::Whisper("Is that your BREATH, or did something die in here?  Now go away!");
		}
		if ($random == 2) {
			plugin::Whisper("I wonder how much I could get for the tongue of a blithering fool?  Leave before I decide to find out for myself.");
		}
	}
	...
	if(($text=~/supplies/i) && ($faction < 6)) {
		plugin::Whisper("We have all kinds of supplies!  Normally we deliver them to the giants who live outside Kael.");
	}
...
sub EVENT_ITEM {
	if($faction < 6) {
		...
		if(plugin::takeItems(25278=>1)) { # Velium Torque
			plugin::Whisper("Ahh, a Velium torque!  Here, I'll give you one hundred pieces of gold for that!'  Svekk places the torque in his pocket and drops some coins at his feet.");
			quest::ding();
		...
		if(plugin::takeItems(25270=>1, 25271=>1)) { # Supply Voucher / Klezendian Crystal
			plugin::Whisper("Did you travel to the crystal caverns to get that crystal, $name?  You are quite brave for a $race.  I have the other supplies ready for your trip, they are quite heavy but I think you will get by.");
			quest::ding();
			quest::faction(189,10); # Kromzek
			quest::faction(188,10); # Kromrif
			quest::faction(179,10); # King Tormax
