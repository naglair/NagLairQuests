...
sub EVENT_SAY {
  if ($faction <= 4) {
    ...
    if ($text=~/waste/i) {
      plugin::Whisper("It is such a shame, this bow deserves to be used. If only there were someone who would use it to hunt the beasts - someone to slay them and skin them like they should be.");
    }
    if ($text=~/hunt the beasts/i) {
      plugin::Whisper("You will, little one? One such as yourself is a hunter? I did not know your kind had any skill in such things. In fact, I do not believe your kind could hunt a wurm, not without my wonderful bow. I will not simply hand it to a foolish $race. Hunt down some big game before you bother talking to me again. Perhaps if you bring me the tail of the elusive Vluudeen.");
    }
  }
  else {
    plugin::Whisper("You need to prove your dedication to our cause before
...
sub EVENT_ITEM {
  if ($faction <= 4) {
    if (plugin::check_handin(\%itemcount, 25137 => 1)) {
      plugin::Whisper("My, you do have a hunters spirit! If you could track down Vluudeen and slay him without the aid of my bow, you must be worthy. Take the bow, huntsman. Use it to slay others of Vluudeen's kind.");
