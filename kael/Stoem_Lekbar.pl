...
sub EVENT_SAY {
  if ($faction == 1) { #requires ally Kromzek faction
    ...
    if ($text=~/i am an enchanter/i) {
      plugin::Whisper("I thought so. I have several tasks for you to accomplish. Once you have completed them I will have a cap, a robe, sleeves, wristbands, gloves, leggings and boots to reward you with.");
    }
    if ($text=~/cap/i) {
    ...
    if ($text=~/wristband/i) {
      plugin::Whisper("The crafting of this wristband requires that you bring me an ancient silk wristband and three crushed pieces of onyx sapphire.");
    }
    if ($text=~/gloves/i) {
      plugin::Whisper("For this fine pair of gloves you must seek out and return to me three crushed topazes and a pair of ancient silk gloves.");
    }
    if ($text=~/leggings/i) {
      plugin::Whisper("This pair of leggings will be yours provided you supply me with a pair of ancient silk pantaloons as well as three nephrites.");
    }
    if ($text=~/boots/i) {
      plugin::Whisper("These supple boots shall be yours upon receipt of a pair of ancient silk boots and a trilogy of crushed jaundice gems.");
    }
  }
  else {
