...
sub EVENT_SAY { 
  if ($faction == 1) { #requires ally Kromzek faction 
    if ($text=~/hail/i) { 
      plugin::Whisper("Do I see an inner rage there in your eyes? Are you a berserker then?"); 
    } 
    if ($text=~/i am a berserker/i) { 
      plugin::Whisper("I will be able to craft armor for you."); 
    } 
    if ($text=~/armor/i) { 
      plugin::Whisper("What I have for you is as such, a coif, a breastplate, armplates, bracers, gauntlets, greaves and boots."); 
    } 
    if ($text=~/coif/i) { 
    ...
    if ($text=~/greaves/i) { 
      plugin::Whisper("Bring unto me a pair of ancient tarnished chain leggings and a trilogy of flawed sea sapphires."); 
    } 
    if ($text=~/boots/i) { 
      plugin::Whisper("A fine pair of boots shall you have, once I have a pair of an ancient tarnished chain boots and three pieces of crushed black marble."); 
    } 
  } 
  else { 
