...
sub EVENT_ITEM
{
	...
	if (($faction <= 2) && (($class eq "Warrior") || ($class eq "Paladin") || ($class eq "Shadowknight") || ($class eq "Bard") || ($class eq "Cleric"))) {
		...
		#Wrists
			...
			plugin::Whisper("The Blood Standard of Dranik! So it was not destroyed! Well done, $name. I make sure this banner remains in the hands of those still loyal to the old ways. May it fly over the free armies of Kuua once again. Here is the wristguard I have promised. May they serve you well in your future battles against the Muramites!");
			if ($class eq "Warrior") {
		...
		#Arms
			...
			plugin::Whisper("A map of Old Kuua! How this map makes me long for the old days, back when we were a free race! My thanks, $name. This map may help relearn the new landscape in the context of the old. Here is a set of ornate arm guards as your reward.");
			if ($class eq "Warrior") {
		...
		#Head
			...
			plugin::Whisper("Well met, $name. Hopefully this book will contain some valuable information on the nature of Discord. To survive on my own, I must first understand the poison affecting the land. As promised, here is the helm as your reward for such a dangerous mission.");
			if ($class eq "Warrior") {
		...
		#Chest
			...
			plugin::Whisper("Hope!  Take this.");#Text made up.
			if ($class eq "Warrior") {
		...
		#Feet
			...
			plugin::Whisper("So, the embers do exist! Excellent work, $name. I plan to hold this ember in the forge when crafting in the future. This will ensure that the ember's heat is used to forge armor for the enemies of the Muramites. Poetic justice, don't you think? For your efforts, here are the greaves as promised!");
			if ($class eq "Warrior") {
