...
if ($text=~/hail/i) {
 if ((defined $qglobals{ikky} && $qglobals{ikky} == 7)) { 
   plugin::Whisper("Welcome back, $name. Have you found any clues from the [Crumbled Sanctuary] yet? Please find anything you can and return to me as soon as possible!");
  }
  else{
    quest::emote("glances at you suspiciously.");
    plugin::Whisper("Can I help you with something? Has someone sent you? Speak up!");
    plugin::Whisper("If you want to see what you've completed at any time, just ask me for a [progress update]!");
  }
}
...
if (($text=~/four temples/i) && (defined $qglobals{ikky} && $qglobals{ikky} >= 12)) {
    plugin::Whisper("As you've already uncovered, the Sanctuary of Divine Destruction was used to uncover the purpose for the other three. I believe the Muramites destroyed that temple because it holds the key to the remaining three temples and the artifacts that they hold. Now that we know what's ahead, we can continue forth and unravel this [mystery].");
}
if (($text=~/mystery/i) && (defined $qglobals{ikky} && $qglobals{ikky} >= 12)) {
    plugin::Whisper("It's clear to me that you must proceed into each of the three remaining temples and recover the three artifacts that are in them. It's up to you how you want to proceed, but you will have to enter the [Sanctuary of the Righteous], the [Sanctuary of the Glorified], or the [Sanctuary of the Transcendent]. Each one of these holds an artifact that I believe will be used to gain entrance to whatever lies beyond the Altar of Destruction. Which will you choose?");
}
...
if (($text=~/vial of smelted molten ore/i) && (defined $qglobals{ikky} && $qglobals{ikky} == 13)) { 
   plugin::Whisper("To make the ore you must use a [Inferno Scepter] with [Inanimate Ore] in a forge.");
}
if (($text=~/inferno scepter/i) && (defined $qglobals{ikky} && $qglobals{ikky} == 13)) { 
   plugin::Whisper("To create an Inferno Scepter visit Chasmos Thromsk in Abysmal Sea and purchase Flaring Coals, an Inferno Scepter mold, a Blazing torch and a Flarestone.  A highly skilled smith will be needed to successfully complete the scepter.");
   }
if (($text=~/inanimate ore/i) && (defined $qglobals{ikky} && $qglobals{ikky} == 13)) {
   plugin::Whisper("Inanimate ore can be found on various creatures that roam these lands.");
   }
}
...
if (defined $qglobals{ikky} && $qglobals{ikky} == 8) {
    if (plugin::check_handin(\%itemcount, 60166 =>1 )) {
      plugin::Whisper("Nicely sewn, $name. I would have tried doing it myself, but I have the most unsteady fingers when it comes to things like that. In any case, let me have a look at what this says.' Tublik looks over the clue for a time before continuing. 'Unbelievable. Simply astounding. These notes weren't made by the Muramites at all. In fact, they were created by trusik priests from their own flesh in some kind of [ritual].");
...
if (defined $qglobals{ikky} && $qglobals{ikky} == 9) {
  if (plugin::check_handin(\%itemcount, 60167 =>1, 60168 =>1, 60169 =>1 )) {
      ...
      plugin::Whisper("I can't be sure, but I believe we are getting close to something big, $name. I need more time to examine these clues, but I should have something for you when you return. You've done well and I know that we wouldn't have been able to get this far without your help. Good job!");
   quest::exp(100000);
...
if (defined $qglobals{ikky} && $qglobals{ikky} == 10) {
  ...
  plugin::Whisper("You have done well.  It is now time to find the final relic which is guarded by the protector.  Are you ready to [stop it]?");
  quest::setglobal("ikky",11,5,"F");
}
}
if (defined $qglobals{ikky} && $qglobals{ikky} == 11) {
  if (plugin::check_handin(\%itemcount, 60161 =>1 )) {
  plugin::Whisper("Well done!  You are now ready to venture into the [four temples].  Let me know when you are ready.");
  quest::exp(100000);
...
if (defined $qglobals{ikky} && $qglobals{ikky} == 12) {
  ...
  plugin::Whisper("Well done!  You can have these back.  It looks like you are ready for the final temple.  These artifacts must be combined with three [Vial of Smelted Molten Ore]. Once they are combined you will have your key to the final temple.  Bring it to me and I will allow you access to its secrets.");
...
if (defined $qglobals{ikky} && $qglobals{ikky} >= 12) {
  ...
  plugin::Whisper("Well done!  You can have this back.  Take it to the Temple and you will now be allowed entry.");
...
if (plugin::check_handin(\%itemcount, 60174 =>1 )){
  ...
  plugin::Whisper("You have done great things for us, but your journey is not yet over.");  #need live text
  ...
  plugin::Whisper("You have done well in the Temple, please return when you have recovered the fragment also.");
