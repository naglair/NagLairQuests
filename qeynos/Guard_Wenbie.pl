sub EVENT_SAY {
  ...
  if($text=~/Hail/i) {
    ...
    plugin::Whisper("Hello, $name. What brings you to Qeynos? Must be the mighty fine muffins over at [Voleen's Bakery]. I just love those muffins!");

  }

  if($text=~/voleen's bakery/i) {

    plugin::Whisper("Oh, it will be easier for us both if I just show you where it is. Follow me. But be quick about it, because I have to get back to my patrol.");
...
sub EVENT_ITEM {
  ...
  if ($muffins>=1) {
    ...
    for ($i=0; $i<$muffins; $i++) {
   plugin::Whisper("Oh, thank you so much. I do get hungry on patrol. Here, please take this reward for your kindness.");
