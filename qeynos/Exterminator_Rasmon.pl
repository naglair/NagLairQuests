sub EVENT_SAY {
  if($text=~/Hail/i){ #Start of Exterminate the Vermin Quest (Scripted By: Fatty Beerbelly)
    plugin::Whisper("Hello, $name. Have you seen these pesky rodents? They are everywhere, and I simply cannot stand them! If you are willing to do a little cleaning up of the pests here, I will reward you for every four rat whiskers you bring me.");
  }
}
...
sub EVENT_ITEM {
  ...
  if(plugin::check_handin(\%itemcount, 13071 => 4)){ #End of Exterminate the Vermin (Scripted By: Fatty Beerbelly)
    plugin::Whisper("I am very impressed, $name! A few more cleaners like yourself and we could have a rodent-free Qeynos in no time!");
