sub EVENT_SAY { 
if($text=~/Hail/i){
plugin::Whisper("Hello..  Welcome to the Wind Spirit's Song.  I'm Sansa, and I help out the local bards here."); 
}
if ($text=~/Fabian/i) {
  plugin::Whisper("Ahh - jolly Fabian. He is one of our best customers! That is, of course, due to the fact that he has the heaviest fingers of any bard I know. Anyhow, down to business. I assume he sent you for a new set of strings. Unfortunately, I have not been able to procure any of the special - uhh - things needed to make his strings lately.");
}
if ($text=~/special things/i) {
  plugin::Whisper("Uhh - ummm - uhhh - Fabian does not know this, but his strings are - uh - made of - uh - troll guts.");
}
if ($text=~/troll guts/i) {
  plugin::Whisper("Well, Fabian likes the thick resonance that only troll gut strings can provide. Trouble is, trolls are scarce around these parts,
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 13703 => 1, $platinum >= 1)) {
    plugin::Whisper("I am not even going to bother asking where you got that. Just take this set I already made and hurry them back to Fabian.");
