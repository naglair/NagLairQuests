...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::Whisper("Hail, $name - If you are interested in helping the League of Antonican Bards by delivering some mail then you should talk to my wife, Eve.");
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 18150 => 1) || plugin::check_handin(\%itemcount, 18151 => 1)) {
    plugin::Whisper("Incoming mail - very good!  Please take this gold for your troubles.");
