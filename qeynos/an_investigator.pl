...
sub EVENT_SAY {
  if($text=~/first suspect/i) {
    plugin::Whisper("The first suspect was last seen near the shop called Fireprides.");
    #quest::unique_spawn(1183,0,0,$x+10,$y+10,$z+10);
  }
  if($text=~/follow me/i) {
	plugin::Whisper("Right, let's get going.");
	quest::follow($userid);
 }
 ...
 if($text=~/second suspect/i) {
	plugin::Whisper("The second suspect was sighted below the Grounds of Fate.");
 ...
 if($text=~/third suspect/i) {
	plugin::Whisper("The last suspect has been known to hang around that low life known as Raz the Rat.");
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 18289 => 1)) { #Investigator's Briefing
    plugin::Whisper("Very well, let's get to work. I assume you have read the briefing and committed it to memory. This is how you will be tested. I shall play the part of student and you shall play the master who directs me. I promise I won't make it easy on you either, we want only the best. We'll see how you do and then I'll decide if you are worthy to wear the badge or not."); 
    quest::follow($userid);
  }
  elsif(plugin::check_handin(\%itemcount, 2369 => 1, 2394 => 1, 2279 => 1)) { #Riley's Confession, Willie's Confession, Head of Donally Stultz
    plugin::Whisper("You have done our city a great service and helped to bring several criminals to justice this day! Wear this Investigator's Badge with pride! Speak again with Vegalys Keldrane, he may have information about [advancing further] in rank.");
    quest::sfollow();
