...
sub EVENT_SAY {
  if($text=~/Hail/i){
    plugin::Whisper("Hail, $name. My name is Endric and my task is to clean the [aqueducts] of Qeynos, although I don't relish the job.");
    }
  if($text=~/what aqueducts/i){
    plugin::Whisper("Aye, the aqueducts are a horrible place. I have heard rumors of horrible [beasts] who live under the city in the water and just the other day, a child playing in the water just up and disappeared!");
  }
  if($text=~/what beasts/i){
    plugin::Whisper("I know nothing of what or where they may be, just rumors, friend. However, should you foolishly choose to enter the waters to brave the possibilities and return with some evidence of the [child's] fate, I would be most appreciative.");
  }
  if($text=~/what child/i){
    plugin::Whisper("The daughter of one of our own guardsmen is missing. He has offered me a reward for any information about her disappearance. So, if you find out anything, let me know and I will cut you in on the reward.");
  }
}
...
sub EVENT_ENTER # Random speech added, that I remember being there
{
	...
	plugin::Whisper("Clean the [aqueducts] they tell me. Peh, they don't pay me enough to climb into that filth!");
	}else{
	#Do Nothing
 }
}
...
sub EVENT_ITEM {
  if($itemcount{13713} == 1){
    plugin::Whisper("Yes,  I do believe this was the child's dress. Here, take this as your share of the reward. Her father will be heartbroken..");
