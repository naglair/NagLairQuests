...
sub EVENT_SAY { 
  if($text=~/Hail/i) {
    plugin::Whisper("Greetings, $name!  Are you a [citizen of Qeynos] or are you a [traveler of sorts]?");
  }

  if($text=~/traveler of sorts/i) {
    plugin::Whisper("Ah, yes! Traveling can be truly dangerous. Have you ever been to Erudin or do you not care for that place?");
  }

  if($text=~/have been to Erudin/i) {
    plugin::Whisper("The trip to Erudin takes far too long. I have been designing a [titanic bridge] which will shorten the trip. Well, I cannot take full credit, most of the plans are being designed by [three great engineers].");
  }  

  if($text=~/three great engineers/i) {
    #text made up
    plugin::Whisper("I'm keeping the list of the engineers to myself until the project is underway.  There has been some opposition to the idea, and we wish to protect them from harm.");
  }

  if($text=~/citizen of qeynos/i) {
    plugin::Whisper("Then you are aware of how few ships come and go to the continent of Erudin. The port authority is even planning to tax all passage aboard the ships. You will be happy to know that a few other engineers and I are designing a [titanic bridge].");
  }

  if($text=~/what titanic bridge/i) {
    plugin::Whisper("My grand plan has always been to erect a bridge which would span the distance between Qeynos and Odus. It would bypass the city of Erudin. Antonius Bayle would take full control of the bridge and its adjacent territories. This has had some opposition in Erudin, but for the most part, they just believe it is impossible to build such a bridge.");
  }
}
