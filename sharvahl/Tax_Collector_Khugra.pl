...
## Fixme for lost stamped certificate.
  if(($text=~/certificate/i) && ($qglobals{Shar_Vahl_Cit}  == 3)){
    plugin::Whisper("I suppose I can issue another one. Just give me a second.");
...
sub EVENT_ITEM {
  ...
  if(plugin::check_handin(\%itemcount, 2874 => 1)) {
    ...
    plugin::Whisper("Ahh, a new taxpayer, wonderful! You must always remember that it is a distinct privilege to contribute to the upkeep of our noble society and not merely a duty or a burden. I look forward to collecting your honorable taxes in the future. May the spirits prosper you, $name.'");
