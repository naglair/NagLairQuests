sub EVENT_WAYPOINT_ARRIVE {
	...
	if($wp == 49) {
    plugin::Whisper("Uh oh? this looks dangerous, I don't think I am allowed to be here.");
	}
	if($wp == 51) {
    quest::emote("notices the pile of bones next to her and shudders, 'Eeep! I wonder what happened to him.");
	}
	if($wp == 57) {
    plugin::Whisper("Hi, how do I get outside, huh?");
	}
	if($wp == 58) {
	...
	if($wp == 68) {
    plugin::Whisper("Oh look, there it is!");
	}
	if($wp == 80) {
...
sub EVENT_SAY {
	...
	if(($wp < 17) && ($text=~/Hail/i)){
		plugin::Whisper("Hello, my name is Shainai and I am on a very important mission for my Daddy. He ran out of buttons for the officers' clothes so I have to get him more. I am an excellent helper.");
	}
	if(($wp > 60) && ($text=~/Hail/i)){
		plugin::Whisper("Hi there, I am headed home before I get in trouble.  I got a little lost, but I am ok now.  I hope Daddy isn't too mad.  Bye bye!");
	}	
	if((($wp > 16) && ($wp < 61)) && ($text=~/Hail/i)){
		plugin::Whisper("Hi. I was getting some buttons for Daddy and I got a little lost. I know the way, I mean it...but if you wanted to [follow] me home to make sure I got there safe and all, you could.");
	}
	if((($wp > 16) && ($wp < 61)) && ($text=~/follow/i)){
		plugin::Whisper("You will?!? Oh good, now I won't be so lonesome. Here hold my bag of buttons if you please, my arms are tired. Ready? Follow me now, I know the way.  'Shainai pauses and looks around, ' Hmmmm...");
                quest::ding();
