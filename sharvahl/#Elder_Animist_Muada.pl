...
if ($text=~/hail/i) {
    plugin::Whisper("Hm, you appear to have the makings of a great master of beasts, but I do believe you must prove yourself. You can perhaps help with some tasks should you [feel able] to.");
  }
  if ($text=~/feel able/i) {
  ...
  if ($text=~/dedication is true/i) {
    plugin::Whisper("Very well then $name. You must collect fungal padding, a rare piece of acrylia ore, and two gems of the void. Many of you have completed this task with little difficulty if you should need further direction. Give these items to Weapons Master Rahoul when you are done.");
  }

if(plugin::check_hasitem($client, 8495) && plugin::check_hasitem($client, 8496)){
 if($text=~/hail/i) {
    plugin::Whisper("I will say 'tis very good to see you again. Shall I assume we have some business to tend to?");
  }
 }

if ($text=~/business to tend to/i) {
  plugin::Whisper("We can only talk in generalities for now as we are still investigating the strange disturbance with the feral spirits. I can tell you that the opening of a way to Discord and that dark realm itself have something to do with all of this. The spirit world has been [infected] by Discord somehow -- we are certain of that.");
}
...
if ($text=~/wild/i) {
 ...
 if ($text=~/believe/i) {
  ...
  if ($text=~/letter/i) {
      ...
      plugin::Whisper("Oh, no. What a terrible turn. How can this be? Two of our most [majestic and unique creatures] -- with such wisdom and magic. Of all, I thought these would not be touched or affected.");
}
}
if (defined $qglobals{muadalet} && $qglobals{muadalet} == 7) {
  if ($text=~/majestic/i) {
  plugin::Whisper("You would know them as two halves of a whole, I suppose. I cannot speak their names for fear. There is so much we don't know and so many suspicions. The corruption of our precious beasts feels more than coincidence now. These creatures that stand so proud and regal know what is happening to them and we need to help them, but that path is a difficult one. I cannot tell you more as we can trust very few and there are prying eyes and ears. Go to Marrey McGrannel and ask her about the danger to our majestic creatures.");
  }
  }
if (defined $qglobals{muadalet} && $qglobals{muadalet} == 10) {
  if ($text=~/does not end/i) {  
  plugin::Whisper("There is still much to do as the sickly magic of Discord trickles through time and space to Norrath and Luclin. It appears that we may need to go to its source. You must go to Rashara. She will have a tale to tell you that will help you on your way to ending this threat to the wild.");
  }
}
if (defined $qglobals{muadalet} && $qglobals{muadalet} == 11) {
  if ($text=~/Rashara sent me/i) {  
  plugin::Whisper("It seems that as soon as we overcome one problem, another makes itself known. This time, though, we have found a [beast] in Discord that truly means to abuse the strength of the wild spirits");
}
}
if (defined $qglobals{muadalet} && $qglobals{muadalet} == 11) {
  if ($text=~/beast/i) {  
  plugin::Whisper("There is a foul creature that is borne of Discord itself. We believe it has discovered a way to abuse the ways of the beastlord for its own gains. It has invaded the ethereal world of the wild and is trapping the magic and power we consider sacred. It must be [stopped] before it grows ever powerful and erodes the ability of all beastlords to be at one with our feral companions.");
}
}
if (defined $qglobals{muadalet} && $qglobals{muadalet} == 11) {
  if ($text=~/stopped/i) {  
  plugin::Whisper("Go speak to Erana Farrel. She is in the city of the dragorn and will most likely be in hiding while she investigates. She was the first to discover this evil plot and has the most information to help you. Give her this cloak. She had asked for some protection while she was out in that volatile realm.");
