...
sub EVENT_SAY { 
if($text=~/Hail/i){
plugin::Whisper("Greetings citizen... you are very brave to approach me in this manner.  That sort of bravery warrants my respect.  How may I help you?"); }
##Non-canon fixme if you lose note.
if(($text=~/note/i) && ($qglobals{Shar_Vahl_Cit} == 5)){
  plugin::Whisper("I guess I can write you a new note. Try not to lose it this time.");
  quest::summonitem("18304");
}
}
sub EVENT_ITEM { 
if(plugin::check_handin(\%itemcount, 18299 => 1)){
plugin::Whisper("Your humility and willingness to serve shall not be wasted. There is much to be done. $name. and our people thank you in advance for your selfless service.");
