...
sub EVENT_SAY { 
if($text=~/Hail/i){
plugin::Whisper("Greetings, young one. If you are here for training you must first present to me the note you were given.");
}
...
if(($text=~/cloak/i) && ($qglobals{Shar_Vahl_Cit} == 7)){
  plugin::Whisper("Someone found a grimling wearing this in the pit. Try not to lose it this time.");
  quest::summonitem(2878);}
}

sub EVENT_SIGNAL {
  if ($signal == 1) {
    plugin::Whisper("I know many of the taverns and eateries have their own barrels, but you may want to try Jakhal and Kahala's Brewery. You can find it in the southwestern section of the Merchant's Quarter, or Corridor of Heroes, as I like to call it..");
}
 }
...
plugin::Whisper("Welcome back, $name! I see that your documents are in order and you are an official citizen of Shar Vahl, which is great news! Let me be the first to welcome you to the ranks of the Dar Khura! Here, take this cloak and wear it with pride.");
plugin::Whisper("Present your acrylia slate to spiritist Fehril and he will guide you through your early training. May the strongest of our ancestral spirits be your inspiration.");
quest::summonitem(2878);
