...
sub EVENT_SAY {
   ...
   if($FindTomer == 1) {
      if($text=~/Hail/i) {
         plugin::Whisper("Ahhhgggg.. Those mangy [dogs] put up a tough fight.. If I don't make it back to the [Clan House] soon, it'll all be over for me.");
      }
      if($text=~/dogs/i or $text=~/mutts/i) {
         plugin::Whisper("It's those Darkpaws.. They've beat me pretty badly.. but they'll get theirs soon enough!");
      }
      if($text=~/clan house/i) {
         plugin::Whisper("I am a new member of the Silent Fist Clan.. I need someone from my guild to help me find my way back.");
      }
      if($text=~/Seta sent me to find you/i) {
         plugin::Whisper("Oh.. Thank goodness you found me.. I'm lost and weak, those [mutts] are a vicious lot.. an you carry my [backpack] for me?");
      }
      if($text=~/backpack/i) {
         # This flag is set by Seta_Bakindo
         plugin::Whisper("Thank you, friend.. Now, can you [lead me back to Master Seta] of the Silent Fist Clan? ");
         # Bag of Provisions ID-20456
         quest::summonitem("20459");
      }
      if($text=~/lead you back to master seta/i) {
         plugin::Whisper("Lead the way, and I shall follow. When we make it back, please inform Seta that you have rescued me.");
   ...
   # You must have the updated flag from Seta_Bakindo for this part quest
      ...
      if($text=~/Hail/i) {
         plugin::Whisper("Yes, he saved my life.. I owe him much thanks. Please return my pack to me now, good friend.");
      }
   }
}
...
sub EVENT_ITEM {
   ...
   if($FindTomer == 2 && plugin::check_handin(\%itemcount, 20459 => 1)) {
      quest::ding();
      plugin::Whisper("Oh, you have the makings of a true hero.. The Silent Fist Clan is proud to have you as ally. May your soul guide and protect you through these chaotic times.");
      quest::exp("100");
