...
sub EVENT_SAY {
	...
	## NPC: Planes Keeper
	my $zoneaccessname = "Planes";
	my @killsarray = ("AtenHaRa");
	my $flaglevelgiven = 4;
	my $summoneditem = 0;  ## Make 0 if nothing is summoned!
	my $portsenabled = 1;  ## 1 = enabled     0 = disabled or a npc that ONLY handles flagging with no ports
	my @portzones = ("poknowledge","potranquility");  #array containing short names
	#############################################################################################################
