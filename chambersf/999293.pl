...
my $timorous = quest::saylink ("Timorous Deep",1);

if ($text =~/Hail/i && $qglobals{"flagging"} == 0)
{
plugin::Whisper(" Sorry, $name. You are not flagged for these zones");
}

if ($text =~/Hail/i && $qglobals{"flagging"} >=1)
{
plugin::Whisper ("Well hello there $name. Would you like to go to [$field], [$firiona], [$overthere], [$emerald], [$skyfire],[$dreadlands], [$timorous], [$guildlobby] or [$abysmal]?  ");
}
if ($text =~/fob/i)
{
