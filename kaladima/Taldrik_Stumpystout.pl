sub EVENT_SAY {
	if($text=~/hail/i){
		plugin::Whisper("Move along dere yer blocking me view lad. I await me friend that has gone off to fetch me some water. I sure is hot out here.");
	}
	if($text=~/recipe/i){
...
sub EVENT_ITEM {
	...
	# Disciple Symbol of Brell Serilis
		plugin::Whisper("Ah ha! So ye are Bronlors chosen aye? Well den these fine brews can only mean one thing! Yep its dat youre a drinker like meh! Arg, if I only had me [recipe].");
	}
