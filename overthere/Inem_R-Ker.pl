...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 10243 => 1)) {#Galdorin
    plugin::Whisper("Well done, $name. Galdorin had it coming, so don't feel too guilty about your part in this.");
    quest::summonitem(9590);
  }
  if (plugin::check_handin(\%itemcount, 52416 => 1, 10239 =>1)) {#marsingers
    plugin::Whisper("Well done, $name.  Only one more slanderous bard left to warn, and silence if necessary.");
    quest::summonitem(9590); 
  }
  if (plugin::check_handin(\%itemcount, 10246 => 1)) {#Lythe Songbird
    plugin::Whisper("That should do the trick. I think our contact will speak with you now. His name is Lirpin, and he is a paladin that works for the Wayfarer's Brotherhood. I hear he led an expedition into Kuua recently, but his current location is anyone's guess. Good Luck.");
    #set flag here
  }
}