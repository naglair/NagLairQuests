...
sub EVENT_SAY {
  if($text=~/Hail/i) {
    plugin::Whisper("Har har har! Yu funy lukking. Oooo.. do u hav doze [smarty writin's]?");
  }
  if($text=~/smarty writin's/i) {
    plugin::Whisper("Me not know what dem ar for shure. Dem only hav dees names. Umm.. let me see if I can member dem. Taaalisman de umm.. Jasinth. Dat's one of dem. Spirited of Scaley?? OH!! Dis my favorite. Kripple. Den the last is the painful one. Canaabaalize canaabaalize canaabaalize. Yep, dat super duper one. Bring me bak one of dem, me trade.");
  }
}
