sub EVENT_SAY {
 if($text=~/hail/i){
  ...
  plugin::Whisper("Are you a [new worker]? Huh? Speak or go!!");
 }
 elsif($text=~/new worker/i){
  # The New Worker [Part 2]
  plugin::Whisper("Hmmph!! Too much flesh!! If you want to work, you must first fill my eye sockets with my favorite gem. This shall be your payment for your sledgehammer.");
 }
}
...
sub EVENT_ITEM {
 ...
 if(plugin::check_handin(\%itemcount, 10023 => 1)){
  ...
  plugin::Whisper("Here!! You shall be assigned to the lower decks of the Scaled Trident.");
