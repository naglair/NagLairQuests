sub EVENT_SAY {
  if($text=~/Hail/i){
    plugin::Whisper(" It is good to see our numbers growing on this land.
...
There are many [new powers] to be gained from this land.");
  ...
  if($text=~/new powers/i){
    plugin::Whisper(" The new powers are scrolls that give us access to new and powerful spells.
My collection is almost complete. I am simply [lacking] four more and then I will return back to the homeland.");
  }
  if($text=~/lacking/i){
    plugin::Whisper(" I am missing the scroll Atol's Spectral Shackles, Tears of Druzzil, Inferno of Al'kabor, and Pillar of Frost. Should you run into one, bring it to me and I'll perform an exchange for another scroll.");
  }
}
