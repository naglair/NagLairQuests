...
sub EVENT_SAY { 
	if ($text=~/Hail/i){
  	plugin::Whisper("Welcome to the Port of Erudin. What can I help you with today?");
  }
	if ($text=~/Glory/i){
	...
	if ($text=~/safe passage/i){
  	plugin::Whisper("How unfortunate.  I don't like Trolls much, but I feel I must help.  Take this and give it to him.");#Text completely made up -Kilelen
