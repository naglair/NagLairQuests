...
sub EVENT_SAY {
  ...
  if ($text=~/hail/i) {
  if ($event == 0) {
    plugin::Whisper("I have a problem");
    }
  }
}
  if (defined $qglobals{sewers} && $qglobals{sewers} == 5) {
    ...
    if ($event == 0) {
  plugin::Whisper("I have lost my map.  It shows me how to get home. Can you help me find it?");
    }
  }
}
  if (defined $qglobals{sewers} && $qglobals{sewers} == 5) {
  if ($text=~/I will help you find it/i) {
      if ($event == 0) {
    plugin::Whisper("Thanks!  Try to find a big pile of slime, I think he has my map.");
