...
sub EVENT_SAY {
  if(plugin::check_hasitem($client, 9955) && plugin::check_hasitem($client, 5532)) { #9955 Harmony of the Soul, #5532 Water Sprinker of Nem Ankh
    ...
    if($text=~/information/i) {
      plugin::Whisper("Travel to Taelosia. There you will find a young native, named Jomaj who has some suspicious dealings with someone claiming they were with the wayfarer's brotherhood. This is the closest I have to direction, gather information from this Taelosian, and bring it to my friend Shav on the Thorn. Find proof as to whether or not these traitors actually exist and return it to me.");
    }
  }
  else {
    if($text=~/hail/i) {
      plugin::Whisper("Greetings $name. I am afraid that I need someone that I can [trust] with this task, these are dark times, and it is hard to tell friend from foe.");
    }
    if($text=~/trust/i) {
      plugin::Whisper("Hmm, well I think I have a task I can entrust you with. Speak to my brother, tell him that I have sent you to him. Earn his trust, and you shall have mine.");
    }
  }
}
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 9585 => 1)) { #9585 Magically Sealed Letter
    ...
    plugin::Whisper("Interesting, I heard about your fight with the leader of the Disciples. I've never seen anything like this. Take it to Borik Darkanvil in the Plane of Knowledge. Return to me when you are done with him, in the meantime I will try to find information on where he went after your fight.");
