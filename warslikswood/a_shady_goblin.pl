...
sub EVENT_SAY{
  if (defined($qglobals{RegalBandBathezid}) && ($qglobals{RegalBandBathezid} == 1)) { # we handed the Signet of Service yet
    ...
    if ($text=~/hail/i){
      plugin::Whisper("If yer not my contact ya best be movin' on. Got no time to waste on ya.");
    }
    if ($text=~/I am your contact/i) {
      plugin::Whisper("Are ya? Hum, let me see yer insignia then, and the note Skargus gave ya.");
    }
  }
}
...
sub EVENT_ITEM{
  if (defined($qglobals{RegalBandBathezid}) && ($qglobals{RegalBandBathezid} == 1)) { # we handed the Signet of Service yet
    if (plugin::check_handin(\%itemcount, 6475 => 1, 6471 => 1)) { # forged note & Drogan Warlord's Insignia
      ...
      plugin::Whisper("Errm.. welp, dunno what to make of the note. If I know the right of it, Skargus finally got him some girly to do his writin' for him. Ooooh I bet she's a tasty one, graarr. If I was warlord I know I'd have me a girly too. Gah, take this report and get outta my sight.");
