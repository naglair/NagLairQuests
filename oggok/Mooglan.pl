sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::Whisper("Yuz come lookeeng to make sum armorz? Yuz come to da right place, if'n yuz be a shadowknight. Is you a [shadowknight]?");
  }
  ...
  if ($text=~/legplates/i) {
    plugin::Whisper("Wez Ogre have big legz, which make big targets for da enemies, so itz a gud ting to protekt dem as best you can. To kraft some legplates, yuz need to gather two pristine lizard scales, one black spider silk, and one chunk of multi-colored fungus, and kraft dem into dis here majikal assembly kit to get yur legplate material. Den yuz take dat material and dis mold to a forge and smelt dem together into some gud legplates.");
    quest::summonitem(51375);
  }
  if ($text=~/gloves/i) {
    plugin::Whisper("We also have big handz, which makes dem very easy to get hurt in da battles, so it also be a good idea to protekt dem too. If yuz want to kraft a gloves, yuz need to gather a pristine lizard scale, one black lizard scale, and one spiderling silk, and kraft dem into dis here majikal assembly kit to get yur gloves material. Den yuz take dat material and dis mold to a forge and smelt dem together into a some gud gloves.");
    quest::summonitem(51373);
  }
  if ($text=~/breastplate/i) {
    plugin::Whisper("Da brestplate is a gud piece to have, it protektz yuz from all da beatingz dat you get if you fight in da Feerrott. If yuz want to kraft a brestplate, yuz need to gather two pristine lizard scales, one black lizard scale, and four black spider silks, and kraft dem into dat there majikal assembly kit to get yur brestplate material. Den yuz take dat material and dis mold to a forge and smelt dem together into a gud breastplate.");
    quest::summonitem(51376);
  }
  if ($text=~/trust/i) {
    plugin::Whisper("Sumbudy make Mooglan angry. No one make me angry and live. You go into da jungle and find me four jungle spider venom sacs. Me want to make da poison to kill dis person! Dis be secret. You not tell anybody! If you do dis for me, I give you a reward. Now go!");
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 55210 => 4)) {
    plugin::Whisper("Dis be good. You do a great evil work on dis day. Take dis weapon.");
    quest::summonitem(55211);
    quest::exp(100);
  }
  else {
    plugin::Whisper("I not needz dis."); # text made up
