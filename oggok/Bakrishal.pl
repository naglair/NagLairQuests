sub EVENT_SAY {
  if($text=~/hail/i){
    plugin::Whisper("Whats do yuz wants? Yuz come to mez to lern bout speshul armorz? If'n yuz ar a Beastlord, yuz kan lern to kraft yur own armor, yuz just needz da recipe. Ar yuz a [Beastlord]?");
  }
  if($text=~/beastlord/i){
...
sub EVENT_ITEM {
  ...
  if(plugin::check_handin(\%itemcount, 55212 => 4)){
    plugin::Whisper("Guud job! Here da reward me promised you $name.");
