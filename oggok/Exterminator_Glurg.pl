sub EVENT_SAY {
  if($text=~/Hail/i){ #Start of Exterminate the Vermin Quest (Scripted By: Fatty Beerbelly)
    plugin::Whisper("Hello dere. Do you see all dese darn critters running around here? Dere are little armadillos everywhere! We need to git rid of dem. If you bring me four of dere husks, I pay you money. Too many for me to smash alone.");
  }
}
...
sub EVENT_ITEM {
  ...
  if(plugin::check_handin(\%itemcount, 19195 => 4)){ #End of Exterminate the Vermin (Scripted By: Fatty Beerbelly)
    plugin::Whisper("You smashed dem gud. Take dese coins!");
