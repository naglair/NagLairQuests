sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::Whisper("You have the aura of a beastlord I should trust. If Elder Muada hasss sent you, you must prove it to me.");
  }
}
...
sub EVENT_ITEM {
  ...
  #Hand in text is not live
    plugin::Whisper("Seek out the beast to the north.  Beware of his companions!");
  ...
  if (plugin::check_handin(\%itemcount, 52907 =>1 )) {
    plugin::Whisper("You have certainly accomplished what I was not able to. I must go now, but be sure to take this letter and the essence back to Rashara.");
