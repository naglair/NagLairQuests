...
sub EVENT_SAY
{
	...
	if ($text=~/discord/i) {
		plugin::Whisper("So, you know of Discord then! Excellent!' The dragorn extends his hand. 'I am a student of Yemall the Arcane, a dragorn noble who sought to [understand] the magic of Discord. He was a great teacher . . . until he died, leaving me to carry on the study of Discord alone. Some say the study of Discord has made me insane. Some say, 'Karsor, stop studying Discord! It is making you insane,' but I say to them, 'If I'm insane, then why are my eyes on fire?' Ha ha! That usually puts them in their place, especially when my eyes are, in fact, on fire.");
	}
	
	if ($text=~/understand/i) {
		plugin::Whisper("Ahhh, you want to understand too, do you? He he. Well, I've been studying the magic of Discord for ten years! That's a long time. So . . . I . . . understand! If you bring me a Muramite [rune], I can help you understand too.");
	}
	
	if ($text=~/rune/i) {
		plugin::Whisper("Hurry! We are running out of time! Time! It is imperative that you bring me a Muramite rune so I can translate it for you! Bring me one . . . bring one now or I will languish for eternity!");
	}
	
	#if ($text=~/mmr/i) {
	#	if(!defined $qglobals{MuramiteMinor}){
	#		plugin::Whisper("No global to reset.");
	#	}
	#	else {
	#		quest::delglobal("MuramiteMinor");
	#		plugin::Whisper("Muramite Minor Reset.");
	#	}
	#}
	...
	#		plugin::Whisper("Muramite Normal Reset.");
	#	}
	#}
	...
	#		plugin::Whisper("Muramite Greater Reset.");
	#	}
	#}
	...
	#		plugin::Whisper("Muramite Glowing Reset.");
	#	}
	#}
	...
	#		plugin::Whisper("Muramite Ancient Reset.");
	#	}
	#}
}
...
sub EVENT_ITEM
{
	if(plugin::check_handin(\%itemcount, 59983 => 1)) {#Minor Muramite Rune
		...
		if(!defined $qglobals{MuramiteMinor}){
			...
			plugin::Whisper("MuramiteMinor should be 1 now.");
		}
		else {
		...
		plugin::Whisper("global at $qglobals{MuramiteMinor}");
		...
		if (($class eq "Magician") && ($qglobals{MuramiteMinor} == 0)) {
			...
			plugin::Whisper("Hmm, a bit of magic you have discovered! I'll write the meaning of this rune onto a parchment for you. Don't show anyone else. They wouldn't understand!");
		...
		if (!($qglobals{MuramiteNormal} >= $count) && !(@spell_choices[$qglobals{MuramiteNormal}] eq "none")) {
			plugin::Whisper("Quite a find, for a filthy Human! Yes, indeed! Quite a find! Give me a moment while I decipher this rune. Do not disturb me. Do not!");
		...
		if (!($qglobals{MuramiteGreater} >= $count) && !(@spell_choices[$qglobals{MuramiteGreater}] eq "none")) {
			...
			plugin::Whisper("The filthy little $race has brought me a rune! How thoughtful! In return I shall translate its magic for you. You must keep this secret though! Mata Muram has spies everywhere.");
		...
		if (!($qglobals{MuramiteGlowing} >= $count) && !(@spell_choices[$qglobals{MuramiteGlowing}] eq "none")) {
			plugin::Whisper("Excellent find! I can decipher that for you.");
