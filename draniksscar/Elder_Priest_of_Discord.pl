...
sub EVENT_SAY {
    if($text=~/Hail/i) {
   plugin::Whisper("Should you wish to return from whence you came, tell me you wish to [go home] and I will send you.  I am also able to aid wizards and druids with the ability to [find their way] further into Discord.");
    }

    if($text=~/find/i) {
   plugin::Whisper("If you bring me the right materials -- three samples of pure discordant blood from a dark creature and a rare enchanted scroll, I can help to create teleportation spells.");
    }

    if($text=~/home/i) {
   if(defined $qglobals{OOW_PoD_Origin}) {
       plugin::Whisper("Very well.  You are now to return from whence you came.");
      ...
      quest::movepc(50, 653, -2217, 2, 100);
       ...
       plugin::Whisper("I do not know from whence you came."); # Text made up
...
sub EVENT_ITEM {
    ...
    if(plugin::check_handin(\%itemcount, 77767 => 3, 77768 => 1)) {
        ...
        plugin::Whisper("You may now travel further into Discord, true power awaits!");
        if($class eq "Wizard") {
