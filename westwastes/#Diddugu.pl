sub EVENT_SAY {
  if (defined($qglobals{muadalet}) && ($qglobals{muadalet} == 4)) {
    if ($text=~/mastodon/i) {
      plugin::Whisper("Geez. No rest for me. He he. Must go. You go to. Dangerous for you here. You not so tough!");
