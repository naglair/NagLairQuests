sub EVENT_AGGRO {
	...
	if($mlevel==51) {
		plugin::Whisper("We will protect our city at all costs.");
	}
	if(($mclass==1) && ($mgender==0) && ($mlevel !=51)){
		plugin::Whisper("You fail to understand our devotion to this ruined city!");
	}
	if(($mclass==3) && ($mgender==0) && ($mlevel !=51)){
		plugin::Whisper("You will not leave here alive!  You should not have come.");
	}
	if(($mclass==3) && ($mgender==1) && ($mlevel !=51)){	
		quest::emote("glides forward with a ferocious hiss.");
	}
	if(($mclass==1) && ($mgender==1) && ($mlevel !=51)){	
		plugin::Whisper("There are no invitations to our city!");
	}
}
