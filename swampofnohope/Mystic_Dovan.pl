...
sub EVENT_SAY
{
	...
	if ($text=~/cure disease/i) {
		plugin::Whisper("Your malady shall dissipate once you deliver to me one giant blood sac.");
	}
	
	if ($text=~/heal/i) {
		plugin::Whisper("I can call upon the power of the ancients to mend your wounds if you can deliver to me two brittle Iksar skulls.");
	}
	
	if ($text=~/purge toxins/i) {
		plugin::Whisper("The toxins shall surely be purged from your system when I have proof of your allegiance to the empire. Let that proof be one goblin watcher signal torch.");
	}
	
	#Shaman Skull Quest No.4 Event_Say
	if($text=~/I come to inquire about the Crusaders of Rok Nikol/i && $faction <= 5 && $shmskullquest >= 6) {
		plugin::Whisper("Mystic Dovan lowers his head in sorrow let us have a moment of peace. I cannot find the words to speak of the unspeakable act of the gods which took the lives of the legendary Crusaders of Rok Nolok. You must find the answers for yourself. All I can do is wait for their return. ");
	}
	
	if($text=~/Who is Galdon Vok Nir?/i && $faction <= 7 && $shmskullquest >= 6) {
		plugin::Whisper("He is a merchant hiding in The Overthere. He is greedy and will not give the skull up easily. Seek him out and ask him what he would [trade] for the skull.");
	}
}
...
sub EVENT_ITEM
{
	...
	if(plugin::check_handin(\%itemcount, 12736 => 1, 5143 => 1) && $faction <= 5 && $shmskullquest >= 6) {
		plugin::Whisper("I see that you have found your answers. Now I must ask you to to retrieve for the council the skulls of the Di Nozok.  The sarnak in Lake of Ill Omen hold one of the skulls, while [Galdon Vok Nir] holds the other. Once you have retrieved them, give them to Hierphant Zand.");
	...
	#Cure disease.
		...
		plugin::Whisper("Day destroys the night, night divides the day. Tried to run, tried to hide, break on through to the other side.");#Obviously borrowed.
