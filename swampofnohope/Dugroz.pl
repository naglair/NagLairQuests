...
sub EVENT_SAY {
  if($text=~/hail/i){
    plugin::Whisper("Fffrroaaak!!  Go away!  I don't want to [work for them] anymore! Go find another slave!");
  }
  if ($text=~/work/i) {
  ...
  if ($text=~/escape/i) {
    plugin::Whisper("After I escaped, I lost myself in the depths of the swamp. I discarded all the keys but one. I will gladly give it to you if you can do me a [small favor].");
  }
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 7236 => 1, 7256 => 1)) {
    plugin::Whisper("My thanks to you, $name. Take this key--I know not what door it opens. Now if I can just find that necklace of power that I took from my master's lair!");
