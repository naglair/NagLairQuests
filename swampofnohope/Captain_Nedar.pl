sub EVENT_SAY {
 if($text=~/hail/i){
  plugin::Whisper("Come to serve the garrison? Good. I aim to offer a reward for the return of four froglok tad tongues. Best if we handle the pesky critters before they grow too big and too numerous.");
 }
}
...
sub EVENT_ITEM {
 ...
 if(plugin::check_handin(\%itemcount, 12439 => 4)){
  plugin::Whisper("Well done. It is good to serve the Warlord. Here is a small reward befitting such a small task.");
