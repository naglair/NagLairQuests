sub EVENT_SAY {
  if($text=~/hail/i) {
    plugin::Whisper("Ah, welcome stranger. Perhaps I could interest you in one of my famous Boysenberry pies, the delight of Sky Shrine.");
    quest::signalwith(114496,1,1); #Ruru the Cook
  }
  if($text=~/boysenberry pie/i) {
    plugin::Whisper("chuckles, 'I thought you might, what say you now Ruru? Who is the better cook between us now?'");
...
sub EVENT_SIGNAL {
  if($signal == 1) {
    plugin::Whisper("Why would I admit to such a lie? Everyone loves my pies. I rarely see anyone come in here asking for your Roasted Walrus.");
    #quest::signalwith(114496,2,1); #Ruru the Cook
  }
  if($signal == 2) {
    plugin::Whisper("Please ignore my friend Ruru here. He is a bit delusional.");
