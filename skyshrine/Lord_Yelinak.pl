...
sub EVENT_ITEM {
  if ($faction <= 3) { # require kindly faction
    if(plugin::check_handin(\%itemcount, 30516 => 1)){
      ...
      plugin::Whisper("I look upon this face, and even though he is dead, the hatred burns on. My grief and anger are unchanged even in the face of ultimate fate. His grandfathers act was so despicable and all-encompassing that it instilled an... immortal loathing. Nevertheless, you have completed the task I set before you and, as such, you deserve my gratitude. Please accept this [treasure].");
      quest::summonitem(29647);
    }
    elsif(plugin::check_handin(\%itemcount, 29647 => 1)){
      plugin::Whisper("I lost the scales that made these boots in a battle with Tormaxs minions. The casters of the shrine gathered these fallen scales up and fashioned a very powerful enchantment into these boots.");
      quest::summonitem(29645);
    } 
    elsif(plugin::check_handin(\%itemcount, 29645 => 1)){
      plugin::Whisper("This helm was created by our smiths and enchanted by the priests of the shrine to provide protection to our new allies who have found that their heads are much more fragile than those of our kin.");
    ...
    plugin::Whisper("I do not know you well enough to trust you with such an item, yet.");
    plugin::return_items(\%itemcount);
  }
}
