...
sub EVENT_ITEM
{
	...
	if (plugin::check_handin(\%itemcount, 51445 => 1, 51473 => 2)) #wrists
		{
		plugin::Whisper("You've recovered the banner! And to think I thought it lost for good! I'll be sure this flies over the graves of my fallen kin. May its glory rekindle the spirits of the dragorn still under the Muramite whip! As promised, my cloth wrist band shall be your reward!");
		quest::exp(100000);
			if($class eq "Enchanter")
				{
	...
	if (plugin::check_handin(\%itemcount, 51444 => 1, 51472 => 2)) #arms
		{
		plugin::Whisper("Thank you $name. Please take this as a reward for your efforts."); #made up text Correct text unavailable
		quest::exp(100000);
			if($class eq "Enchanter")
				{
	...
	if (plugin::check_handin(\%itemcount, 51440 => 1, 51468 => 2)) #head
		{
		plugin::Whisper("You've recovered The Duskfall Chronicles! Perhaps with time the dragorn will learn to resist the poison of Discord to once again stand against a common enemy. May so many of my kin never die in vain again. Thank you, $name. As promised, here is my cloth battle hood as your reward.");
		quest::exp(100000);
			if($class eq "Enchanter")
				{
	...
	if (plugin::check_handin(\%itemcount, 51441 => 1, 51469 => 3)) #chest
		{
		plugin::Whisper("Yes, this is it the scepter of Yumall! I recognize the markings! Alas, the glow has left the scepter which means Yumall no longer draws breath. At least now I know to prepare a burial ritual in his honor. As promised, here is my former battle tunic. In the days when I still fought for Dranik, this tunic turned many blades that otherwise would have found my heart.");
		quest::exp(100000);
			if($class eq "Enchanter")
				{
	...
	if (plugin::check_handin(\%itemcount, 51443 => 1, 51471 => 2)) #feet
		{
		plugin::Whisper("Excellent work, $name. I shudder to think what names could be etched on this stone, but I thank you for your efforts nonetheless. As promised, here are my slippers as your reward. May they keep you fleet of foot and out of harm's way.");
		quest::exp(100000);
			if($class eq "Enchanter")
				{
