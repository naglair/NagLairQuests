sub EVENT_SAY {
  ...
  if ($faction < 5) {
    ...
    if ($text=~/assist/i) {
      plugin::Whisper("I vish to ztop ze Kromzek Foreman from continuing ta deztroy thiz land. Iv you vish to azzizt you can zlay ze Foreman and bring me hiz helm az proof.");
    }
  }
  else {
    # Made text up
...
sub EVENT_ITEM {
  ...
  if ($faction < 5) {
    if (plugin::check_handin(\%itemcount, 24873 => 1)) {
      # Made text up
      plugin::Whisper("Good, ze Foreman iz dead and now we will be left in peazz.  Take diz az a zign of my friendship.");
