...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::Whisper("A pleasure to meet you. I am Countess Eysa Florawhisper of the Tunarean Court. Are you a friend of the court?");
  }
  if ($text=~/friend of the court/i) {
    plugin::Whisper("You appear to be a creature that has seen many foreign lands. There are a number of fruits and seeds that I desire in order to plant them and preserve their species here in the wakening land. I will provide a bag for you that will preserve the seeds until they can be returned to me. Will you gather seeds for me? ");
  }
  if ($text=~/gather/i) {
    plugin::Whisper("I seek the following fruits and seeds. A misty acorn, emerald orange, bag of caynar nuts, vineclinger berries, ripened heartfruit, rathe berries, marr cherries, and flarefire seeds.");
...
sub EVENT_ITEM {
  ...
  if (plugin::check_handin(\%itemcount, 24863 => 1)) {
    plugin::Whisper("Ohhhh! You're the sweetest, $name. The nymphs of the Tunarean Court recognize you as a friend of our kind and our botanical brethren.");
