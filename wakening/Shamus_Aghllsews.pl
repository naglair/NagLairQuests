...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::Whisper("Welcome to my fiefdom, wayward $race. I am Duke Shamus Aghllsews of the Tunarean Court. Only those who pledge friendship or membership to the Court are allowed sanctuary in the Wakening Lands.");
  }
  if ($text=~/pledge friendship/i) {
    plugin::Whisper("You must first prove your intentions to the Tunarean Court. I will permit you to participate in an oath of friendship only after you bring me four picks used by the Kromrif Laborers that are attempting to build into our lands.");
  }
}
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 30220 => 4)) {
    ...
    plugin::Whisper("You have proven yourself to the Fauns of the Tunarean Court. The destruction of the Kromzeks building supplies will slow their progress for some time.");
    quest::summonitem(24865); #crest of the fauns
  }
  elsif (plugin::check_handin(\%itemcount,  1095=> 1)) {
    plugin::Whisper("Thank you $name, here is your reqard.  Welcome to full membership in the Tunarean Court");
