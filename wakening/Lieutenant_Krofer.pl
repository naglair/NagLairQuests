sub EVENT_SAY { 
	...
	if(($text=~/hail/i) && ($faction <= 5)){
		plugin::Whisper("Unless you're the new mercenary reinforcements I suggest you remove yourself from my presence before I decorate the bottom of my boot with your intestines.");
	}
...
sub EVENT_ITEM {
	...
	if((plugin::check_handin(\%itemcount, 22851 => 2, 22852 =>1, 1703 =>1))&& (defined $qglobals{krofer_assignment})) { # 2x panther meat, raptor meat, Tar Goo Strand
		plugin::Whisper("Well, I suppose you may be worth something more than fodder after all. Here is your payment. Speak to me again when you are ready for your next assignment. For now, get some rest, you will need it.");
		quest::ding();
