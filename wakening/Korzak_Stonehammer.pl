...
sub EVENT_SAY { 
	if(($text=~/missing helmet/i) && ($faction < 6)) { 
		plugin::Whisper("Recently helmets have been stolen from our workers.  One of them reported seeing one of those wreched flying monkey beasts carry one off.  Having those helmets back would be very useful.  If you come across any of the workers helmets, return them to me and I will bestow a reward totem upon you.");
...
sub EVENT_ITEM {
	if(plugin::check_handin(\%itemcount, 25266 =>1)) { # Giant Sack of Supplies
		plugin::Whisper("It's about time the supplies got here.  Not like it's a long trip from the center of Kael to us out here.  Payment?  I already paid Wenglawks for the supplies I ordered.  Don't give me any trouble, little one.  I have enough trouble with the missing helmets.");
		quest::ding();
