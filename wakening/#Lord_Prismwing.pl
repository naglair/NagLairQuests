sub EVENT_SAY {
  ...
  if ($faction < 5) {
    if ($text=~/friend/i) {
      plugin::Whisper("I am in need of some help. One of my heralds was sent to deliver a message to the Othmir in the Cobalt Scar some time ago and has not returned. I fear for his safety. If your travels should lead you in that direction please attempt to find news of my missing herald.");
    }
  }
  else {
    # Made text up
...
sub EVENT_ITEM {
  ...
  if ($faction < 5) {
    if (plugin::check_handin(\%itemcount, 24876 => 1)) {
      # Made text up
      plugin::Whisper("Thank you for your assistance, $name, though I am greatly saddened by the death of my herald.  Truly you are a friend of the Tunarean Court.  Take this crest as a token of my gratitude.");
