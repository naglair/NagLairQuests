...
sub EVENT_SAY {
  if($text=~/hail/i) {
    plugin::Whisper("Well met, $name. This has been a very stressful time for many of us and some of
...
sudden [change] has left many on edge.");
  ...
  if($text=~/change/i) {
    plugin::Whisper("There are a lot of people that resent the outsiders and view their presence as
...
restricted].");
  ...
  if($text=~/access was restricted/i) {
    plugin::Whisper("The Protectors of the Pine became concerned when adventurers began wandering
...
it was just our ancestors that dwelled on the [Throne of Karana's Fury].");
  ...
  if($text=~/Throne of Karana/i) {
    plugin::Whisper("Yes, that was what this place was called until recently but I'll get to that
...
[outsider] snuck through Surefall Glade and entered the Unkempt Wood."); #'outsider' still needed
  ...
  if($text=~/poacher/i) {
    plugin::Whisper("Yes poachers often sneak in and mindlessly kill our sacred bears for their
...
about though.");
  ...
  if($text=~/group/i) {
    plugin::Whisper("They are a pair of ne'er-do-well couples that have been terrorizing the animals
...
They need to be stopped. They call themselves the [Pirates of the Pine].");
  ...
  if($text=~/Pirates of the Pine/i) {
    plugin::Whisper("They are a bunch of good for nothing lowlives! I would stop them myself but
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 9270 => 1, 9271 => 1, 9272 => 1, 9273 => 1)) {
    plugin::Whisper("Great Karana's blessing!! I see justice by your hand is both swift and eternal.
