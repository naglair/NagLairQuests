...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::Whisper("The bloody Kromrif ambushed me! I escaped, but I am near death. They'll be tracking me down to finish me off at any moment. Without [help], I'm as good as dead.");
  }
  if ($text=~/help/i && $flag == 0) {
    $flag = 1;
    quest::settimer(3, 600);
    plugin::Whisper("Thank Brell! I hear them approaching from just over that hill! Slay the leader, Ghrek, and give me his elixir.");
...
sub EVENT_ITEM {
  ...
  if (plugin::check_handin(\%itemcount, 30138 => 1)) {
    quest::stoptimer(1);
    $npc->SetAppearance(0);
    plugin::Whisper("I will report your selfless actions when I return to Thurgadin. Take this for your troubles, friend. I hope you find it useful. Fare thee well.");
...
sub EVENT_TIMER {
  if ($timer == 1) {
    $npc->SetAppearance(3);
    plugin::Whisper("Ohhhh, someone... help, please...");
  }
  elsif ($timer == 2) {
    quest::depop();
  }
