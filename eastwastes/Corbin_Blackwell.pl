...
sub EVENT_SAY { 
  if($text=~/hail/i && !defined($event_spawn) && !defined($ring_seven_start)) {
    plugin::Whisper("Hurry! There's not much time. Give me the key and show me proof that you are a friend sent to rescue me...");
  }
}
...
sub EVENT_TIMER {
  if($timer == 1 && $x == -2012 && $y == 197 && !defined($event_spawn) && !defined($final_path)) {
    ...
    plugin::Whisper("Uh oh, looks like they were tipped off somehow... I hope you can handle them.");
  ...
  if($timer == 1 && $x == -3183 && $y == -586 && defined($final_path) && $final_path == 1) {
    ...
    plugin::Whisper("I have escaped! With the help of our friends here I was saved from certain death. We are in their debt.");
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 1046 => 1, 30162 => 1) && !defined($ring_seven_start) && !defined($final_path)) {
    plugin::Whisper("I thought I was a dwarfskin rug there for a minute! Thank Brell for your help stranger! Now cover me while I make good my escape. I am weakened and cannot endure much more.");
