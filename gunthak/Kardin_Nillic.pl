...
sub EVENT_ITEM{
  if(plugin::check_handin(\%itemcount, 59022 => 1)) {
    ...
    plugin::Whisper("I can't read all of it, Take the rest to Jusean Evanesque of South Qeynos, he will understand.");
    quest::summonitem(59023);
 }
  elsif(plugin::check_handin(\%itemcount, 59024 => 1)) {
    plugin::Whisper("You have done well $name, my friend will now be saved, let me share my song with you.");
    quest::exp(1000);
