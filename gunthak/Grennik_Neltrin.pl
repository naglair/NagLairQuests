...
sub EVENT_SAY { 
	if ($text=~/Hail/i){
		if (($class eq "Shaman") ||($class eq "Cleric") || ($class eq "Druid")){
			plugin::Whisper("Hello. Have you been exploring the island? I came here to discover the secrets of the Broken Skull trolls but I have yet to even explore the island. Rumors that their high priest Nadox held the power to cure any disease have been drifting around Norrath for some time. I've come to see if the rumors are true. Unfortunately the island is much more dangerous than I had imagined and I haven't found anything useful. If you find anything having to do with the troll High Priest, please bring it to me.");
		}
		else {
			plugin::Whisper("Hello, $name. Nice weather isn't it?  Well considering our location I mean.  I somehow doubt that the sun ever shines on this cursed rock.");
  	}
 	}
}
sub EVENT_ITEM {
