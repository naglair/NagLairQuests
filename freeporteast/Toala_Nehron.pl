...
sub EVENT_ITEM {
	if ($faction < 5) {
		...
		if(plugin::check_handin(\%itemcount, 12246 => 1)) {## # Box with Two Heads ID- 12246
			quest::ding();
			plugin::Whisper("Good work!! We will soon catch Opal. I have started to formulate a plan to stop her. When I complete it, I shall notify you. Here. Take this small reward. I am sure killing Shintl was no trouble. She was just a halfling.");
			quest::exp(100);
		...
		if(plugin::check_handin(\%itemcount, 13814 => 1)) {# L.S. Pouch ID-13814
			quest::ding();
			plugin::Whisper("You found my pouch! Thanks kid. Let me buy you A drink and this is for the good work. Hmmmm. It looks as though they took my voucher. Darn it! Hey... It looks like they were using my bag to hold items they were stealing. Here you go. You can have it. It looks like junk. ");
			quest::exp(200);
	...
	if ($faction == 5) {
		if(plugin::check_handin(\%itemcount, 18814 => 1)) {# A Sealed Letter ID-18814 filename "NERtoShintl"  Item will be lost if not amiable or better
			plugin::Whisper("The Steel Warriors have no cause to dislike you, but you have yet to truly prove your worth to this guild.");
		}
	}
	if ($faction > 5) {
		if(plugin::check_handin(\%itemcount, 18814 => 1)) {# A Sealed Letter ID-18814 filename "NERtoShintl"  Item will be lost if not amiable or better
