...
sub EVENT_ITEM {
  ...
  if ($facval >= 5) { #indifferent or better priests of innoruuk
    ...
    plugin::Whisper("I don't need this."); # not real text
    plugin::return_items(\%itemcount);
  }
}
