...
sub EVENT_SAY { 
   if($text=~/Hail/i){
      plugin::Whisper("Greetings!  I am the holder of the secret of the [Breastplate of Ro].  You may rest here.  You are quite safe within this camp of paladins.");
   }
   if($text=~/breastplate of ro/i){
      if($faction <= 4) { # requires amiable
         ...
         plugin::Whisper("You need to prove your loyalty to my temple.  Go to Felwithe and seek out the Clerics of Tunare.  Prove to them your faith.  Then ask the leader of this church if you are a [honored member]."); 
      }
   }
}
sub EVENT_ITEM {
   if(plugin::check_handin(\%itemcount,12309=>2) && $faction <=4) { # A Dark Cauldron
      plugin::Whisper("You have earned the mold.  You will now need go and speak with Thomas about [Lord Searfire].");
      ...
      plugin::Whisper("I instructed you to return with no less than two dark pots.");
