sub EVENT_SAY {
  ...
  if ($text=~/hail/i) {
    plugin::Whisper("My mind . . . something is [happening].");
  }
}
  if (defined $qglobals{muadalet} && $qglobals{muadalet} == 9) {
  if ($text=~/happening/i) {
     plugin::Whisper("Something . . . is wrong. My companion . . . it weakens me . . .");
...
if (plugin::check_hasitem($client, 52902)){
   if ($text=~/hail/i) {
     plugin::Whisper("You must give me the remains of that creature.");
   }
   }
   }
