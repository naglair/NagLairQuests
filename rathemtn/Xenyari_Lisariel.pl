...
sub EVENT_SAY {
   if($text=~/Hail/i){
      plugin::Whisper("Hello, $name. Oh, I wish I had a prickly pear. I just have this insatiable craving for a prickly pear. But Permafrost is just so far! <Sigh.>");
   }
   if($text=~/prickly pear/i){
      plugin::Whisper("Yes, yes! Prickly pears! Do you have any? I just love prickly pears but Permafrost is just so far away even for us druids.");
   }
}
