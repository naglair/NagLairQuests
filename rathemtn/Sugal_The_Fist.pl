sub EVENT_SAY {
    ...
    if ($race eq "Froglok") {
      if ($client->HasSkill(0) && $client->HasSkill(1) && $client->HasSkill(2) && $client->HasSkill(3) && $client->HasSkill(36)) {
        if (defined $qglobals{token} && $qglobals{token} == 1) {
          plugin::Whisper("And hail to you. Well, let me have a look at you. You sure that you have [really trained]?");
        }
        else {
          plugin::Whisper("You're late! Feast of flies, what a weakling you are! We have no time to lose. [No rest] for the strong.");
        }
      }
      else {
        plugin::Whisper("Wander away, tad. You do not have what it takes.");
      }
    }
    else {
      plugin::Whisper("The Fist Strikes!");
  ...
  if ($text=~/no rest/i) {
    if ($race eq "Froglok") {
      if ($client->HasSkill(0) && $client->HasSkill(1) && $client->HasSkill(2) && $client->HasSkill(3) && $client->HasSkill(36)) {
        plugin::Whisper("You've slept your last easy night, recruit. If you want to stay a common footman, then keep hopping. But if you think you can be elite, if you think you can hack being a commando, then you've come to the [right place]. Welcome to the Guja.");
      }
      else {
        plugin::Whisper("Wander away, tad. You do not have what it takes.");
      }
  ...
  if ($text=~/different examples/i) {
    if ($client->GetSkill(0) >= 75 && $client->GetSkill(1) >= 75 && $client->GetSkill(2) >= 75 && $client->GetSkill(3) >= 75 && $client->GetSkill(36) >= 75) {
      plugin::Whisper("From North Ro collect Blackrock Lichen and Dry Eye Weed. From Everfrost collect Ice Blood and Winter Lilly. And finally, from Lavastorm collect Stonefruit and Lava Pears. Combine them into a useful bundle and return once you've learned your way.");
    }
    else {
      plugin::Whisper("Ha! You're eager, but you're not ready. Go find some more fights.");
    }
  ...
  if ($text=~/third notch/i) {
    if (defined $qglobals{token} && $qglobals{token} == 2) {
      plugin::Whisper("Though the trolls represent a threat to our people, their threat is distant. The Lizardmen of The Feerrott, however, present a real and menacing danger. I know that they already feed on our young and plan our demise. But we will [act first].");
    }
  }
  if ($text=~/act first/i) {
    if (defined $qglobals{token} && $qglobals{token} == 2) {
      plugin::Whisper("We must learn how to fight them. We must learn how their dark lord changed them. Go, fight them at the footsteps of that black temple. Harvest their scales, their claws and their fangs. Get me three of each and put them in this bag. Turn the bag and your token in to me. It may take some time, but the blood you spill and the information you gain will help greatly. Now go.");
  ...
  if ($text=~/another option/i) {
    if ($race eq "Froglok" && defined $qglobals{token} && $qglobals{token} == 3) {
      plugin::Whisper("The lizardmen have challenged us, demanding that you represent Gukta. Should you win, war will be averted and many lives spared. Of course, it may mean your [own life].");
    }
  }
  if ($text=~/own life/i) {
    if (defined $qglobals{token} && $qglobals{token} == 3) {
      plugin::Whisper("I applaud your bravery, $name. Come victory or death, you will always be remembered. Go to the obelisk bridge in front of the temple. Give your Guja Token to their emissary. It was the only sign they would accept to guarantee your identity. And it means you will only have one chance. Be wary. They are not to be trusted.");
    }
  }
}
...
sub EVENT_ITEM {
  ...
  if (plugin::check_handin(\%itemcount, 63098 => 1)) { # Notched Gukta Token
    if ($race eq "Froglok" && defined $qglobals{token} && $qglobals{token} == 1) {
      if ($client->GetSkill(0) >= 165 && $client->GetSkill(1) >= 165 && $client->GetSkill(2) >= 165 && $client->GetSkill(3) >= 165 && $client->GetSkill(36) >= 165) {
        plugin::Whisper("Hmm, You've learned much. You know how to survive in a wide variety of environments. You've learned the ways of watery warfare. Yes, I will notch your token. But the [third notch] is something you must earn on your own.");
        quest::exp(10000);
  ...
  if (plugin::check_handin(\%itemcount, 63103 => 1, 63104 => 1)) { # Twice Notched Guja Token, Lizardman Samples
    if ($race eq "Froglok" && defined $qglobals{token} && $qglobals{token} == 2) {
      plugin::Whisper("I wish I could say I welcome your return. But news has reached us. Apparently you did your job too well. The Lizardmen are even now readying for war. We cannot afford that fight. The council has decided we must . . .accept [another option].");
      quest::exp(10000);
  ...
  if (plugin::check_handin(\%itemcount, 63106 => 1)) { # Tongue of Thugat
    if ($race eq "Froglok" && defined $qglobals{token} && $qglobals{token} == 3) {
      plugin::Whisper("Well done! The Lizardmen have been spurned. They will lick their wounded pride for months. This is a great victory for Gukta, $name. Here, take my own sigil of the Guja. And my shield. They are the best gifts I can think to give. Wear them proudly. You are truly a Guja now.");
      quest::exp(10000);
