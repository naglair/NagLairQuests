...
sub EVENT_SAY { 
   if($text=~/Hail/i){
      plugin::Whisper("Rodcet Nife welcomes you into our noble camp.  I am the keeper of the [Vambraces of Ro].");
   }
   if($text=~/vambraces of ro/i){
      if($faction <=4){
         plugin::Whisper("The mold will be offered to you when you have performed a task for the Temple of Life.  The oceans near our home are host to a plague..  the plague sharks!!  They have been infected with a deadly malady which has been turning up in the Qeynos Hills.  Kill the sharks and bring me two of their rotten shark portions as proof.");
      } else {
         plugin::Whisper("Foolish person!! The word is out amongst the followers of the Prime Healer not to trust you."); 
      }
  }
}
sub EVENT_ITEM {
   if(plugin::check_handin(\%itemcount,12310=>2) && $faction <=4) {
      plugin::Whisper("You now own a mold for the Vambracers of Ro.  Go and ask Thomas of [Lord Searfire] for the final component.");  
      ...
      plugin::Whisper("I said two portions of rotten shark meat.");
