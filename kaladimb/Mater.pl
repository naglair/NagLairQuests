sub EVENT_SAY {
  if ($text=~/hail/i) {
    if ($class eq "Rogue") {
      ...
      plugin::Whisper("Welcome to the mines of Kaladim!"); 
    }
  }
  if (($text=~/pick 628/i) && ($class eq "Rogue")) {
    if (plugin::check_hasitem($client, 12166)) { #Parrying Pick 628
      plugin::Whisper("Ah, so Diggins sent you, eh? You can prove you are worthy of the Miners Pick 628 if you bring me the head of that filthy Boog Mudtoe... oh, and 300 gold pieces should cover the cost of the pick."); #Text made up
    }
    else {
      plugin::Whisper("I do not think you are ready for such power. You need to prove yourself first. Maybe you should go talk with Diggins."); #Text made up
    }
  }
}
...
sub EVENT_ITEM {
  ...
  if (plugin::check_handin(\%itemcount, 13316 => 1)) { #Ogre Head
    if ($cash >= 30000) { #300 gold
      plugin::Whisper("Excellent work, $name. Take this as proof of your hard work!"); #Text made up
      quest::summonitem(12161); #Miners Pick 628
    }
    else {
      plugin::Whisper("It seems you are a little short on gold pieces...");
      quest::summonitem(13316); #Ogre Head
      quest::givecash($copper, $silver, $gold, $platinum);
    }
  }
  elsif (plugin::check_handin(\%itemcount, 18767 => 1)) { #Small, Folded Note
    plugin::Whisper("Very well then, take this and get to work.");
