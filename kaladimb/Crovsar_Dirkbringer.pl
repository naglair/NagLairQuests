sub EVENT_SAY {
  ...
  if($text=~/tunics/i){
    plugin::Whisper("To create your tunic material you will need to combine 5 Bricks of Crude Bronze, 1 Enraged Goblin Beads, 1 Pristine Forest Drakeling Scale, 1 Golden Bandit Tooth, 1 Basilisk Tongue and 1 Goblin Parts in your assembly kit. Once you have created the proper material take it to a forge along with this mold to fashion your very own Apprentice Miners Tunic. When ye finish yer tunic come on back fer I got one more [assignment] for a miner that I know I can trust!");
    #Summon: Crude Breastplate Mold
    quest::summonitem(19637);
  }
  if($text=~/assignment/i){
    plugin::Whisper("Well da assignment is simple $name. I jess simply need ye to fetch me some items that I need to create a minin pick dat I plan on distributing to all da new miners around dese parts dat pass my exercises, much like yerself. If you would like to help me [collect] the items I need I could perhaps fix ye up one of dese first minin picks. Whatta ya say dere $name?");
  }
  if($text=~/collect/i){
    plugin::Whisper("Das great news to hear indeed! Please seek out 1 Aqua Goblin Backbone and 2 Pristine Skunk Claws and I will surely reward ye for yer trouble. I hope to see ye soon!");
  }
}

sub EVENT_ITEM {
if(plugin::check_handin(\%itemcount, 28067 => 1, 28068 => 2)){
    plugin::Whisper("Ah thank ye $name. Here is yer minin pick.");
