	...
	if ($raid) {
		...
		for ($count = 0; $count < $raid->RaidCount(); $count++) {
			...
			if ($rpc->GetZoneID() == $zoneid) {
				$rpc->SetGlobal("Phara Dar",1,5,"F");
				$rpc->SendMarqueeMessage(15, 510, 1, 1, 3000, "Your raid received credit for killing Phara Dar.");
			}
		}
	}
	else {
		$client->SetGlobal("Phara Dar",1,5,"F");
		$client->SendMarqueeMessage(15, 510, 1, 1, 3000, "Your group received credit for killing Phara Dar.");
	}
}
sub EVENT_SPAWN {
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 69316 => 1)) { #Box of Phara Dar (Full)
    ...
    plugin::Whisper("What? Foolish mortal! I do not need these trinkets!");
  }
}
