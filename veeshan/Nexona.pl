	...
	if ($raid) {
		...
		for ($count = 0; $count < $raid->RaidCount(); $count++) {
			...
			if ($rpc->GetZoneID() == $zoneid) {
				$rpc->SetGlobal("Nexona",1,5,"F");
				$rpc->SendMarqueeMessage(15, 510, 1, 1, 3000, "Your raid received credit for killing Nexona.");
			}
		}
	}
	else {
		$client->SetGlobal("Nexona",1,5,"F");
		$client->SendMarqueeMessage(15, 510, 1, 1, 3000, "Your group received credit for killing Nexona.");
	}
}
sub EVENT_SAY {
  if ($text=~/Hail/i) {
  ...
  plugin::Whisper("If I understand this writing correctly, you have delivered. I am pleased, but I still do not trust you. However, in fairness, I shall reward you for your effort. If this reward is unsatisfactory, return it and I will find something more applicable. Now leave my lair before my hate overwhelms me!");
