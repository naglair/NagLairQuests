	...
	if ($raid) {
		...
		for ($count = 0; $count < $raid->RaidCount(); $count++) {
			...
			if ($rpc->GetZoneID() == $zoneid) {
				$rpc->SetGlobal("Silverwing",1,5,"F");
				$rpc->SendMarqueeMessage(15, 510, 1, 1, 3000, "Your raid received credit for killing Silverwing.");
			}
		}
	}
	else {
		$client->SetGlobal("Silverwing",1,5,"F");
		$client->SendMarqueeMessage(15, 510, 1, 1, 3000, "Your group received credit for killing Silverwing.");
	}
}
sub EVENT_SAY {
  if ($text=~/Hail/i) {
    plugin::Whisper("So, young one, you have come to seek an audience with me. What do you want? Hurry up and speak carefully. I'm not known for my patience or trust of your kind. Perhaps you can [assist] me by collecting a few items, then I may learn to be more trusting.");
  }
  if ($text=~/I will assist you/i) {
  plugin::Whisper("Very well. One of my scouts has been attempting to acquire the locations of powerful, yet inscribable words for me, but has not returned as soon as I'd like. I've grown impatient -- as you might guess. These words contain great magic, I'm sure of it, and when incanted correctly, create a fabled spell of extraordinary power. I had obtained some, but more are missing. Here is my lexicon and a list of found words so far. Go quickly and acquire all eight of the words I need and the spell scroll.");
...
sub EVENT_ITEM {
    if (plugin::check_handin(\%itemcount, 69324 => 1)) {
      plugin::Whisper("I'm somewhat impressed, stranger. You cannot imagine the power this spell may unleash. Take this reward and leave me. If the reward I have given you is not pleasing, hand it back and I will give you a different one. Make haste and leave me be, for I will need to recite this spell in private.");
    ...
    plugin::Whisper("I have no need for these items, you can have them back");
  }
  }
}