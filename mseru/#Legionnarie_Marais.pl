...
sub EVENT_SAY {
  if($text=~/hail/i) {
    $npc->SetAppearance(1);
    plugin::Whisper("Corporal Donfeld?");
  }
} 

sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 18332 => 1)) { #Torn Half of a Note
    plugin::Whisper("I knew someone would come if I... just held out... long enough... Take these... be careful... ' And with that, Marais allows himself to finally die.");
