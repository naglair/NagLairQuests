...
sub EVENT_SAY {
   if ($text=~/hail/i && defined($qglobals{GUKtalk}) && plugin::check_hasitem($client, 41000)) {
      ...
      if($guk_wins >= 52 && $qglobals{GUKtalk} == 25) {
         ...
         plugin::Whisper("Me not so tired now! Warlord Ykesha went so deep into de fortress and no one heard from him. Things went strange because trolls had no leader. De froggies saw dat de trolls were running around confused, so they started to organize. Uhm. Too much talking. Go away now and come back.");
         $client->Message(15, "You have received an invaluable piece of information!");
         if ($guk_wins >= 58) {
            $client->Message(13, "You have additional information to uncover! Try speaking to this adventurer again."); # red message
         }
         ...
         plugin::Whisper("De story is a bit scary. While Warlord Ykesha was crazy, de Broken Skull trolls saw a chance to kill a bunch of de Ykesha clan trolls and take de Grozmok Stone. And they did. When de Broken Skull trolls left, de froggies grabbed all de weapons they had stolen from de trolls and finished where they left off, taking over de fortress and they called it Guk! Me tell you more later. Promise!");
         $client->Message(15, "You have received an invaluable piece of information!");
         if ($guk_wins >= 60) {
            $client->Message(13, "You have additional information to uncover! Try speaking to this adventurer again."); # red message
         }
