sub EVENT_WAYPOINT_ARRIVE {
	if(($wp == 1) || ($wp == 9) || ($wp == 27) || ($wp == 42) || ($wp == 51)) {
		plugin::Whisper("Is there anything I can get for you gentlemen tonight? Perhaps a light ale, or maybe some malt beer?");
	}
	if(($wp == 2) || ($wp == 10) || ($wp == 28) || ($wp == 43) || ($wp == 52)) {
		plugin::Whisper("Alright, I've got your order, I'll be right back with your drinks.");
	}
	if(($wp == 5) || ($wp == 19) || ($wp == 37) || ($wp == 46) || ($wp == 61)) {
		plugin::Whisper("Here are the drinks you ordered, I'll put them on your tab, just pay at the counter on your way out.");
	}
	if($wp == 1) {
