...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::Whisper("I need a dark knight, one who walks in the land of the shadow. Would you be thus, a [shadowknight]?");
  }
  if ($text=~/shadowknight/i) {
    plugin::Whisper("I too serve the shadow. If you are in need of new equipment such as [helm], [legplates], [chestplate], [bracer], [vambraces], [gloves], or pair of [boots] let me know.");
  }
  if ($text=~/bracer/i) {
    plugin::Whisper("Bring me Muramite Bracer Armor to work with. One sheet of infused Muramite metal along with the new bracer from me placed in a forge should be sufficient for it to fit just right.");
  }
  if ($text=~/helm/i) {
