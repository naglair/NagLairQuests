...
if ($text=~/news/i) {
 ...
 if(($text=~/interesting information/i) && (defined $qglobals{bic} && $qglobals{bic} == 18)) {
     plugin::Whisper("Where are my manners? I should have known someone as adventurous as you would be interested in this. One of our researching experts by the name of Beanalle has recently discovered some very intriguing information about the temples beyond Yxtta and Kod'Taz.");
   }     
    if(($text=~/sealer/i) && (defined $qglobals{bic} && $qglobals{bic} >= 1)) {
      plugin::Whisper("Be careful with this one.");
      quest::summonitem(67659);
      }
