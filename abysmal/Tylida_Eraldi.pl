sub EVENT_SAY {
  if ($text=~/hail/i) {
    ...
    plugin::Whisper("Hello. I don't suppose you're a baker by profession, or perhaps you're [interested] in learning? I have several jobs that I need done, but I just don't have time to do them myself. Are you interested in helping out?");
  }
  if ($text=~/interested/i) {
    plugin::Whisper("This crew needs a lot of food. My specialty is long-lasting food items that can be taken on long journeys. Obviously a lot of Wayfarers are taking a lot of long trips these days. I was wondering if you would mind [helping me] out? I really want to spend more time working with the newly discovered strain of local wheat. There's a wild variety of wheat that only grows in the mountainous areas, which is a very odd place for any wheat to grow, But this stuff grows there, if sparsely. The thing is, there are some very odd properties to this wheat, and with the proper technique it can be made into a very [unique flour]. So if you have a chance, I would be pleased if you could [help out]. Unfortunately, the recipes I'm working on require the use of Taelosian Wheat Flour. If you can find some Taelosian Wheat, Ordin can make flour from it. Bring that to me, and I'll have plenty of tasks for you to do.");
  }
  if ($text=~/unique flour/i) {
    plugin::Whisper("I'm still a bit shocked that wheat is growing on the side of mountains. They even say that some of the creatures up there eat it by the handful like a snack. Obviously we'd prefer to make baked items from it. If you can get me some of that Taelosian Mountain Wheat, I'll make flour out of it. I'll only keep a little of it for myself.");
  }
  if ($text=~/help out/i) {
    plugin::Whisper("I can definitely use your help. I need some Taelosian Wheat Flour for the tasks I want you to do. Ordin can make flour from the raw wheat if you have any. If you bring me some flour, we can get started. You'll have to sweat a bit before we're done, but if you're up to the task I can reward you with the secret of milling the local wheat. Because I'll be asking you to find your own supplies, I can reward you a bit sooner than Ordin will.");
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 58155 => 1)) { #Taelosian Wheat Flour
    ...
    plugin::Whisper("Ah, yes. Here is your flour.");
