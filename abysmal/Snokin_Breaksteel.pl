sub EVENT_SAY {
  if ($text=~/hail/i) {
    if (quest::istaskactivityactive(170,4)) { #Smithing Collect Step 5
      plugin::Whisper("No more work now. Snokin happy wit what you do already. Snokin show you secret now. You not tell anyone!");
    ...
    if(quest::istaskactivityactive(163,4)) { #Smithing Freebie Step 5
      ...
      plugin::Whisper("Hello, Snokin can help you with da smith stuff. Can you [help] Snokin? Snokin too busy, need more time at forge with new ore, less time doing silly work for Wayfarers.");
    }
  }
  if ($text=~/help/i) {
    plugin::Whisper("You is nice person! Da Wayfarers wants Snokin to make lots of silly things for dem, but Snokin want to make stuff with [new ore], not old steel. Can you do some [silly work] for Snokin?");
  }
  if ($text=~/new ore/i) {
    plugin::Whisper("It is strange ore, Snokin never see anything like it. It very easy to work with, if you knows the way. It also make very strong steel. Dey say dat it get dat way when something big, like explosion or something magic, make all da metal line up. So dey call it aligned ore,' Snokin says the words, clearly unsure of what they mean. 'It make great steel! Very strong and make good, sharp edges if you hammer on it right way. Snokin know how to make steel from new ore. If you find any ore, bring to Snokin and Snokin give you new steel.");
  }
  if ($text=~/silly work/i) {
    plugin::Whisper("Tank you! If you make stuff for Snokin, Snokin can work more on new ore. You work as hard as Snokin, den Snokin give you good reward. Snokin knows people work good for good reward. Take [dis stuff] and make tings for me.");
    quest::taskselector(163); #Smithing Freebie
  }
  if ($text=~/dis stuff/i) {
    if(quest::istaskactivityactive(163,0)) { #Smithing Freebie Step 1
      plugin::Whisper("Take [dis stuff] and make studs so Snokin no have to. Just go to forge and put stuff in. One of each is good. When you have da studs, bring to Snokin. When you do enough work, Snokin will show you da secret for makin da new ore into da new steel.");
    ...
    if(quest::istaskactivityactive(163,1)) { #Smithing Freebie Step 2
      ...
      plugin::Whisper("Good! Snokin needs some rings for da Wayfarers. If you could make dem. Snokin be happy. Take dis stuff and go over to da forge and make rings. Bring da rings to Snokin.");  
      quest::summonitem(58113,20); #Simple Ring Mold
      quest::summonitem(58055,20); #Metal Scraps
      quest::summonitem(58103,20); #Simple Iron File
    }
    if(quest::istaskactivityactive(163,2)) { #Smithing Freebie Step 3
     plugin::Whisper("Snokin not tink you as bad as dey say. Snokin need chisels and you can make dem. Take dis stuff to da forge, put one of each of dem in and make da chisel. Bring da chisel to Snokin fast, da Wayfarers needs dem.");
     quest::summonitem(58055,20); #Metal Scraps
     quest::summonitem(58091,20); #Simple Chisel Mold
    }
    if(quest::istaskactivityactive(163,3)) { #Smithing Freebie Step 4
      plugin::Whisper("Good! More stuff for you to do for Snokin. Snokin like you. Make files and bring Snokin soon. Take dis stuff and put into forge to make files. Go now, work hard for reward, not stand and look at Snokin.");
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 58018 => 1)) { #Aligned Ore
    plugin::Whisper("Ok. Take dis steel.");
