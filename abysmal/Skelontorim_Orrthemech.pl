sub EVENT_SAY {
  if ($text=~/hail/i) {
    if (quest::istaskactivityactive(165,4)) { #Tinkering Freebie Step 5
      ...
      plugin::Whisper("That should work well enough with the infused dark matter too. Now run off and save the world, but don't be a stranger!");
    ...
    plugin::Whisper("Begone! You are not a gnome!");
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 58241 => 1)) { #Raw Dark Matter
    plugin::Whisper("Ah, yes. Here is your dark matter.");
    ...
    plugin::Whisper("Ah, yes. Good work.");
  }
