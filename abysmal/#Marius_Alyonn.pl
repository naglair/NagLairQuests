...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    if (defined($qglobals{qvichelmchange}) && ($qglobals{qvichelmchange} == 1)) {
      ...
      plugin::Whisper("Ah, so I see you have returned. Do you wish to change your headgear back? Or are you here to alter another? Either way, I am going to have to charge you a small [fee] this time. I am planning to sell this invention in Norrath and beyond, but I need to save a little more money first.");  
    }
    else {
      quest::emote("looks at $name and begins to speak.");  
      plugin::Whisper("Come to exchange your [helm] or you just like looking at gnomes?");  
    }
  }
  if ($text=~/helm/i){
    plugin::Whisper("Yes that's what I said. I have been working with the dwarves to create a device that can alter the view of your headgear. This advanced gnomish technology, that's right gnomish technology, will remove the shroud that makes your helm invisible to the naked eye. It can also replace the shroud if you decide you don't like it being shown. Amazing isn't it? And it was all my design! The only drawback to this device, appropriately named the Helmet Eclipsing Liquidation Machine or H.E.L.M. for short, is that it is only compatible with certain [headgear].");  
  }
  if ($text=~/headgear/i){
    plugin::Whisper("Well, so far I have only been successful with the latest class quest helms but I am hoping to expand soon. For now though if you have any of these and wish to try my machine out, then hand it over and let the show begin.");  
  }
  if (defined($qglobals{qvichelmchange}) && ($qglobals{qvichelmchange} == 1)) {
    if ($text=~/fee/i) {
      plugin::Whisper("'Hmm, let me see, the cost of materials, having to perform maintenance after the process is done, opening the item and closing it again, plus a small labor charge. I think a diamond sounds fair, what do you say [yes] or [no]");  
    }
    if ($text=~/yes/i) {
      plugin::Whisper("Good, then please hand the gem and I will give you my latest version of the H.E.L.M. device that you can use at your leisure. But before you give me the diamond remember that this device is only compatible with the latest class quest helms.");  
    }
    if ($text=~/no/i) {
      plugin::Whisper("Then please stop standing there looking at me like a peice of meat.");  
    }
  }
}
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 10037 => 1)) { # diamond
    plugin::Whisper("Here is the latest version of my H.E.L.M device.");  
    ...
    plugin::Whisper("There you go! Let me know what you think. If you don't like it, you can always come back, but it will cost you. This type of technology does not come cheap you know.");  
