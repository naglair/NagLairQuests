sub EVENT_SAY {
	if ($text=~/hail/i) {
		plugin::Whisper("I wonder what kind of strange fish we can catch in these new waters?");
	}
}
...
sub EVENT_SIGNAL {
	...
	if(($signal == 2) && ($x == 249) && ($y == -202)) {
		plugin::Whisper("I'll have a malt and hurry it up!  I haven't got all day.");
	}
	if($signal == 5) {
