...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 54062 => 1, 10053 => 1, 54048 => 1)) { #melee 1
    ...
    plugin::Whisper("Wonderful! I also have some more [work] for you to do in the plane of hatred if you wish."); #need real text
    quest::exp(60000);
  }
  if (plugin::check_handin(\%itemcount, 54050 => 1, 54049 => 1, 54033 => 1)) { #melee 2
    quest::summonitem(54034); #gem of contention
    plugin::Whisper("Splendid work! Should you venture to the Plane of Hatred again, I have a [final task] that needs to be completed."); #need real text
    quest::exp(60000);
  }
  if (plugin::check_handin(\%itemcount, 54051 => 1, 54034 => 1)) { #melee 3
    quest::summonitem(54035); #gem of victory
    plugin::Whisper("You are a far greater person than I. I must contact the League at once."); #need real text
    quest::exp(60000);
  }
