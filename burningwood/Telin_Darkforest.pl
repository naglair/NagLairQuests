...
sub EVENT_SAY {
  if ($faction < 4) {
    ...
    if ($text=~/what are you doing/i) {
      plugin::Whisper("Mighty this place must have been, once. Now scarred by hatred and suffering. I have been sent here seeking answers to problems not yet understood.");
    }
    if ($text=~/answers/i) {
    ...
    if ($text=~/evil/i) {
      plugin::Whisper("If I knew that, I wouldn't be here looking for answers. I would be elsewhere, looking for solutions. For now, I suspect much but know little.");
    }
    if ($text=~/suspect/i) {
    ...
    if ($text=~/action/i) {
      if ($ulevel >= 46) {
        ...
        plugin::Whisper("You are not yet powerful enough to complete this task. Return when you have gained more experience.");
      }
    }
  }
  else {
    plugin::Whisper("I can not trust you with such important information, yet.");
  }
}
