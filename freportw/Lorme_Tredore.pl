...
sub EVENT_SAY {
  if ($text=~/hail/i && ($class eq "Magician")) {
    plugin::Whisper("Hello $name. If you are new here you might have a note for me.");
    quest::emote("mumbles under his breath - Now where did I lose that Fleshy Orb.");
  }
  else {
    plugin::Whisper("Hello $name. Welcome to the Academy.");
  }
}

sub EVENT_ITEM {
    if(plugin::check_handin(\%itemcount, 13951 => 1)) { #Fleshy Orb
      plugin::Whisper("Ah. Thank you for bringing this to me! I will make very good use of it. Here take this small token of my appreciation in return. Guard Jenkins will no longer require it as he was killed on the training field yesterday. Tsk. tsk. tsk.");
      quest::ding();
      quest::exp(100);
      quest::summonitem(5353); #Fine Steel Scimitar
    }
    elsif(plugin::check_handin(\%itemcount, 18740 => 1)) { #A Tattered Note
      plugin::Whisper("Thanks. Please take this robe to help you on your journeys.");
      quest::ding();
      quest::exp(100);
