...
sub EVENT_SAY {
  if($Bakery == 0) {
    if($text=~/hail/i) {
      plugin::Whisper("Greetings! Please move along. I am not paid to converse with strangers... unless of course you have a muffin or two..?");
    }
    if($text=~/brownloe bakery/i) {
      plugin::Whisper("Oh, yes! That is close by. I am sure Lady Shae will be fine for just a minute. I will just run there and run back. Thanks, friend!");
...
sub EVENT_ITEM {
  ...
  if ($muffins >= 1) {
    for ($i=0; $i<$muffins; $i++) {
      plugin::Whisper("Mmmm. This smells delicious. Oh great!! No milk!! Don't you have any sense?! Just tell me the name of the bakery and I will run and get it myself. I am sure Lady Shae will be safe.");
      quest::ding();
      quest::exp(10);
...
sub EVENT_TIMER {
  if(($x == -693) && ($y == -279) && ($Bakery == 0)) {
    $Bakery = 1;
    quest::pause(60);
    plugin::Whisper("I sure could use some of those famous muffins you make!");
...
sub EVENT_SIGNAL {
  if($signal == 1) {
    plugin::Whisper("Thank you very much! These look delicious. Well, I need to get back to duty. I'll be back tomorrow!");
