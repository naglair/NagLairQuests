...
sub EVENT_SAY { 
  if($text=~/hail/i) {
    plugin::Whisper("Hello. It is always good to meet someone new. I am Lady Shae of the House of Dumas. And what [house] are you from?");
  }
  if($text=~/no house/i) {
    plugin::Whisper("Good. I care not to hang around any snobs this evening. Would you be so kind as to buy me some wine?");
  }
  if($text=~/steel warriors sent me/i) {
    plugin::Whisper("Thank you for checking into this matter. I told the Militia, but they just ignored me. It appears the darkelves keep coming in leaving mail for [Shintl] Lowbrew. Before I tell you more could you please buy me A white wine please.");
  }
  if($text=~/shintl/i) {
    plugin::Whisper("Dyllin was the name of A Qeynos guard who was sent to pick up the list I was holding for dear sweet Antonius. He left just yesterday. If you wish to meet up with him, I heard him say he was going to stop at Highpass Hold.");
  }
  if($text=~/house of pancakes/i) {
    plugin::Whisper("I can tell. You look like you ATE a house of pancakes.");
  }
  if($text=~/house of style/i) {
    plugin::Whisper("I would of never guessed by the way you look.");
  }
  if($text=~/dyllin/i) {
    plugin::Whisper("Dyllin was the name of a Qeynos guard who was sent to pick up the list I was holding for dear, sweet Antonius. He left just yesterday. If you wish to meet up with him, I heard him say he was going to stop at Highpass Hold.");
  }
...
sub EVENT_ITEM { 
  ...
  if ($wine >= 1) {
    for ($i = 0; $i < $wine; $i++) {
      plugin::Whisper("Thank you. Pandos has been telling me to try white wine forever. I mostly only drink red wine. Pardon me for getting off track. Anyway, it is a good thing you showed up. The lady in room 2 has been receiving mail from a Dark Elf. You [need the mail for room two]. The Innkeeper usually holds it for the guests.");
      quest::ding();
      quest::faction(100,2); #Felguard - Only NPC in the game with it.
      quest::exp(1000);
    }
