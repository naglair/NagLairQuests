...
sub EVENT_SAY {
  ...
  if($class eq "Magician" || $class eq "Enchanter" || $class eq "Wizard") {
    ...
    if($text=~/final assignment/i) {
      plugin::Whisper("I have a trinket that was given to me by my mother a long time ago. Before she died I told her I would always keep it close to me. Recently, some militia officers came in to our academy and confiscated anything that appeared to be valuable. Fearing for my life I did not fight to keep the necklace my mother gave me. The officer that took it was named Teridsan. If you should find him you have my permission to kill him if that's what it takes to get my necklace back. Return to me when you have my necklace along with 2 Lion Teeth.");
    }
  }
  else {
    plugin::Whisper("Only casters may earn the rewards of the Academy!");
  }
}

sub EVENT_ITEM {
  if($class eq "Magician" || $class eq "Enchanter" || $class eq "Wizard") {
    if(plugin::check_handin(\%itemcount, 9917 => 2, 9933 => 1)) { #Lion Tooth x 2, Shana's Necklace
      plugin::Whisper("Thank you for bringing back one of my most valued possesion. You have proven yourself to be worthy to wield the Dagger of the Academy.");
      quest::ding();
