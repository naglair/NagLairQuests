...
sub EVENT_SAY {
  if($text=~/hail/i) {
    plugin::Whisper("What do you want? Leave me be! Can't a man drown his sorrows in peace!?");
  }
}
...
sub EVENT_ITEM {
  if($class eq "Enchanter" || $class eq "Magician" || $class eq "Necromancer" || $class eq "Wizard") {
    ...
    plugin::Whisper("I am sorry. But you have no business giving me this junk.");
    plugin::return_items(\%itemcount);
  }
}
