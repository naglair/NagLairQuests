...
{ 
  ...
  if($text=~/Hail/i)
  {
  plugin::Whisper("Greetings, friend! Welcome to the Hog Caller's Inn! Don't let the name fool you.  We are the finest establishment in Freeport. Please feel free to relax in our lounge. There is usually music to soothe your blues away.");
  }

  if($text=~/need the mail for room two/i)
  {
  plugin::Whisper("Hmmph !! If you're here to get the mail for room two you first need to show me the room key.");
  }
...
sub EVENT_ITEM
{
 ...
 {
	plugin::Whisper("Here you go then.");
