...
sub EVENT_SAY { 
	if($text=~/Hail/i){
		...
		plugin::Whisper("Greetings, $name. It seems some of the strange minerals the Wayfarers Brotherhood sold have broken or stopped working as they should. Luckily, I alone have been tasked by Morden Rasp to exchange a Broken Chunk of Coal, a Cracked Ruby, a Charred Pebble, or any other pieces that don't work properly. I need to examine it first, though. Let me see what you've got!");
	}
}
...
sub EVENT_ITEM {
	...
	if(plugin::check_handin(\%itemcount, 42259 => 1)){# Radiant Slimestone of Retribution 
		...
		plugin::Whisper("Very well, $name. Go ahead and find something else you'd like.");
