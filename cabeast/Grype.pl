...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::Whisper("I will give you the Foot if you get the stein from that Halfling rogue, all in black, from Firiona Vie.");  
  }
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 30146 => 1)) {
    plugin::Whisper("Well, as promised, here's your Foot of the candlestick.");
