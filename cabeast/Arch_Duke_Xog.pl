...
sub EVENT_SAY {
  if($text=~/hail/i) {
    plugin::Whisper("These are the sacred unholy grounds of the Crusaders of Greenmist and the Scaled Mystics. If you do not belong to us, you must leave this temple at once or learn great suffering in the name of Cazic-Thule.");
  }
  ...
  if($text=~/argest/i) { #Test of the Lord (Greenmist Quest 7/8)
    plugin::Whisper("Once a Lord of Pain, Argest is now a great explorer. No lizard has seen more of Kunark than he. He returned one season ago to tell tales of an [ancient library]. He said that he believed that there he would find a tome which would reveal the location of the ancient crusader weapon, Greenmist.");
  }
  if($text=~/ancient library/i) { #Test of the Lord (Greenmist Quest 7/8)
    plugin::Whisper("There are many ancient libraries yet to be discovered. Our once great cities have been decimated, if not by our foes, then by nature itself. Within the outlands are many ruins which have yet to reveal themselves. We look forward to the discovery of these ruins by such explorers as Lord Argest the Great. If only we knew [where] he was...");
  }
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 18205 => 1)) { #Test of the Newbie (Greenmist Quest 0/8)
    ...
    plugin::Whisper("A legible tome of the scrolls of Vok Na Zov! What a find this is!! May the unholy curses of Cazic flow through you. Please accept the weapon of a Lord of Pain. To abandon it is to abandon our ways and earn yourself the hatred of our order.");
    ...
    plugin::Whisper("Thanks for resolving this issue. Here is your reward");
