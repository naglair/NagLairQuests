...
function event_click_door(e)
	...
	if ((door_id >= 8 and door_id <=12) and (raid.valid or e.self:Admin() > 80)) then
		-- assign player the instance of the raid leader
		eq.flag_instance_by_raid_leader(223,0);
		...
		if (instance_id == nil or instance_id == 0) then
				...
				for i = 0, player_list_count - 1, 1 do
					...
					if (client_v.valid) then
						...
						if (e.self:Admin() <= 80 and qglobals["potimeb_lockout"] ~= nil) then
							e.self:Message(13, client_v:GetCleanName().." has a lockout, cannot join instance.");
			...
			if (HasLockout > 0) then
				...
				--eq.set_global("TimeHack_Lock",""..HasLockout,5,"D7");
				NotLocked = player_list_count - HasLockout;
				--eq.set_global("TimeHack_NotLock",""..NotLocked,5,"D7");
				return;
			end
			--No lockouts, create instance
			-- the instance is only good for 3 days(259200 seconds).
			e.self:Message(13,"Creating a new instance with id: "..instance_id.." .");
			instance_id = eq.create_instance("potimeb",0,259200);
			-- create a qglobal for status tracking
			eq.set_global(instance_id.."_potimeb_status","Phase1",7,"D3");
		...
		if (instance_id > 0) then
			e.self:Message(13,"You are assigned to instance id: " .. instance_id .." , zoning you there now!");
			local phase = eq.get_qglobals()[instance_id .. '_potimeb_status'];
			e.self:Message(13,"This instance is in phase ".. phase..".");
