...
sub EVENT_SAY{
  if ($text=~/hail/i) {
    plugin::Whisper("What brings you to my home, $name? Do you seek knowledge of Rallos Zek and the great wars to come?");
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 18083 => 1)) { #Redblade's Legacy
    plugin::Whisper("So, Kargek sent you.  How is my dear brother?  If you come to me with this book then you must be interested in the scabbard. Take this totem and return to me with it and the heart of frost from the goblins of permafrost. Do not return until this task is accomplished."); 
    quest::ding();
    quest::exp(100000);
  ...
  if (plugin::check_handin(\%itemcount, 20681 => 1, 11622 => 1, 11582 => 1)) { #totem of fiery war, red/green dragons scales
    ...
    plugin::Whisper("What are..? 'You have done well, mortal, and you will be rewarded if you are truly brave. Travel to the Plane of Hatred and slay the ancient bard who lives there. Do not return without his hand and this mark or you will never be a true warlord.' Tenal drops to the ground as the last words leave his lips.");
    quest::doanim(16);
    quest::ding();
    quest::exp(100000);
